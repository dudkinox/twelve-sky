...
$Id: md5.cpp,v 1.1 2007/03/21 06:14:07 miracle Exp $
...
void MD5::update(FILE *file)
{
	...
	while ((len=::fread(read_buffer, 1, 1024, file))) {
...
void MD5::update(istream& stream){
	...
	::memset(read_buffer, 0, sizeof(read_buffer));
	while (stream.good()){
		stream.read(read_buffer, 1024); // note that return value of read is unusable.
		len=stream.gcount();
		update(reinterpret_cast<unsigned char *>(read_buffer), len);
	}
