...
int CWorkerProcess::start_svr_service(void) 
{
	...
	unsigned int uiCheckedTime = 0xFFFFFFFF;
#ifdef __EPOLL__
	int res = 0;
	int epfd = -1;
	...
	try {
		if(!mSERVER.Init(&ret_val)) {
			throw Exception("CWorkerProcess::start_svr_service", "server object initialization failed", ret_val);
		}
		...
		for( ; ; ) {
			...
			if ((GetTickCount () - uiCheckedTime) > 10000) {
				uiCheckedTime = GetTickCount ();
				mSERVER.PROCESS_FOR_TIMER();
			}
			...
			for(i = 0; i < res; i++) {
				if(events[i].data.fd == listenfd) {
					...
					if(client_sockfd > 0) {
						...
						::setsockopt (client_sockfd, IPPROTO_TCP, TCP_NODELAY, &iOptNoDelay, sizeof (int));
						...
						if(ProcessResponse(WM_NETWORK_MESSAGE_1, &ev.data.fd, FD_ACCEPT, (void *)&cli_addr.sin_addr) != 0) {
                                                        ev.events |= EPOLLOUT;
