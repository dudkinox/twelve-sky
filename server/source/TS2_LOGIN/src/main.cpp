...
static void load_config(void) 
{
    ...
    char db_acct_str[MAX_SIZE_OF_CONFIG_ENTRY];
    char db_host_addr_str[MAX_SIZE_OF_CONFIG_ENTRY];
    char db_host_addr_str2[MAX_SIZE_OF_CONFIG_ENTRY];
    char db_host_port_str[MAX_SIZE_OF_CONFIG_ENTRY];
    char db_acct_passwd_str[MAX_SIZE_OF_CONFIG_ENTRY];

	char db2_name_str[MAX_SIZE_OF_CONFIG_ENTRY];
    char db2_acct_str[MAX_SIZE_OF_CONFIG_ENTRY];
    char db2_host_addr_str[MAX_SIZE_OF_CONFIG_ENTRY];
    char db2_host_addr_str2[MAX_SIZE_OF_CONFIG_ENTRY];
    char db2_host_port_str[MAX_SIZE_OF_CONFIG_ENTRY];
    char db2_acct_passwd_str[MAX_SIZE_OF_CONFIG_ENTRY];
    char premium_server[MAX_SIZE_OF_CONFIG_ENTRY]; // @_Premium_Server_@
	/*****************************************************************/
	...
	ret_val += config_obj_ptr->GetValue(CONFIG_KEY_DATABASE_HOST_ADDR2, db_host_addr_str2);
	...
	ret_val += config_obj_ptr->GetValue(CONFIG_KEY_DATABASE2_HOST_ADDR, db2_host_addr_str);
	ret_val += config_obj_ptr->GetValue(CONFIG_KEY_DATABASE2_HOST_ADDR2, db2_host_addr_str2);
	ret_val += config_obj_ptr->GetValue(CONFIG_KEY_DATABASE2_HOST_PORT, db2_host_port_str);
	ret_val += config_obj_ptr->GetValue(CONFIG_KEY_PREMIUM_SERVER, premium_server); // @_Premium_Server_@
	/*****************************************************************/
#ifdef __USE_DB_POOL__
	ret_val += config_obj_ptr->GetValue(CONFIG_KEY_DATABASE_NAME, db_name_str);
	ret_val += config_obj_ptr->GetValue(CONFIG_KEY_DATABASE_ACCT, db_acct_str);
	...
	if (ret_val != 41) {
#else 
	if (ret_val != 28) { // 프리미엄 서버 여부 값 추가로 인해 +1 // @_Premium_Server_@
#endif
    ...
    CHECK_CONFIG_VARIABLE(premium_server,                            CONFIG_KEY_PREMIUM_SERVER); // @_Premium_Server_@
#ifdef __USE_DB_POOL__
	...
	CHECK_CONFIG_VARIABLE(db_acct_passwd_str,                        CONFIG_KEY_DATABASE_PW);
	CHECK_CONFIG_VARIABLE(db_host_addr_str,                          CONFIG_KEY_DATABASE_HOST_ADDR);
	CHECK_CONFIG_VARIABLE(db_host_addr_str2,                         CONFIG_KEY_DATABASE_HOST_ADDR2);
	CHECK_CONFIG_VARIABLE(db_host_port_str,                          CONFIG_KEY_DATABASE_HOST_PORT);
	CHECK_CONFIG_VARIABLE(db_name_str,                               CONFIG_KEY_DATABASE_NAME);
	...
	CHECK_CONFIG_VARIABLE(db2_host_addr_str,                          CONFIG_KEY_DATABASE2_HOST_ADDR);
	CHECK_CONFIG_VARIABLE(db2_host_addr_str2,                         CONFIG_KEY_DATABASE2_HOST_ADDR2);
	...
	g_env.db2_host_addr_str_obj = db2_host_addr_str;
	g_env.db2_host_port = ::atoi(db2_host_port_str);


    ::memset (&mSERVER_INFO, 0, sizeof (mSERVER_INFO));
	::snprintf(mSERVER_INFO.mServerName, 100, "%s", svr_name_str);
	mSERVER_INFO.mServerPort = g_env.listen_port;
	
	::snprintf(mSERVER_INFO.mDB_1_IP[0], 16, "%s", db_host_addr_str);
	::snprintf(mSERVER_INFO.mDB_1_IP[1], 16, "%s", db_host_addr_str2);
    ...
    ::snprintf(mSERVER_INFO.mDB_2_IP[0], 16, "%s", db2_host_addr_str);
    ::snprintf(mSERVER_INFO.mDB_2_IP[1], 16, "%s", db2_host_addr_str2);
	...
	g_env.premium_server = ::atoi(premium_server); // @_Premium_Server_@
	mSERVER_INFO.mCheckPremiumServer = g_env.premium_server; // @_Premium_Server_@

    // Login Server Specific Configuration.
    dictionary* pIni = iniparser_load (g_config_str_obj.c_str());
    if (pIni == NULL) {
        LOG_TO_BOTH_1 ("Could not parse file (%s). \n", DEFAULT_LOGFILE_PATH);
        ::exit (1);
    }
    
	mSERVER_INFO.mIslandNum = iniparser_getint (pIni, "SERVER:ISLAND_NUM", -1);
    if (mSERVER_INFO.mIslandNum == -1) {
        LOG_TO_BOTH_2 ("Could not found keyword (%s) in (%s) section.", "SERVER", "ISLAND_NUM");
        ::exit (1);
    }

	mSERVER_INFO.mPartitionNum = iniparser_getint (pIni, "SERVER:PARTITION_NUM", -1);
    if (mSERVER_INFO.mPartitionNum == -1) {
        LOG_TO_BOTH_2 ("Could not found keyword (%s) in (%s) section.", "SERVER", "PARTITION_NUM");
        ::exit (1);
    }

    // World Index (1 based).
    mSERVER_INFO.mWorldNumber = iniparser_getint (pIni, "SERVER:WORLD_NUM", -1);
    if (mSERVER_INFO.mWorldNumber == -1) {
        LOG_TO_BOTH_2 ("Could not found keyword (%s) in (%s) section.", "SERVER", "WORLD_NUM");
        ::exit (1);
    }

    // Client - Login Version.
    mSERVER_INFO.mServerVersion = iniparser_getint (pIni, "SERVER:VERSION", -1);
    if (mSERVER_INFO.mServerVersion == -1) {
        LOG_TO_BOTH_2 ("Could not found keyword (%s) in (%s) section.", "SERVER", "VERSION");
        ::exit (1);
    }

    // Upper Server (PlayUser) connection ip info.
    char* pUpperIP = iniparser_getstring (pIni, "SERVER:UPPER_SVR1_IP", NULL);
    if (pUpperIP == NULL) {
        LOG_TO_BOTH_2 ("Could not found keyword (%s) in (%s) section.", "SERVER", "UPPER_SVR1_IP");
        ::exit (1);
    }
    LOG_TO_BOTH_1 ("!PLAYUSER SERVER IP [%s]\n", pUpperIP);
    ::snprintf (mSERVER_INFO.mUpperServer_1_IP, 16, "%s", pUpperIP);

    // Upper Server (PlayUser) connection port info.
    mSERVER_INFO.mUpperServer_1_Port = iniparser_getint (pIni, "SERVER:UPPER_SVR1_PORT", -1);
    if (mSERVER_INFO.mUpperServer_1_Port == -1) {
        LOG_TO_BOTH_2 ("Could not found keyword (%s) in (%s) section.", "SERVER", "UPPER_SVR1_PORT");
        ::exit (1);
    }
		
    // Upper Server (Center) connection ip info.
    pUpperIP = iniparser_getstring (pIni, "SERVER:UPPER_SVR2_IP", NULL);
    if (pUpperIP == NULL) {
        LOG_TO_BOTH_2 ("Could not found keyword (%s) in (%s) section.", "SERVER", "UPPER_SVR2_IP");
        ::exit (1);
    }
   ::snprintf (mSERVER_INFO.mUpperServer_2_IP, 16, "%s", pUpperIP);

    // Upper Server (Center) connection port info.
    mSERVER_INFO.mUpperServer_2_Port = iniparser_getint (pIni, "SERVER:UPPER_SVR2_PORT", -1);
    if (mSERVER_INFO.mUpperServer_2_Port == -1) {
        LOG_TO_BOTH_2 ("Could not found keyword (%s) in (%s) section.", "SERVER", "UPPER_SVR2_PORT");
        ::exit (1);
    }

    // Upper Server (Log) connection ip info.
    pUpperIP = iniparser_getstring (pIni, "SERVER:UPPER_SVR3_IP", NULL);
    if (pUpperIP == NULL) {
        LOG_TO_BOTH_2 ("Could not found keyword (%s) in (%s) section.", "SERVER", "UPPER_SVR3_IP");
        ::exit (1);
    }
    ::snprintf (mSERVER_INFO.mUpperServer_3_IP, 16, "%s", pUpperIP);

    // Upper Server (Log) connection port info.
    mSERVER_INFO.mUpperServer_3_Port = iniparser_getint (pIni, "SERVER:UPPER_SVR3_PORT", -1);
    if (mSERVER_INFO.mUpperServer_3_Port == -1) {
        LOG_TO_BOTH_2 ("Could not found keyword (%s) in (%s) section.", "SERVER", "UPPER_SVR3_PORT");
        ::exit (1);
    }

    // Upper Server (Iyoyo-AP) connection ip info.
    pUpperIP = iniparser_getstring (pIni, "SERVER:UPPER_SVR4_IP", NULL);
    if (pUpperIP == NULL) {
        LOG_TO_BOTH_2 ("Could not found keyword (%s) in (%s) section.", "SERVER", "UPPER_SVR4_IP");
        ::exit (1);
    }
    ::snprintf (mSERVER_INFO.mUpperServer_4_IP, 16, "%s", pUpperIP);

    // Upper Server (Iyoyo-AP) connection port info.
    mSERVER_INFO.mUpperServer_4_Port = iniparser_getint (pIni, "SERVER:UPPER_SVR4_PORT", -1);
    if (mSERVER_INFO.mUpperServer_4_Port == -1) {
        LOG_TO_BOTH_2 ("Could not found keyword (%s) in (%s) section.", "SERVER", "UPPER_SVR4_PORT");
        ::exit (1);
    }
#ifdef __CHANNEL__
	// Upper Server (Company) connection ip info.
	pUpperIP = iniparser_getstring (pIni, "SERVER:UPPER_SVR5_IP", NULL);
	if (pUpperIP == NULL) {
		LOG_TO_BOTH_2 ("Could not found keyword (%s) in (%s) section.", "SERVER", "UPPER_SVR5_IP");
		::exit (1);
	}
	::snprintf (mSERVER_INFO.mUpperServer_5_IP, 16, "%s", pUpperIP);

	// Upper Server (Company) connection port info.
	mSERVER_INFO.mUpperServer_5_Port = iniparser_getint (pIni, "SERVER:UPPER_SVR5_PORT", -1);
	if (mSERVER_INFO.mUpperServer_5_Port == -1) {
		LOG_TO_BOTH_2 ("Could not found keyword (%s) in (%s) section.", "SERVER", "UPPER_SVR4_PORT");
		::exit (1);
	}
#endif
    // Iyoyo-AP server number info.
    mSERVER_INFO.mIyoyoAPNum = iniparser_getint (pIni, "SERVER:IYOYO_AP_NUM", -1);
    if (mSERVER_INFO.mIyoyoAPNum == -1) {
        LOG_TO_BOTH_2 ("Could not found keyword (%s) in (%s) section.", "SERVER", "IYOYO_AP_NUM");
        ::exit (1);
    }
#ifdef __CHANNEL__
	// company number info.
	mSERVER_INFO.mCompanyNum = iniparser_getint (pIni, "SERVER:COMPANY_NUM", -1);
	if (mSERVER_INFO.mCompanyNum == -1) {
		LOG_TO_BOTH_2 ("Could not found keyword (%s) in (%s) section.", "SERVER", "COMPANY_NUM");
		::exit (1);
	}

	// game number info.
	mSERVER_INFO.mGameNum = iniparser_getint (pIni, "SERVER:GAME_NUM", -1);
	if (mSERVER_INFO.mGameNum == -1) {
		LOG_TO_BOTH_2 ("Could not found keyword (%s) in (%s) section.", "SERVER", "GAME_NUM");
		::exit (1);
	}
#endif

    // ACCOUNT..MemberInfo.
    char* pTableName = iniparser_getstring (pIni, "SERVER:DB1TABLE01", NULL);
    if (pTableName == NULL) {
        LOG_TO_BOTH_2 ("Could not found keyword (%s) in (%s) section.", "SERVER", "DB1TABLE01");
        ::exit (1);
    }
    ::strncpy (mSERVER_INFO.mDB_1_Table01, pTableName, ::strlen (pTableName));

    // ACCOUNT..AvatarNameInfo.
    pTableName = iniparser_getstring (pIni, "SERVER:DB1TABLE02", NULL);
    if (pTableName == NULL) {
        LOG_TO_BOTH_2 ("Could not found keyword (%s) in (%s) section.", "SERVER", "DB1TABLE02");
        ::exit (1);
    }
    ::strncpy (mSERVER_INFO.mDB_1_Table02, pTableName, ::strlen (pTableName));
    
    // ACCOUNT..GiftInfo.
    pTableName = iniparser_getstring (pIni, "SERVER:DB1TABLE03", NULL);
    if (pTableName == NULL) {
        LOG_TO_BOTH_2 ("Could not found keyword (%s) in (%s) section.", "SERVER", "DB1TABLE03");
        ::exit (1);
    }
    ::strncpy (mSERVER_INFO.mDB_1_Table03, pTableName, ::strlen (pTableName));

    // ACCOUNT_SECRET..SecretCardInfo.
    pTableName = iniparser_getstring (pIni, "SERVER:DB1TABLE04", NULL);
    if (pTableName == NULL) {
        LOG_TO_BOTH_2 ("Could not found keyword (%s) in (%s) section.", "SERVER", "DB1TABLE04");
        ::exit (1);
    }
    ::strncpy (mSERVER_INFO.mDB_1_Table04, pTableName, ::strlen (pTableName));

    // GAME_01..RankInfo.
    pTableName = iniparser_getstring (pIni, "SERVER:DB1TABLE05", NULL);
    if (pTableName == NULL) {
        LOG_TO_BOTH_2 ("Could not found keyword (%s) in (%s) section.", "SERVER", "DB1TABLE05");
        ::exit (1);
    }
    ::strncpy (mSERVER_INFO.mDB_1_Table05, pTableName, ::strlen (pTableName));

    // ACCOUNT_GAME_01..WorldInfo.
    pTableName = iniparser_getstring (pIni, "SERVER:DB1TABLE06", NULL);
    if (pTableName == NULL) {
        LOG_TO_BOTH_2 ("Could not found keyword (%s) in (%s) section.", "SERVER", "DB1TABLE06");
        ::exit (1);
    }
    ::strncpy (mSERVER_INFO.mDB_1_Table06, pTableName, ::strlen (pTableName));

    // ACCOUNT_GAME_01..TribeInfo.
    pTableName = iniparser_getstring (pIni, "SERVER:DB1TABLE07", NULL);
    if (pTableName == NULL) {
        LOG_TO_BOTH_2 ("Could not found keyword (%s) in (%s) section.", "SERVER", "DB1TABLE07");
        ::exit (1);
    }
    ::strncpy (mSERVER_INFO.mDB_1_Table07, pTableName, ::strlen (pTableName));
	
	// ACCOUNT_GAME_01..MemberAuth.
    pTableName = iniparser_getstring (pIni, "SERVER:DB1TABLE08", NULL);
    if (pTableName == NULL) {
        LOG_TO_BOTH_2 ("Could not found keyword (%s) in (%s) section.", "SERVER", "DB1TABLE08");
        ::exit (1);
    }
    ::strncpy (mSERVER_INFO.mDB_1_Table08, pTableName, ::strlen (pTableName));

    // ACCOUNT_GAME_01..WebMall.
    pTableName = iniparser_getstring (pIni, "SERVER:DB1TABLE09", NULL);
    if (pTableName == NULL) {
        LOG_TO_BOTH_2 ("Could not found keyword (%s) in (%s) section.", "SERVER", "DB1TABLE09");
        ::exit (1);
    }
    ::strncpy (mSERVER_INFO.mDB_1_Table09, pTableName, ::strlen (pTableName));

    // ACCOUNT_GAME_01..GiftItemList.
    pTableName = iniparser_getstring (pIni, "SERVER:DB1TABLE10", NULL);
    if (pTableName == NULL) {
        LOG_TO_BOTH_2 ("Could not found keyword (%s) in (%s) section.", "SERVER", "DB1TABLE10");
        ::exit (1);
    }
    ::strncpy (mSERVER_INFO.mDB_1_Table10, pTableName, ::strlen (pTableName));

    // GAME_01..DefineMaxUserNum.
    pTableName = iniparser_getstring (pIni, "SERVER:DB2TABLE01", NULL);
    if (pTableName == NULL) {
        LOG_TO_BOTH_2 ("Could not found keyword (%s) in (%s) section.", "SERVER", "DB2TABLE01");
        ::exit (1);
    }
    ::strncpy (mSERVER_INFO.mDB_2_Table01, pTableName, ::strlen (pTableName));

    // GAME_01..MasterInfo.
    pTableName = iniparser_getstring (pIni, "SERVER:DB2TABLE02", NULL);
    if (pTableName == NULL) {
        LOG_TO_BOTH_2 ("Could not found keyword (%s) in (%s) section.", "SERVER", "DB2TABLE02");
        ::exit (1);
    }
    ::strncpy (mSERVER_INFO.mDB_2_Table02, pTableName, ::strlen (pTableName));

    // GAME_01..AvatarInfo.
    pTableName = iniparser_getstring (pIni, "SERVER:DB2TABLE03", NULL);
    if (pTableName == NULL) {
        LOG_TO_BOTH_2 ("Could not found keyword (%s) in (%s) section.", "SERVER", "DB2TABLE03");
        ::exit (1);
    }
    ::strncpy (mSERVER_INFO.mDB_2_Table03, pTableName, ::strlen (pTableName));

    // GAME_01..DeleteAvatarInfo.
    pTableName = iniparser_getstring (pIni, "SERVER:DB2TABLE04", NULL);
    if (pTableName == NULL) {
        LOG_TO_BOTH_2 ("Could not found keyword (%s) in (%s) section.", "SERVER", "DB2TABLE04");
        ::exit (1);
    }
    ::strncpy (mSERVER_INFO.mDB_2_Table04, pTableName, ::strlen (pTableName));
...
int main(int argc, char **argv)
{
    ...
    if (mSERVER_INFO.mCheckPremiumServer == 1) {
        LOG_TO_BOTH  ("!********************************************************\n");
        LOG_TO_BOTH  ("!***************  Premium Server Start!!  ***************\n");
        LOG_TO_BOTH  ("!********************************************************\n");
    } // @
