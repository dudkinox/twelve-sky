...
void W_LOGIN_SEND( int tUserIndex )
{
	...
	mUSER[tUserIndex].uLoginPremiumPCRoom = mDB.DB_PROCESS_14(mUSER[tUserIndex].mIP, tID);
	/* Mir PCRoom check - end  */
	mDB.DB_PROCESS_02( &mUSER[tUserIndex].mIP[0], &tID[0], &tPassword[0], &mUSER[tUserIndex].uID[0], &mUSER[tUserIndex].uCheckMOTP, &mUSER[tUserIndex].uMousePassword[0], &mUSER[tUserIndex].uSecretCard, &mUSER[tUserIndex].uUserSort, &mUSER[tUserIndex].uGoodFellow, &mUSER[tUserIndex].uLoginPlace, &mUSER[tUserIndex].uLoginPremium, &mUSER[tUserIndex].uTraceState, &mUSER[tUserIndex].uBonus100Money, &mUSER[tUserIndex].uPremiumServerExpirationDate, &mUSER[tUserIndex].uSaveMoney, mUSER[tUserIndex].uSaveItem, mUSER[tUserIndex].mAvatarInfo, &mUSER[tUserIndex].mAuthInfo, &tResult, &first_login ); // &mUSER[tUserIndex].uPremiumServerExpirationDate 추가. // @_Premium_Server_@
#endif
	if( tResult != 0 )
	{
		mTRANSFER.B_LOGIN_RECV( tResult, &tID[0], 0, 0, 0, 0, 0, 0, "0000", 0, 0 );
	...
	mPLAYUSER_COM.U_REGISTER_USER_FOR_LOGIN_1_SEND( &mUSER[tUserIndex].mIP[0], &mUSER[tUserIndex].uID[0], mUSER[tUserIndex].uUserSort, mUSER[tUserIndex].uGoodFellow, mUSER[tUserIndex].uLoginPlace, mUSER[tUserIndex].uLoginPremium, mUSER[tUserIndex].uLoginPremiumPCRoom, mUSER[tUserIndex].uTraceState, mUSER[tUserIndex].uBonus100Money, mUSER[tUserIndex].uPremiumServerExpirationDate, &mUSER[tUserIndex].mAuthInfo); // mUSER[tUserIndex].uPremiumServerExpirationDate 추가. // @_Premium_Server_@
...
//-------------------------------------------------------------------------------------------------
  ...
  tAvatarInfo.aKillMonsterNum2 = 0; // 몬스터 사냥 수에 따른 기여도 제공 여부 계산용 변수 - 몬스터를 사냥한 수(count).
...
void W_CHANGE_AVATAR_NAME_SEND( int tUserIndex )
{
	...
	// 2009.12.18  : 김성수
	// 통합에 의한 무명변환의 경우에 아래를 체크 하지 않는다.
	...
	else // 서버 통합에 의한 무명 변환의 경우 == 아바타 무명에 특수문자가 들어 가 있는 경우 == tPage, tIndex 가 -1인 경우.
    {
    	if (mGAME.CheckNameString( mUSER[tUserIndex].mAvatarInfo[tAvatarPost].aName ) == FALSE)
        {
        	isChangeNameIntegration = true;
        }
        else // 서버 통합에 의한 무명 변환도 아닌데 -1, -1을 보내다니...
        {
			mUSER[tUserIndex].Quit(__FILE__, __FUNCTION__, __LINE__);
			return;
        }
	...
	//이름변경 - 이게 실패 한다는 것은 동일 무명이 있는 경우와 그외에 쿼리가 잘못되었을 경우이다.
	// 따라서 이 경우 실패시에 동일 무명이 있다고 판단한다.
