...
#define MAX_SIZE_OF_FD       MAX_USER_NUM
int CWorkerProcess::start_svr_service(void) 
{
    ...
    try {
		if(!mSERVER.Init(&ret_val)) {
			//LOG_TO_FILE ("The mserver init end");
		...
		//LOG_TO_FILE ("The epoll creating\n");
		epfd = epoll_create (MAX_SIZE_OF_FD);
		if (!epfd)
		{
			
			//LOG_TO_FILE ("The epoll create failed\n");
		...
		{
			LOG_TO_FILE ("!The epoll ctl add falied\n");
			LOG_TO_FILE_2 ("![%s] : %s\n", __FUNCTION__, strerror (errno));
		...
		for ( ; ;)
		{
			...
			if (res == -1)
			{
				if (EINTR != errno)
				{
					LOG_TO_FILE ("!The epoll wait not einter\n");
					LOG_TO_FILE_2 ("![%s] : %s\n", __FUNCTION__, strerror (errno));
					continue;
			...
			for (i = 0; i < res; ++i)
			{
				...
				{
					//LOG_TO_FILE_2("current evetn : %d %d\n", events[i].events & EPOLLIN, events[i].events & EPOLLOUT);
					if((events[i].events & EPOLLIN)) {
						n_read = ProcessResponse (WM_NETWORK_MESSAGE_1, &events[i].data.fd, FD_READ);
						//LOG_TO_FILE_2 ("---> 0x0%x, n_read %d\n", events[i].events & EPOLLIN, n_read);
#if 0
						if (n_read < 0)
						{
							if (errno != EAGAIN && errno != EWOULDBLOCK)
							{
								LOG_TO_FILE_2 ("!Recv error of %s occur : %d\n", strerror (errno), events[i].data.fd);
							...
							if (!(events[i].events & EPOLLIN))
							{
								LOG_TO_FILE_1 ("!Epoll event isn't EPOLLIN on recving : %d\n", events[i].data.fd);
						...
						{
							//LOG_TO_FILE_1 ("Recv size is zero : %d\n", events[i].data.fd);
							ev.data.fd = events[i].data.fd;
							ev.events = EPOLLIN | EPOLLERR | EPOLLHUP;
							epoll_ctl (epfd, EPOLL_CTL_DEL, events[i].data.fd, &ev);
							ProcessResponse (WM_NETWORK_MESSAGE_1, &events[i].data.fd, FD_CLOSE);
