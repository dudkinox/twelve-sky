...
BOOL ZONENPCINFO::Init( void )
{
...
#ifdef __WIN32__
	hFile = CreateFile( "DATA\\002.BIN", GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL );
	...
	while(1) {
		nRead = fread(mDATA, 1
				, sizeof(ZONENPCINFODATA) * MAX_ZONE_NUMBER_NUM , r_fp);
		if(nRead <= 0)
			break;
		nReadByte += nRead;
	}

	if(nReadByte != ( sizeof(ZONENPCINFODATA) * MAX_ZONE_NUMBER_NUM ))  {
		fclose(r_fp);
		return FALSE;
	}
	fclose(r_fp);
...
BOOL HACKINFO::Init( void )
{
...
#ifdef __WIN32__
	hFile = CreateFile( "HACKINFO.TXT", GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL );
	if( hFile == INVALID_HANDLE_VALUE )
	{
		return FALSE;
	}
	...
	mTotalHackInfoNum = atoi( tempString01 );
	for( index01 = 0 ; index01 < mTotalHackInfoNum ; index01++ )
	{
		...
		mHackSize[index01] = atoi( tempString01 );
	}
	if( !CloseHandle( hFile ) )
	{
		return FALSE;
