...
void MyWork::ProcessForRelay( int tSort, BYTE tData[MAX_BROADCAST_DATA_SIZE], int type )
{
	...
	case 113 : //[TRIBE_NOTICE]
		...
		mTRANSFER.B_TRIBE_NOTICE_RECV( &tAvatarName01[0], &tContent[0], type );
		for( index01 = 0 ; index01 < mSERVER.mMAX_USER_NUM ; index01++ )
		{
			if( !mAVATAR_OBJECT[index01].mCheckValidState )
			{
...
void MyWork::ProcessForData( int tUserIndex, int tSort, BYTE tData[MAX_BROADCAST_DATA_SIZE] )
{
	...
	case 234 : //[USER]-ITEM
		...
		{
		case 1 : //[활력단]
			if( mUSER[tUserIndex].mAvatarInfo.aLifePotionConvertNum < 1 )
			{
				mUSER[tUserIndex].Quit(__FILE__, __FUNCTION__, __LINE__);
			               
				return;
			}
			if( !mUTIL.ProcessForDropItem( 9, 1017, 0, 0, 0, &mAVATAR_OBJECT[tUserIndex].mDATA.aAction.aLocation[0], &mAVATAR_OBJECT[tUserIndex].mDATA.aName[0], 0 ) )
			{
				mTRANSFER.B_PROCESS_DATA_RECV( 1, tSort, &tData[0] );
				mUSER[tUserIndex].Send( TRUE, (char *) &mTRANSFER.mOriginal[0], mTRANSFER.mOriginalSize );
				return;
			...
			mUSER[tUserIndex].mAvatarInfo.aLifePotionConvertNum--;
			mTRANSFER.B_PROCESS_DATA_RECV( 0, tSort, &tData[0] );
			mUSER[tUserIndex].Send( TRUE, (char *) &mTRANSFER.mOriginal[0], mTRANSFER.mOriginalSize );
			return;
		case 2 : //[방파부]
			...
			if( !mUTIL.ProcessForDropItem( 9, 1103, 0, 0, 0, &mAVATAR_OBJECT[tUserIndex].mDATA.aAction.aLocation[0], &mAVATAR_OBJECT[tUserIndex].mDATA.aName[0], 0 ) )
			{
				mTRANSFER.B_PROCESS_DATA_RECV( 1, tSort, &tData[0] );
				mUSER[tUserIndex].Send( TRUE, (char *) &mTRANSFER.mOriginal[0], mTRANSFER.mOriginalSize );
				return;
			...
			return;
		case 3 : //[내공단]
			if( mUSER[tUserIndex].mAvatarInfo.aManaPotionConvertNum < 1 )
			{
				mUSER[tUserIndex].Quit(__FILE__, __FUNCTION__, __LINE__);
			               
				return;
			}
			if( !mUTIL.ProcessForDropItem( 9, 1018, 0, 0, 0, &mAVATAR_OBJECT[tUserIndex].mDATA.aAction.aLocation[0], &mAVATAR_OBJECT[tUserIndex].mDATA.aName[0], 0 ) )
			{
				mTRANSFER.B_PROCESS_DATA_RECV( 1, tSort, &tData[0] );
				mUSER[tUserIndex].Send( TRUE, (char *) &mTRANSFER.mOriginal[0], mTRANSFER.mOriginalSize );
				return;
			...
			mUSER[tUserIndex].mAvatarInfo.aManaPotionConvertNum--;
			mTRANSFER.B_PROCESS_DATA_RECV( 0, tSort, &tData[0] );
			mUSER[tUserIndex].Send( TRUE, (char *) &mTRANSFER.mOriginal[0], mTRANSFER.mOriginalSize );
			return;
		default :
	...
	case 235 : //[공덕치]->[기여도]
		...
		return;
	case 236:
		if(6 >= (mGAME.mTickCount -  mUSER[tUserIndex].mTickCountFor03Second)){
			mTRANSFER.B_PROCESS_DATA_RECV( 1, tSort, &tData[0] );
			mUSER[tUserIndex].Send( TRUE, (char *) &mTRANSFER.mOriginal[0], mTRANSFER.mOriginalSize );
			return;
		}
		...
		return;
	case 237 : //[...]->[..]
	    return; // 2009.12.23 - 유럽, 터키 기여도 전환 시스템 제거 요청.
		...
		mGAMELOG.GL_802_USE_KILLOTHERTRIBE_TO_ITEM( tUserIndex, mUSER[tUserIndex].mAvatarInfo.aKillOtherTribe + 30, mUSER[tUserIndex].mAvatarInfo.aKillOtherTribe);
		/** 2009. 12. 18 : 기여도 관련 로그 추가 (wolf) */
		return;
	//------------//
	//[GM]-COMMAND//
	...
	case 504 : //[GM]-MONEY
		...
		if( !mUTIL.ProcessForDropItem( 4, 1, tValue, 0, 0, &mAVATAR_OBJECT[tUserIndex].mDATA.aAction.aLocation[0], "", 0 ) )
		{
			return;
		}
	...
	case 505 : //[GM]-ITEM
		...
		if( !mUTIL.ProcessForDropItem( 4, tValue, 0, 0, 0, &mAVATAR_OBJECT[tUserIndex].mDATA.aAction.aLocation[0], "", 0 ) )
		{
			return;
		}
	...
	case 601 : //DUEL START
		...
		for(index01 = 0; index01 < mSERVER.mMAX_USER_NUM; ++index01) {
			...
			if(tCheck) {
				...
				mTRANSFER.B_DUEL_START_RECV( &mAVATAR_OBJECT[index01].mDATA.aDuelState[0], mAVATAR_OBJECT[index01].mDuelProcessRemainTime );
				mUSER[index01].Send( TRUE, (char *) &mTRANSFER.mOriginal[0], mTRANSFER.mOriginalSize );
