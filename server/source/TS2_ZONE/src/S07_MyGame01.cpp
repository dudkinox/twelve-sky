    ...
    if ((mSERVER_INFO.mCheckPremiumServer == 1) && (tPresentTime != NULL)) {
        if ((tPresentTime->tm_hour == 23) && (tPresentTime->tm_min == 55)) {
            if (mPremiumServerExpirationCountDown < 0) {
                ...
                mPremiumServerExpirationCountDown = 5; // 카운트 5분으로 설정.
            }
        }
        ...
        if ((mPremiumServerExpirationCountDown >= 0) && ((mTickCount % 120) == 0)) {
            ...
            if (mPremiumServerExpirationCountDown < 0) {
                ...
                mUSER[index01].uPremiumServerExpirationDate = 0; // 실제 DB의 값을 update 하지는 않습니다.
            }
        }
    } // @
...
// 함수를 호출하는 서버에 관계없이 동맹세력을 리턴하는 함수를 별도로 작성하여 ReturnAllianceTribe 함수를 수정합니다.
...
//PROCESS_ZONE_051_TYPE (광도성 20 ~ 29).
...
void MyGame::GetLevelBattleRewardInfo( int tLevel1, int tLevel2, int *tGainExperience, int *tGainMoney, BOOL tWin )
{
	...
	*tGainMoney = 0;
	...
	// 2009.10.26 : 김성수
	// 26일 수령한 4분기10월 업데이트 정규쟁 패치분
	// 아래의 /*/를 //*/로 고쳐주면 해당 업데이트분이 적용됩니다.
	// 명일 필리핀 패치가 있는 관계로 작업내용을 주석으로 막아 놓습니다.
	// 2009.11.06 : 김성수
	// 엑셀에 있는 정보를 읽을때, int 변수에 오버 플로어가 발생하는 하기 때문에 하드 코딩으로 다시 변경
...
void MyGame::Process_Zone_175_TYPE( void )
{
	...
	case  5 : //[1단계_진행]
		...
		for( index01 = 0 ; index01 < mSERVER.mMAX_USER_NUM ; index01++ )
		{
			...
			// 2010.1.18 : 김성수 : 무신 붙이면서 처리한거 
			//쟁 보상시에 점혈상태일떄 .. 해당 케릭터의 점혈 상태를 풀어 준다
...
// 2010.01.07 이전에 년도가 바뀌면 자동물약 관련 필드가 초기화되는 문제가 있어
// 관련 함수(MyGame::Add_Use_Auto_Day, MyGame::Check_Use_Auto_Day)를 수정하였습니다.
//
// 이전에는 aUse_AutoPotion_Time에 남은 일수가 저장되어 있었는데 관련 로직을 변경한 후에는
// aUse_AutoPotion_Time은 사용하지 않습니다(단, Client에서는 aUse_AutoPotion_Time 변수를 사용 -
//                                          서버에서 보내준 날짜를 임시로 저장하는 변수).
// aUse_AutoPotion_Date에는 time() 값(1970.01.01 부터 자동물약 사용 만기일까지의 초(sec.))을 저장합니다.
// time() - 2038년 문제는 무시합니다.
BOOL MyGame::Add_Use_Auto_Day(int iIndex, int iDay)
{
    const int SEC_PER_DAY = 86400; // 1일을 초로 계산 - 60 * 60 * 24 == 86400초
    ...
    // 2010.01.07 - 서버에서는 aUse_AutoPotion_Time 변수(DB 필드 포함)는 사용하지 않습니다.
    // Client에서는 aUse_AutoPotion_Time 변수를 사용하지만 서버에서 보내준 날짜를
    // 임시로 저장하는 변수로 사용합니다.
...
int MyGame::Check_Use_Auto_Day(int mIndex)
{
    ...
    if (mUSER[mIndex].mAvatarInfo.aUse_AutoPotion_Date > 0)
    {
        ...
        return ( ((int)((mUSER[mIndex].mAvatarInfo.aUse_AutoPotion_Date - nowTimeSec) / SEC_PER_DAY)) + 1 ); // 잔여 일수.
    }
    else
    {
        mUSER[mIndex].mAvatarInfo.aUse_AutoPotion_Date = 0;
...
int MyGame::CheckMoveZoneForMoveZoneItem(int tTribe, int tLevel, int tZoneNumber)
{
    ...
    // 입장가능 세력 및 레벨 검사.
    if (tCheckTribe != -1) { // 세력 제한이 있는 존 검사.
