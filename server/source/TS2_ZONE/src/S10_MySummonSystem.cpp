...
void MySummonSystem::Free( void )
{
	...
	if( mMONSTER_SUMMON )
	{
		free( mMONSTER_SUMMON );
		mMONSTER_SUMMON = NULL;
	}
	mTotalBossMonsterSummonInfoNum = 0;
	if( mBOSS_MONSTER_SUMMON )
	{
		free( mBOSS_MONSTER_SUMMON );
		mBOSS_MONSTER_SUMMON = NULL;
	}
}
//LOAD_REGION_INFO_1
void MySummonSystem::LoadRegionInfo_1( char *tFileName )
{
	...
	if((nReadByte = fread(&tTotalRegionInfoNum, 1, 4, r_fp)) < 0) {
		fclose(r_fp);
		return;
	}
	...
	if( tTotalRegionInfoNum > 0 )
	{
		tRegionInfo = (WORLD_REGION_INFO *)calloc(1, ( tTotalRegionInfoNum * sizeof( WORLD_REGION_INFO ) ) );
		if( tRegionInfo == NULL )
		{
			fclose(r_fp);
			return;
		...
		//if((nReadByte = fread(&tRegionInfo[0], 1, ( tTotalRegionInfoNum * sizeof( WORLD_REGION_INFO ) ), r_fp)) < 0) {
		if((nReadByte = fread(tRegionInfo, 1, ( tTotalRegionInfoNum * sizeof( WORLD_REGION_INFO ) ), r_fp)) < 0) {
			free( tRegionInfo );
			fclose(r_fp);
			return;
		}
