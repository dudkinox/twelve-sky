...
/* but uses gjrand_mrand48() to generate the numbers. */
...
"blatgj_m size bigseed\n"
"blatgj_m size seed0 seed1 seed2\n"
"blatgj_m size seed0 seed1 seed2 a0 a1 a2 c\n"
"size is required file size in bytes, or inf");

    if (argc==3)
    {
	if (sscanf(argv[2], "%ld", &seedval)!=1) crash("bigseed not numeric");
	gjrand_srand48(seedval);
    }
    else
    {
	...
	if (argc==9)
	{
	    ...
	    gjrand_lcong48(xsubi);
	}
	else (void)gjrand_seed48(xsubi);
    }
