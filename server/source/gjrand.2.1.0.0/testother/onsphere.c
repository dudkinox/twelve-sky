...
test(int dim, unsigned long count, struct gjrand *s)
{
    ...
    printf("radius anomaly max = %15e   min = %15e\n\n",
	sqrt(hi)-1.0,sqrt(lo)-1.0);
    for (i=0; i<dim; i++)
...
main(int argc, char **argv)
{
    ...
    if (argc!=4 && argc!=3) crash("usage: onsphere dim count [ seed ]");

    if (sscanf(argv[1],"%d",&dim)!=1 || dim<=0)
	crash("first arg dim should be a  positive integer");
    if (dim>=MAX) crash("first arg dim is too big");
