...
/* This one looks for bad things in 6 dimensions. */
...
dobuf(double fpart[], unsigned char ipart[], int s)
{
    ...
    for (i= -DIM; i<0; i++) index=ipart[i]+8*index;

    for (i=0; i<s; i++)
    {
	index = (ipart[i]+8*index) & DIMMASK;
	stats[index][6]++;
	stats[index][0] += fpart[i];
	stats[index][1] += fpart[i-1];
	stats[index][2] += fpart[i-2];
	stats[index][3] += fpart[i-3];
	stats[index][4] += fpart[i-4];
	stats[index][5] += fpart[i-5];
    }
}
...
doan(int64_t s)
{
    ...
    t-=DIMMASK;
    t/=sqrt(2.0*DIMMASK);
    t*=0.89; /* very approx fudge factor to compensate for co-variance. */
