...
void MyTransfer::B_REGISTER_USER_FOR_ZONE_1_RECV( int tResult, int tUserSort, int tGoodFellow, int tLoginPlace, int tLoginPremium, int tLoginPremiumPCRoom, int tTraceState, char tBonus100Money, int tPremiumServerExpirationDate, AVATAR_INFO *tAvatarInfo, int tEffectValue[MAX_AVATAR_EFFECT_SORT_NUM][2], int tCheckFirstZone, AUTH_INFO *tAuthInfo, MOVE_ITEM_INFO *tMoveItemInfo ) // 함수 선언 수정 - int tPremiumServerExpirationDate 추가. // @_Premium_Server_@ // 이동서 사용정보(MOVE_ITEM_INFO *tMoveItemInfo) 추가.
{
	mOriginal[0] = P_REGISTER_USER_FOR_ZONE_1_RECV;
	CopyMemory( &mOriginal[1], &tResult, 4 );
	CopyMemory( &mOriginal[5], &tUserSort, 4 );
	CopyMemory( &mOriginal[9], &tGoodFellow, 4 );
	CopyMemory( &mOriginal[13], &tLoginPlace, 4 );
...
void MyTransfer::B_USER_ZONE_SEND_RECV_CASH( int tResult )
{
	mOriginal[0] = P_USER_ZONE_RECV_CASH;
	CopyMemory( &mOriginal[1], &tResult, 4 );
	mOriginalSize = S_USER_ZONE_RECV_CASH;
}
