...
void W_REGISTER_USER_FOR_LOGIN_1_SEND( int tUserIndex )
{
	...
	CopyMemory( &tLoginPlace, &tPacket[( 25 + MAX_USER_ID_LENGTH )], 4 );
	...
	tResult = mGAME.RegisterUserForLogin_01( &tIP[0], &tID[0], tUserSort, tGoodFellow, tLoginPlace, tLoginPremium, tLoginPremiumPCRoom, tTraceState, tBonus100Money, tPremiumServerExpirationDate, &tPlayUserIndex, &tAuthInfo ); // tPremiumServerExpirationDate 추가. // @_Premium_Server_@
...
void W_REGISTER_USER_FOR_ZONE_1_SEND( int tUserIndex )
{
	...
	tResult = mGAME.RegisterUserForZone_01( mUSER[tUserIndex].mZoneServerNumber, tPlayUserIndex, &tID[0], &tAvatarName[0], &tLogoutInfo[0], &tUserSort, &tGoodFellow, &tLoginPlace, &tLoginPremium, &tLoginPremiumPCRoom, &tTraceState, &tBonus100Money, &tPremiumServerExpirationDate, &tAvatarInfo, tEffectValue, &tCheckFirstZone, &tAuthInfo ); // &tPremiumServerExpirationDate 추가. // @_Premium_Server_@
	mTRANSFER.B_REGISTER_USER_FOR_ZONE_1_RECV( tResult, tUserSort, tGoodFellow, tLoginPlace, tLoginPremium, tLoginPremiumPCRoom, tTraceState, tBonus100Money, tPremiumServerExpirationDate, &tAvatarInfo, tEffectValue, tCheckFirstZone, &tAuthInfo, &tMoveItemInfo ); // tPremiumServerExpirationDate 추가. // @_Premium_Server_@ // 이동서 사용 정보(&tMoveItemInfo) 추가.
	mUSER[tUserIndex].Send( TRUE, (char *) &mTRANSFER.mOriginal[0], mTRANSFER.mOriginalSize );
}
...
void W_REGISTER_USER_FOR_ZONE_3_SEND( int tUserIndex )
{
	...
	tResult = mGAME.RegisterUserForZone_03( mUSER[tUserIndex].mZoneServerNumber, tPlayUserIndex, &tID[0], &tAvatarInfo, tEffectValue, &tAuthInfo );
...
void W_USER_ZONE_SEND_CASH( int tUserIndex )
{
	char *tPacket = &mUSER[tUserIndex].mBUFFER_FOR_RECV[0];
	if( mUSER[tUserIndex].mCheckServerType != 2 )
	{
	...
	//LOG_TO_FILE_2("%s %d",tID,tSort);
	tResult = 0;
	if( !mDB.DB_PROCESS_16( &tID[0], tSort) )
	{
		tResult = 1;
	}
	mTRANSFER.B_USER_ZONE_SEND_RECV_CASH( tResult );
	mUSER[tUserIndex].Send( TRUE, (char *) &mTRANSFER.mOriginal[0], mTRANSFER.mOriginalSize );
}
