...
int MyServer::PROCESS_FOR_NETWORK(SOCKET *cli_fd, UINT fd_status, void *arg = NULL)
{
	...
	switch(fd_status)
	{
		...
		case FD_ACCEPT :
			...
			sprintf( mUSER[index01].mIP, "%s"
					, inet_ntoa(((struct in_addr)((arg_t *)arg)->cli_addr->sin_addr)));
			//LOG_TO_FILE_1("!client addr : %s\n", mUSER[index01].mIP);
...
void MyServer::PROCESS_FOR_TIMER( void )
{
	...
	//LOG_TO_FILE_1("!Current user num : %d\n", mGAME.ReturnPresentUserNum(0));
}
#ifdef __WIN32__
//WRITE_FOR_ERROR
