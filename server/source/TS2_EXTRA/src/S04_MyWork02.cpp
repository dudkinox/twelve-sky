...
#else
			if(strcasecmp( tGuildInfo1.gSubMaster01, "" ) == 0)
#endif
//-------------------------------------------------------------------------------------------------
...
{
	//LOG_TO_FILE ("!GET_CASH_SIZE_FOR_EXTRA_SEND\n");
	char *tPacket = &mUSER[tUserIndex].mBUFFER_FOR_RECV[0];
	if( mUSER[tUserIndex].mCheckServerType != 2 )
	{
...
{
	//LOG_TO_FILE ("!BUY_CASH_ITEM_FOR_EXTRA\n");
	char *tPacket = &mUSER[tUserIndex].mBUFFER_FOR_RECV[0];
	if( mUSER[tUserIndex].mCheckServerType != 2 )
	{
	...
	{
		LOG_TO_FILE_1 ("!CostInfoIndex error[%d]\n", tCostInfoIndex);
	...
	{
		LOG_TO_FILE_3("!CostInfoValue error :tCostInfoIndex = %d iIndex = %d ,mCostInfoValue = %d  \n" 
...
void W_UPDATE_CASH_ITEM_INFO_FOR_EXTRA_SEND(int tUserIndex)
{
    ...
    {
        LOG_TO_FILE("!--------------------------------------------------------------\n");
        LOG_TO_FILE("!-------------------- Reload COST_INFO.TXT --------------------\n");
        LOG_TO_FILE("!--------------------------------------------------------------\n");
        if (mGAME.Init())
        {
            mTRANSFER.B_UPDATE_CASH_ITEM_INFO_FOR_EXTRA_RECV( 0 );
            mUSER[tUserIndex].Send( TRUE, (char *) &mTRANSFER.mOriginal[0], mTRANSFER.mOriginalSize );
            LOG_TO_FILE("!--------------------   Reload SUCCESS   ----------------------\n");
            LOG_TO_FILE("!--------------------------------------------------------------\n");
            return;
        }
        else // 잘못된 COST_INFO인 경우.
        {
            mTRANSFER.B_UPDATE_CASH_ITEM_INFO_FOR_EXTRA_RECV( 2 );
            mUSER[tUserIndex].Send( TRUE, (char *) &mTRANSFER.mOriginal[0], mTRANSFER.mOriginalSize );
            LOG_TO_FILE("!--------------------   Reload FAIL !!   ----------------------\n");
            LOG_TO_FILE("!--------------------------------------------------------------\n");
            return;
        }
    }
    else if (tVersion != mGAME.mCashItemInfoVersion) // 잘못된 COST_INFO인 경우.
    {
        mTRANSFER.B_UPDATE_CASH_ITEM_INFO_FOR_EXTRA_RECV( 2 );
        mUSER[tUserIndex].Send( TRUE, (char *) &mTRANSFER.mOriginal[0], mTRANSFER.mOriginalSize );
        return;
...
void W_GET_CASH_ITEM_INFO_FOR_EXTRA_SEND( int tUserIndex )
{
    ...
    int tZoneNumber = 0;
    CopyMemory( &tZoneNumber, &tPacket[1], 4 );
    LOG_TO_FILE_2("!Zone(%d) - CashItemInfo Update(Version - %d)!\n", tZoneNumber, mGAME.mCashItemInfoVersion);
...
void W_USE_PACKAGE_ITEM_FOR_EXTRA_SEND( int tUserIndex )
{
    ...
    int tResult = 1;
#ifdef __CASH_COM__
    if (mCASH_COM.UsePackageItem(&tID[0], iIndex) == 1) { // 성공.
    	tResult = 0;
    }
#endif
