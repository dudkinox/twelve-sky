...
BOOL MyDB::Init( void )
{
				...
				, g_env.db_name_str_obj.c_str() , mSERVER_INFO.mDB_1_ID, mSERVER_INFO.mDB_1_PASSWORD)) 
	{
		...
		LOG_TO_FILE_1("[%s] DB Connection false! \n" , __FUNCTION__);
		return FALSE;
	}
#endif
...
#endif
	if(mDB02.set_db_conn_info(mSERVER_INFO.mDB2_1_IP[0], mSERVER_INFO.mDB2_1_IP[1], mSERVER_INFO.mDB2_1_PORT
				, g_env.db2_name_str_obj.c_str (), mSERVER_INFO.mDB2_1_ID, mSERVER_INFO.mDB2_1_PASSWORD) < 0) {
		return FALSE;
	}
	if(!mDB02.connect()) {
		return FALSE;
...
BOOL MyDB::RECONNECT_TO_DATABASE01( void )
{
	...
	LOG_TO_FILE_1("[%s] DB reonnection call! \n" , __FUNCTION__);

	if( !mDB01.Reconnect() )
	{
		// 2008.11.19 Kim SungSoo : Add log for reconnect false
		LOG_TO_FILE_1("[%s] DB reonnection false! \n" , __FUNCTION__);
		return FALSE;
	}
#ifdef __TK__
	if(mDB01.exec_query("SET character SET latin5;", CMySQL::CLEAR) != 0)  {
...
BOOL MyDB::DB_PROCESS_02( char tGuildName[MAX_GUILD_NAME_LENGTH] )
{
	...
	if((iCounts = mDB01.get_num_rows()) != 1) {
		...
		LOG_TO_FILE_2("[%s] Query get rownum false : %s! \n" , __FUNCTION__ , mQUERY);
		return false;
	}  
	mDB01.clear_res();
...
BOOL MyDB::DB_PROCESS_03( char tGuildName[MAX_GUILD_NAME_LENGTH], char tMasterName[MAX_AVATAR_NAME_LENGTH] )
{
	...
	gMember[12] = '0';
	gMember[850] = '\0';
	sprintf( mQUERY, "insert into %s values (now() , %d ,'%s',1,'%s','','','%s','','','','' )", mSERVER_INFO.mDB_2_Table02, g_env.world_num, tGuildName, tMasterName, gMember );
	if( !RECONNECT_TO_DATABASE01() )
	{
		return FALSE;
	}
...
BOOL MyDB::DB_PROCESS_04( char tGuildName[MAX_GUILD_NAME_LENGTH], GUILD_INFO *tGuildInfo )
{
	...
	sprintf( mQUERY, "select gName,gGrade,gMaster01,gSubMaster01,gSubMaster02,gMember,gNotice01,gNotice02,gNotice03,gNotice04 from %s where gName='%s' AND aWorldNumber = %d"
	...
	if((iAffected = mDB01.exec_query(mQUERY)) < 0) {
		...
		LOG_TO_FILE_2("![%s] : exec_query fail %s\n", __FUNCTION__, mQUERY);
		return false;
	}
	iCounts = 0;
#if 0
	if((iCounts = mDB01.get_num_rows()) != 1) {
		...
		LOG_TO_FILE_2("![%s] : exec_query rows wrong : row = %s\n", __FUNCTION__, iCounts);
		return false;
	}
	...
	if(pDBValue == NULL)
	{
		LOG_TO_FILE_1("![%s] : get_value is NULL when gName\n", __FUNCTION__);
		return false;
	}
	...
	if(pDBValue == NULL)
	{
		LOG_TO_FILE_1("![%s] : get_value is NULL when gGrade\n", __FUNCTION__);
		return false;
	}
	...
	if(pDBValue == NULL)
	{
		LOG_TO_FILE_1("![%s] : get_value is NULL when gMaster01\n", __FUNCTION__);
		return false;
	}
	...
	if(pDBValue == NULL)
	{
		LOG_TO_FILE_1("![%s] : get_value is NULL when gSubMaster01\n", __FUNCTION__);
		return false;
	}
	...
	if(pDBValue == NULL)
	{
		LOG_TO_FILE_1("![%s] : get_value is NULL when gSubMaster02\n", __FUNCTION__);
		return false;
	}
	...
	if(pDBValue == NULL)
	{
		LOG_TO_FILE_1("![%s] : get_value is NULL when gMember\n", __FUNCTION__);
		return false;
	}
	...
	if(pDBValue == NULL)
	{
		LOG_TO_FILE_1("![%s] : get_value is NULL when gNotice01\n", __FUNCTION__);
		return false;
	}
	...
	if(pDBValue == NULL)
	{
		LOG_TO_FILE_1("![%s] : get_value is NULL when gNotice02\n", __FUNCTION__);
		return false;
	}
	...
	if(pDBValue == NULL)
	{
		LOG_TO_FILE_1("![%s] : get_value is NULL when gNotice03\n", __FUNCTION__);
		return false;
	}
	...
	if(pDBValue == NULL)
	{
		LOG_TO_FILE_1("![%s] : get_value is NULL when gNotice04\n", __FUNCTION__);
		return false;
	}
...
BOOL MyDB::DB_PROCESS_05( char tGuildName[MAX_GUILD_NAME_LENGTH], GUILD_INFO *tGuildInfo )
{
	...
	sprintf( mQUERY, "update %s set gGrade=%d,gSubMaster01='%s',gSubMaster02='%s',gMember='%s',gNotice01='%s',gNotice02='%s'\
			...
			, tGuildInfo->gNotice[1], tGuildInfo->gNotice[2], tGuildInfo->gNotice[3], tGuildInfo->gName  , g_env.world_num);
	if( !RECONNECT_TO_DATABASE01() )
	{
		return FALSE;
	}
