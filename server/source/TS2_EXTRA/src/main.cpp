...
static void load_config(void) 
{
	...
	char cash_ip_addr_str[MAX_SIZE_OF_CONFIG_ENTRY];
	char cash_port_str[MAX_SIZE_OF_CONFIG_ENTRY];
	/*****************************************************************/
	...
	ret_val += config_obj_ptr->GetValue(CONFIG_KEY_SVR_CASH_IP_ADDR, cash_ip_addr_str);
	ret_val += config_obj_ptr->GetValue(CONFIG_KEY_SVR_CASH_PORT, cash_port_str);
	/*****************************************************************/
#ifdef __USE_DB_POOL__
	ret_val += config_obj_ptr->GetValue(CONFIG_KEY_DATABASE_NAME, db_name_str);
	ret_val += config_obj_ptr->GetValue(CONFIG_KEY_DATABASE_ACCT, db_acct_str);
	LOG_TO_BOTH_1("Load config [CONFIG_KEY_DATABASE_ACCT] result: %d \n" , ret_val);
	
	ret_val += config_obj_ptr->GetValue(CONFIG_KEY_DATABASE_PW, db_acct_passwd_str);
	LOG_TO_BOTH_1("Load config [CONFIG_KEY_DATABASE_PW] result: %d \n" , ret_val);
	
	ret_val += config_obj_ptr->GetValue(CONFIG_KEY_DATABASE_HOST_ADDR, db_host_addr_str);
	LOG_TO_BOTH_1("Load config [CONFIG_KEY_DATABASE_HOST_ADDR] result: %d \n" , ret_val);
	
	ret_val += config_obj_ptr->GetValue(CONFIG_KEY_DATABASE_HOST_PORT, db_host_port_str);
	LOG_TO_BOTH_1("Load config [CONFIG_KEY_DATABASE_HOST_PORT] result: %d \n" , ret_val);
	
	ret_val += config_obj_ptr->GetValue(CONFIG_KEY_DATABASE_START_UP_CONN, db_start_up_conn_str);
	LOG_TO_BOTH_1("Load config [CONFIG_KEY_DATABASE_START_UP_CONN] result: %d \n" , ret_val);
	
	ret_val += config_obj_ptr->GetValue(CONFIG_KEY_DATABASE_MIN_SPARE_CONN, db_min_spare_conn_str);
	LOG_TO_BOTH_1("Load config [CONFIG_KEY_DATABASE_MIN_SPARE_CONN] result: %d \n" , ret_val);
	
	ret_val += config_obj_ptr->GetValue(CONFIG_KEY_DATABASE_MAX_SPARE_CONN, db_max_spare_conn_str);
	LOG_TO_BOTH_1("Load config [CONFIG_KEY_DATABASE_MAX_SPARE_CONN] result: %d \n" , ret_val);
	
	ret_val += config_obj_ptr->GetValue(CONFIG_KEY_DATABASE_MAX_ALLOWED_CONN, db_max_allowed_conn_str);
	LOG_TO_BOTH_1("Load config [CONFIG_KEY_DATABASE_MAX_ALLOWED_CONN] result: %d \n" , ret_val);
	
	ret_val += config_obj_ptr->GetValue(CONFIG_KEY_DATABASE_MAINTENANCE_TIME, db_maintenance_time_str);
	LOG_TO_BOTH_1("Load config [CONFIG_KEY_DATABASE_MAINTENANCE_TIME] result: %d \n" , ret_val);
	...
	CHECK_CONFIG_VARIABLE(cash_ip_addr_str,                          CONFIG_KEY_SVR_CASH_IP_ADDR);
	CHECK_CONFIG_VARIABLE(cash_port_str,                             CONFIG_KEY_SVR_CASH_PORT);
	/*****************************************************************/
	...
	CHECK_CONFIG_VAR_RANGE(cash_port_str,                             CONFIG_KEY_SVR_CASH_PORT,                1,    65535);
	...
	g_env.cash_addr_str_obj = cash_ip_addr_str;
	g_env.cash_port = ::atoi(cash_port_str);
	...
	::snprintf(mSERVER_INFO.mUpperServer_1_IP, 16, "%s", g_env.cash_addr_str_obj.c_str());
	mSERVER_INFO.mUpperServer_1_Port = g_env.cash_port;
	::snprintf(mSERVER_INFO.mUpperServer_2_IP, 16, "%s", g_env.gamelog_addr_str_obj.c_str());
	mSERVER_INFO.mUpperServer_2_Port = g_env.gamelog_port;
	...
	::snprintf(mSERVER_INFO.mDB_1_Table01, 100, "%s", "MemberInfo");
	::snprintf(mSERVER_INFO.mDB_2_Table01, 100, "%s", "AvatarInfo");
	::snprintf(mSERVER_INFO.mDB_2_Table02, 100, "%s", "GuildInfo"); //aWorldNumber
	::snprintf(mSERVER_INFO.mDB_3_Table01, 100, "%s", "t_log_itemlog");

	//Cash DB
	::snprintf(mSERVER_INFO.mDB2_1_IP[0], 16, "%s", db2_host_addr_str[0]);
	::snprintf(mSERVER_INFO.mDB2_1_IP[1], 16, "%s", db2_host_addr_str[1]);
	mSERVER_INFO.mDB2_1_PORT = g_env.db2_host_port;
	::snprintf(mSERVER_INFO.mDB2_1_ID, 100, "%s", g_env.db2_acct_str_obj.c_str());
	::snprintf(mSERVER_INFO.mDB2_1_PASSWORD, 100, "%s", g_env.db2_acct_passwd_str_obj.c_str());
	::snprintf(mSERVER_INFO.mDB2_1_Table01, 100, "%s", "MemberInfo");
	//-------
	/*****************************************************************/
}
...
int main(int argc, char **argv)
{
	...
	for( ; ; ) {
		...
		if((new_pid = fork()) < 0) {
			...
			//LOG_TO_BOTH_1("Logfile path %s", DEFAULT_LOGFILE_PATH);
