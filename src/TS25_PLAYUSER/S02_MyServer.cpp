...
BOOL MyServer::Init(int* tResult)
{
	...
	if (!SetTimer(hMainWindow, 1, 1000, nullptr))
	{
		*tResult = 1;
		return FALSE;
	}
...
int MyServer::PROCESS_FOR_NETWORK(HWND hWnd, WPARAM wPrm, LPARAM lPrm)
{
	...
	//__LogMgr()->Log("IN Address [%s] [%d]",inet_ntoa(ClientAddr.sin_addr),ClientAddr.sin_port);
...
void MyServer::PROCESS_FOR_TIMER(void)
{
	...
	//__LogMgr()->Log("!Current user num : %d", mGAME.ReturnPresentUserNum(0));
}
