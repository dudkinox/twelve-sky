...
void MyTransfer::B_REGISTER_USER_FOR_ZONE_1_RECV(int tResult, int tUserSort, AVATAR_INFO* tAvatarInfo,
                                                 int tEffectValue[MAX_AVATAR_EFFECT_SORT_NUM][2], int tCheckFirstZone,
                                                 AUTH_INFO* tAuthInfo,
                                                 MOVE_ITEM_INFO* tMoveItemInfo)
// 함수 선언 수정 - int tPremiumServerExpirationDate 추가. // @_Premium_Server_@ // 이동서 사용정보(MOVE_ITEM_INFO *tMoveItemInfo) 추가.
{
	mOriginal[0] = P_REGISTER_USER_FOR_ZONE_1_RECV;
	CopyMemory(&mOriginal[1], &tResult, 4);
	CopyMemory(&mOriginal[5], &tUserSort, 4);
	CopyMemory(&mOriginal[34], tAvatarInfo, SIZE_OF_AVATAR_INFO);
	CopyMemory(&mOriginal[(34 + SIZE_OF_AVATAR_INFO)], tEffectValue, (MAX_AVATAR_EFFECT_SORT_NUM * 8));
	CopyMemory(&mOriginal[(34 + SIZE_OF_AVATAR_INFO + MAX_AVATAR_EFFECT_SORT_NUM * 8)], &tCheckFirstZone, 4);
	CopyMemory(&mOriginal[(38 + SIZE_OF_AVATAR_INFO + MAX_AVATAR_EFFECT_SORT_NUM * 8)], tAuthInfo, sizeof(AUTH_INFO));
	CopyMemory(&mOriginal[(38 + SIZE_OF_AVATAR_INFO + MAX_AVATAR_EFFECT_SORT_NUM * 8 + sizeof(AUTH_INFO))],
	           tMoveItemInfo, sizeof(MOVE_ITEM_INFO));
