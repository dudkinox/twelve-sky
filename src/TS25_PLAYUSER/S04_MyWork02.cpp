...
void W_REGISTER_USER_FOR_LOGIN_1_SEND(int tUserIndex)
{
	...
	CopyMemory(&tAuthInfo, &tPacket[( 17 + 4 + MAX_USER_ID_LENGTH )], sizeof(AUTH_INFO));

	int tResult;
	int tPlayUserIndex;

	tResult = mGAME.RegisterUserForLogin_01(&tIP[0], &tID[0], tUserSort, &tPlayUserIndex, &tAuthInfo);
	// tPremiumServerExpirationDate 추가. // @_Premium_Server_@
...
void W_REGISTER_USER_FOR_LOGIN_3_SEND(int tUserIndex)
{
	...
	CopyMemory(&tID[0], &tPacket[5], MAX_USER_ID_LENGTH);
	tID[(MAX_USER_ID_LENGTH - 1)] = '\0';
	CopyMemory(&tAvatarInfo, &tPacket[( 5 + MAX_USER_ID_LENGTH )], SIZE_OF_AVATAR_INFO);
...
void W_REGISTER_USER_FOR_ZONE_1_SEND(int tUserIndex)
{
	...
	tResult = mGAME.RegisterUserForZone_01(mUSER[tUserIndex].mZoneServerNumber, tPlayUserIndex, &tID[0],
	                                       &tAvatarName[0], &tLogoutInfo[0], &tUserSort, &tAvatarInfo, tEffectValue,
	                                       &tCheckFirstZone, &tAuthInfo);
	mTRANSFER.B_REGISTER_USER_FOR_ZONE_1_RECV(tResult, tUserSort, &tAvatarInfo, tEffectValue, tCheckFirstZone,
	                                          &tAuthInfo, &tMoveItemInfo);
	mUSER[tUserIndex].Send(TRUE, (char*)&mTRANSFER.mOriginal[0], mTRANSFER.mOriginalSize);
}
...
void W_REGISTER_USER_FOR_ZONE_2_SEND(int tUserIndex)
{
	...
	CopyMemory(&tAvatarInfo, &tPacket[( 5 + MAX_USER_ID_LENGTH )], SIZE_OF_AVATAR_INFO);
	mGAME.SafeAvatarInfoData(&tAvatarInfo);
	CopyMemory(tEffectValue, &tPacket[( 5 + MAX_USER_ID_LENGTH + SIZE_OF_AVATAR_INFO )],
	           ( MAX_AVATAR_EFFECT_SORT_NUM * 8 ));
	CopyMemory(&tAuthInfo,
	           &tPacket[( 5 + MAX_USER_ID_LENGTH + SIZE_OF_AVATAR_INFO + ( MAX_AVATAR_EFFECT_SORT_NUM * 8 ))],
	           sizeof (AUTH_INFO));

	int tResult;
	tResult = mGAME.RegisterUserForZone_02(mUSER[tUserIndex].mZoneServerNumber, tPlayUserIndex, &tID[0], &tAvatarInfo,
	                                       tEffectValue, &tAuthInfo);
...
void W_REGISTER_USER_FOR_ZONE_3_SEND(int tUserIndex)
{
	...
	tResult = mGAME.RegisterUserForZone_03(mUSER[tUserIndex].mZoneServerNumber, tPlayUserIndex, &tID[0], &tAvatarInfo,
	                                       tEffectValue, &tAuthInfo);
...
void W_UNREGISTER_USER_FOR_ZONE_SEND(int tUserIndex)
{
	...
	tID[(MAX_USER_ID_LENGTH - 1)] = '\0';
	CopyMemory(&tAvatarInfo, &tPacket[( 5 + MAX_USER_ID_LENGTH )], SIZE_OF_AVATAR_INFO);
	CopyMemory(&tAuthInfo, &tPacket[( 5 + MAX_USER_ID_LENGTH + SIZE_OF_AVATAR_INFO)], sizeof( AUTH_INFO ));
	mGAME.SafeAvatarInfoData(&tAvatarInfo);
...
void W_TRIBE_BANK_INFO_LOAD_SEND(int tUserIndex)
{
	...
	if ((tTribe < 0) || (tTribe > 3) || (tTribeBankInfoIndex < 0) || (tTribeBankInfoIndex > (MAX_TRIBE_BANK_SLOT_NUM -
		1)) || (tInventoryMoney < 0))
	{
...
void W_EVENT_FOR_ZONE_SEND(int tUserIndex)
{
	...
	mDB.DB_PROCESS_15(mGAME.mID[tPlayUserIndex], mGAME.mAvatarInfo[tPlayUserIndex].aName,
	                  mGAME.mAvatarInfo[tPlayUserIndex].aTribe, tKillNum);
}
