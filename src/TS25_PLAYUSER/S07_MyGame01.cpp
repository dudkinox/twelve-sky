...
BOOL MyGame::Init(void)
{
	...
	mEHandle = CreateEvent(nullptr, FALSE, FALSE, nullptr);
	if (mEHandle == nullptr)
	{
		return FALSE;
	}
	mTHandle = CreateThread(nullptr, 0, THREAD_FOR_DB_WORKER, nullptr, 0, &tID);
	if (mTHandle == nullptr)
	{
		return FALSE;
	}
...
BOOL MyGame::LoadForTribeBankInfo(void)
{
	...
	hFile = CreateFile("DATA/TRIBE_BANK_INFO.DAT", GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING,
	...
	if (!ReadFile(hFile, &mTribeBankInfo[0][0], (16 * MAX_TRIBE_BANK_SLOT_NUM), &nReadByte, nullptr))
	{
		CloseHandle(hFile);
		return FALSE;
	}
...
void MyGame::SaveForTribeBankInfo(void)
{
	...
	hFile = CreateFile("DATA/TRIBE_BANK_INFO.DAT", GENERIC_WRITE, FILE_SHARE_WRITE, nullptr, CREATE_ALWAYS,
	                   FILE_ATTRIBUTE_NORMAL, nullptr);
	if (hFile == INVALID_HANDLE_VALUE)
	{
		return;
	}
	if (!WriteFile(hFile, &mTribeBankInfo[0][0], (16 * MAX_TRIBE_BANK_SLOT_NUM), &nWriteByte, nullptr))
	{
		CloseHandle(hFile);
		return;
	}
...
void MyGame::ProcessForPlayUser(void)
{
	...
	for (index01 = 0; index01 < MAX_PLAY_USER_NUM; index01++)
	{
		...
		{
			if ((tPresentTime - mPostTime[index01]) < 180000) // Locked Account 10mins 180000 original
			{
				continue;
			}
		}
		...
		ProcessForSave_1(1, &mID[index01][0], &mAvatarInfo[index01], &mAuthInfo[index01]);
...
//PROCESS_FOR_SAVE_1
void MyGame::ProcessForSave_1(int tSort, char tID[MAX_USER_ID_LENGTH], AVATAR_INFO* tAvatarInfo, AUTH_INFO* tAuthInfo)
{
	while (TRUE)
	{
		...
		else
		{
			...
			CopyMemory(&mSave1AvatarInfo[mTotalSave1Num], tAvatarInfo, SIZE_OF_AVATAR_INFO);
...
void MyGame::ProcessForSave_2(void)
{
	...
	for (index01 = 0; index01 < mTotalSave1Num; index01++)
	{
		...
		CopyMemory(&mSave2AvatarInfo[index01], &mSave1AvatarInfo[index01], SIZE_OF_AVATAR_INFO);
	...
	{
		mDB.DB_PROCESS_03(mSave2Sort[index01], &mSave2ID[index01][0], &mSave2AvatarInfo[index01],
		                  &mSave2AuthInfo[index01]);
	}
}
...
int MyGame::RegisterUserForLogin_01(char tIP[16], char tID[MAX_USER_ID_LENGTH], int tUserSort, int* tPlayUserIndex,
                                    AUTH_INFO* tAuthInfo)
...
int MyGame::RegisterUserForLogin_02(int tPlayUserIndex, char tID[MAX_USER_ID_LENGTH])
{
	if ((tPlayUserIndex < 0) || (tPlayUserIndex > (MAX_PLAY_USER_NUM - 1)) || (!mCheckValidState[tPlayUserIndex]) || (
		mZoneNumber[tPlayUserIndex] != 0) || (strcmp(mID[tPlayUserIndex], tID) != 0))
	{
		return 1;
	}
...
int MyGame::RegisterUserForLogin_03(int tPlayUserIndex, char tID[MAX_USER_ID_LENGTH], AVATAR_INFO* tAvatarInfo)
{
	...
	mSaveTime[tPlayUserIndex] = GetTickCount();
	mPostTime[tPlayUserIndex] = GetTickCount();
	CopyMemory(&mAvatarInfo[tPlayUserIndex], tAvatarInfo, SIZE_OF_AVATAR_INFO);
...
int MyGame::RegisterUserForZone_01(int tZoneNumber, int tPlayUserIndex, char tID[MAX_USER_ID_LENGTH],
                                   char tAvatarName[MAX_AVATAR_NAME_LENGTH], int tLogoutInfo[6], int* tUserSort,
                                   AVATAR_INFO* tAvatarInfo, int tEffectValue[MAX_AVATAR_EFFECT_SORT_NUM][2],
                                   int* tCheckFirstZone,
                                   AUTH_INFO* tAuthInfo)
// 함수 선언 수정 - int *tPremiumServerExpirationDate 추가. // @_Premium_Server_@
{
	if ((tPlayUserIndex < 0) || (tPlayUserIndex > (MAX_PLAY_USER_NUM - 1)) || (!mCheckValidState[tPlayUserIndex]) || (
		mZoneNumber[tPlayUserIndex] != tZoneNumber) || (strcmp(mID[tPlayUserIndex], tID) != 0) || (strcmp(
		mAvatarInfo[tPlayUserIndex].aName, tAvatarName) != 0))
	{
		if (tPlayUserIndex >= 0)
		{
			mCheckValidState[tPlayUserIndex] = false;
	...
	*tUserSort = mUserSort[tPlayUserIndex];
	CopyMemory(tAvatarInfo, &mAvatarInfo[tPlayUserIndex], SIZE_OF_AVATAR_INFO);
...
int MyGame::RegisterUserForZone_02(int tZoneNumber, int tPlayUserIndex, char tID[MAX_USER_ID_LENGTH],
                                   AVATAR_INFO* tAvatarInfo, int tEffectValue[MAX_AVATAR_EFFECT_SORT_NUM][2],\
                                   AUTH_INFO* tAuthInfo)
{
	if ((tPlayUserIndex < 0) || (tPlayUserIndex > (MAX_PLAY_USER_NUM - 1)) || (!mCheckValidState[tPlayUserIndex]) || (
		mZoneNumber[tPlayUserIndex] != tZoneNumber) || (strcmp(mID[tPlayUserIndex], tID) != 0) || (strcmp(
		mAvatarInfo[tPlayUserIndex].aName, tAvatarInfo->aName) != 0))
	{
		if (tPlayUserIndex >= 0)
		{
			mCheckValidState[tPlayUserIndex] = false;
	...
	if ((tTickCount - mSaveTime[tPlayUserIndex]) >= 170000)
	{
		...
		ProcessForSave_1(2, &mID[tPlayUserIndex][0], &mAvatarInfo[tPlayUserIndex], &mAuthInfo[tPlayUserIndex]);
	}
	return 0;
}

//REGISTER_USER_FOR_ZONE_03
int MyGame::RegisterUserForZone_03(int tZoneNumber, int tPlayUserIndex, char tID[MAX_USER_ID_LENGTH],
                                   AVATAR_INFO* tAvatarInfo, int tEffectValue[MAX_AVATAR_EFFECT_SORT_NUM][2],\
                                   AUTH_INFO* tAuthInfo)
{
	if ((tPlayUserIndex < 0) || (tPlayUserIndex > (MAX_PLAY_USER_NUM - 1)) || (!mCheckValidState[tPlayUserIndex]) || (
...
void MyGame::UnRegisterUserForZone(int tZoneNumber, int tPlayUserIndex, char tID[MAX_USER_ID_LENGTH],
                                   AVATAR_INFO* tAvatarInfo, AUTH_INFO* tAuthInfo)
{
	...
	CopyMemory(&mAvatarInfo[tPlayUserIndex], tAvatarInfo, SIZE_OF_AVATAR_INFO);
	CopyMemory(&mAuthInfo[tPlayUserIndex], tAuthInfo, sizeof( AUTH_INFO ));
	ProcessForSave_1(1, &mID[tPlayUserIndex][0], &mAvatarInfo[tPlayUserIndex], &mAuthInfo[tPlayUserIndex]);
...
int MyGame::ProcessKillOtherTribe(char tAvatarName1[MAX_AVATAR_NAME_LENGTH], char tAvatarName2[MAX_AVATAR_NAME_LENGTH])
{
	...
	for (index01 = 0; index01 < MAX_PLAY_USER_NUM; index01++)
	{
		...
		if ((strcmp(mKillOtherTribeAvatarName1[index01], tAvatarName1) == 0) && (strcmp(
			mKillOtherTribeAvatarName2[index01], tAvatarName2) == 0))
		{
...
int MyGame::ChangeAvatarNameForZone(int tPlayUserIndex, char* tChangeAvatarName)
{
	int tResult;
	if ((tPlayUserIndex < 0) || (tPlayUserIndex > (MAX_PLAY_USER_NUM - 1)) || (!mCheckValidState[tPlayUserIndex]) || (
			strcmp(mAvatarInfo[tPlayUserIndex].aName, "") == 0) || (strcmp(mAvatarInfo[tPlayUserIndex].aGuildName, "")
			!= 0)
		|| (strcmp(tChangeAvatarName, "") == 0) || (strcmp(mAvatarInfo[tPlayUserIndex].aName, tChangeAvatarName) == 0))
	{
