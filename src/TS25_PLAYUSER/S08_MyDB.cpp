...
BOOL MyDB::Init(void)
{
	...
	if (mDB02.set_db_conn_info(mSERVER_INFO.mDB_2_IP, mSERVER_INFO.mDB_2_IP, mSERVER_INFO.mDB_2_PORT
	                           , "ts25_gamedb", mSERVER_INFO.mDB_2_ID, mSERVER_INFO.mDB_2_PASSWORD) < 0)
	{
		return false;
	}
	if (!mDB02.connect())
...
//MAKE_QUERY_FOR_SAVE
void MyDB::MakeQueryForSave(int tSort, char tID[MAX_USER_ID_LENGTH], AVATAR_INFO* tAvatarInfo)
{
	int index01;
	int index02;
	...
	case 1:
		...
		{
			sprintf(tempString01, "%05d%03d%09d%08d", (tAvatarInfo->uSaveItem[index01][0] % 100000),
			        (tAvatarInfo->uSaveItem[index01][1] % 1000), (tAvatarInfo->uSaveItem[index01][2] % 1000000000),
			        (tAvatarInfo->uSaveItem[index01][3] % 100000000));
			strcat(uSaveItem, tempString01);
		}
		sprintf(mQUERY_FOR_THREAD,
		        "update %s set uSaveMoney=%d,uSaveMoney2=%d,uSaveItem='%s' "
		        "where uID='%s' AND aWorldNumber = '%d'; "
		        , mSERVER_INFO.mDB_1_Table07, tAvatarInfo->uSaveMoney, tAvatarInfo->uSaveMoney2
	...
	case 2:
		...
		for (index01 = 0; index01 < 2; index01++)
		{
			...
			{
				sprintf(tempString01, "%05d%01d%01d%03d%09d%08d",
				        (tAvatarInfo->aInventory[index01][index02][0] % 100000),
	...
	// aInventorySocket
		...
		sprintf(
			...
			",aGeneralExperience1=%d,aGeneralExperience2=%d,aVit=%d,aStr=%d,aInt=%d,aDex=%d,aEatLifePotion=%d"
			...
			",aDoubleKillNumTime=%d,aDoubleKillExpTime=%d,aNineTowerKeyNum=%d,aZone101TypeTime=%d"
			",aZone125TypeTime=%d,aZone126TypeTime=%d,aKillMonsterNum=%d,aSearchAndBuyDate=%d"
			...
			",aCostume01=%d,aCostume02=%d,aCostume03=%d,aCostume04=%d,aCostume05=%d,aCostume06=%d,aCostume07=%d,aCostume08=%d,aCostume09=%d,aCostume10=%d"
			",aCostumeIndex=%d,aHalo=%d,aRebirth=%d,aTitle=%d,aMoney2=%d,aStoreMoney2=%d,aTradeMoney2=%d,aEatElementAtkDefPotion=%d"
			",aExpPill=%d,aDamageBoostScroll=%d,aHealthBoostScroll=%d,aCriticalBoostScroll=%d,aMountAbsorbTime=%d,aEatStrengthPotion=%d"
			",aEatAgilityPotion=%d,aWingProtectionScroll=%d,aPreventRefineBreak=%d,aIOBTime=%d,aIOS1Time=%d,aIOS2Time=%d,aIOS3Time=%d,aIOS4Time=%d"
			",aIOS5Time=%d,aIOS6Time=%d,iUseOrnament=%d,iGoldOrnamentTime=%d,iSilverOrnamentTime=%d,aLifeOrDeath=%d,aRankPoints=%d,aRankAbility=%d"
			",aAutoBuffDate=%d,aAutoHuntDate=%d,aAutoHuntHour=%d,aDoubleBuffTime=%d,aDailyWars=%d,aDailyPVPKill=%d,aDailyPVMKill=%d,aDailyOnTime=%d"
			",aCPProtCharm=%d,aAutoHunt='%s',aAutoBuffData='%s',aPetInventory='%s',aMountExpActivity='%s',aMountAbilities='%s',aExpandPetInventory=%d"
			",aFactionNoticeScroll=%d,aFactionReturnScroll=%d,aTradeSocket='%s',aEquipSocket='%s',aStoreSocket='%s',aSaveSocket='%s',aInventorySocket='%s'"
			",aTribeVoteDate=%d,aPetExpBoostScroll=%d,aLuckyUpgrade=%d,aLuckyCombine=%d,aLuckyDrop=%d,aDoubleIOBKey=%d"
...
//DB_PROCESS_03
BOOL MyDB::DB_PROCESS_03(int tSort, char tID[MAX_USER_ID_LENGTH], AVATAR_INFO* tAvatarInfo, AUTH_INFO* tAuthInfo)
{
	int iAffected;
	int iCounts;
	long tAvatarNameNum;
	...
	MakeQueryForSave(1, &tID[0], tAvatarInfo);
	if (!RECONNECT_TO_DATABASE01_FOR_THREAD())
	{
		MakeQueryForSave(1, &tID[0], tAvatarInfo);
		WRITE_FOR_DB_LOG_FOR_QUERY_FOR_THREAD();
		MakeQueryForSave(2, &tID[0], tAvatarInfo);
		WRITE_FOR_DB_LOG_FOR_QUERY_FOR_THREAD();
		MakeQueryForSave(3, &tID[0], tAvatarInfo);
		WRITE_FOR_DB_LOG_FOR_QUERY_FOR_THREAD();
		return FALSE;
	}
	iAffected = 0;
	...
	FILE* fp;
	fp = fopen("1.txt", "w");
	fprintf(fp, "%s", mQUERY_FOR_THREAD);
	fclose(fp);
	if (!RECONNECT_TO_DATABASE01_FOR_THREAD())
	{
		MakeQueryForSave(1, &tID[0], tAvatarInfo);
	...
	{
		sprintf(mQUERY_FOR_THREAD,
		        "insert into %s values ('%s',%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d);"
		        , mSERVER_INFO.mDB_2_Table02, tID,
		        tAuthInfo->AuthType, tAuthInfo->FindFlag, tAuthInfo->MoveFlag, tAuthInfo->MoveZoneFlag,
		        tAuthInfo->CallFlag,
		        tAuthInfo->HideFlag, tAuthInfo->ShowFlag, tAuthInfo->KickFlag, tAuthInfo->BlockFlag,
		        tAuthInfo->MonCallFlag, tAuthInfo->EquipFlag,
		        tAuthInfo->UnequipFlag, tAuthInfo->YchatFlag, tAuthInfo->NchatFlag, tAuthInfo->NoticeFlag,
		        tAuthInfo->ItemFlag, tAuthInfo->MoneyFlag,
		        tAuthInfo->ExpFlag, tAuthInfo->DieFlag, tAuthInfo->GodFlag, tAuthInfo->ChatFlag, tAuthInfo->TradeFlag,
		        tAuthInfo->ShopFlag,
		        tAuthInfo->PShopFlag, tAuthInfo->GItemFlag, tAuthInfo->DItemFlag, tAuthInfo->AttackFlag,
		        tAuthInfo->UZonemoveFlag, tAuthInfo->Explock);
	}
	else
	{
		sprintf(mQUERY_FOR_THREAD,
		        ...
		        ",TradeFlag=%d,ShopFlag=%d,PShopFlag=%d,GItemFlag=%d,DItemFlag=%d,AttackFlag=%d,UZonemoveFlag=%d,Explock=%d"
		        " where uID='%s';"
		        , mSERVER_INFO.mDB_2_Table02, tAuthInfo->AuthType, tAuthInfo->FindFlag, tAuthInfo->MoveFlag,
		        tAuthInfo->MoveZoneFlag, tAuthInfo->CallFlag,
		        tAuthInfo->HideFlag, tAuthInfo->ShowFlag, tAuthInfo->KickFlag, tAuthInfo->BlockFlag,
		        tAuthInfo->MonCallFlag, tAuthInfo->EquipFlag,
		        tAuthInfo->UnequipFlag, tAuthInfo->YchatFlag, tAuthInfo->NchatFlag, tAuthInfo->NoticeFlag,
		        tAuthInfo->ItemFlag, tAuthInfo->MoneyFlag,
...
//DB_PROCESS_04
void MyDB::DB_PROCESS_04(char tID[MAX_USER_ID_LENGTH], char tHackName[MAX_HACK_NAME_LENGTH], int tHackSize,
                         int* tResult)
{
	MYSQL_ROW row;
	MYSQL_RES* res;
...
#endif
	if ((hSize > 0) && (hSize != static_cast<long>(tHackSize)))
	{
		*tResult = 0;
		return;
	}
...
//DB_PROCESS_08
BOOL MyDB::DB_PROCESS_08(char tID[MAX_USER_ID_LENGTH], char tAvatarName[MAX_AVATAR_NAME_LENGTH], int tWorldNumber,
                         int tZoneNumber)
{
	int iAffected;

	sprintf(mQUERY, "insert into %s values ( now(),1,'%s','%s',%d,%d )", mSERVER_INFO.mDB_1_Table10, tID, tAvatarName,
	        tWorldNumber, tZoneNumber);
	if (!RECONNECT_TO_DATABASE01())
	{
		return FALSE;
	}
...
bool MyDB::DB_UPDATE_AUTH_KEY(char tID[MAX_USER_ID_LENGTH], char szAuthKey[3][MAX_MOUSE_PASSWORD_LENGTH], BYTE& bResult)
{
	...
	{
		sprintf(mQUERY, "UPDATE %s SET uMousePassword = '%s' WHERE uID = '%s'", mSERVER_INFO.mDB_2_Table01,
		        szAuthKey[1], tID);
		RECONNECT_TO_DATABASE02();
		iAffected = 0;
...
BOOL MyDB::DB_PROCESS_10(TRIBE_INFO* tTribeInfo)
{
	...
	sprintf(mQUERY,
	        ...
	        mSERVER_INFO.mDB_1_Table08, mSERVER_INFO.mWorldNumber);
	if (!RECONNECT_TO_DATABASE01())
	{
...
//DB_PROCESS_11
void MyDB::DB_PROCESS_11(char tID[MAX_USER_ID_LENGTH], char tAvatarName[MAX_AVATAR_NAME_LENGTH],
                         char tChangeAvatarName[MAX_AVATAR_NAME_LENGTH], int* tResult)
{
	...
	sprintf(mQUERY, "update %s set aName='%s' where aName='%s'", mSERVER_INFO.mDB_2_Table03, tChangeAvatarName,
	        tAvatarName);
	if (!RECONNECT_TO_DATABASE02())
	{
		*tResult = 2;
	...
	sprintf(mQUERY, "update %s set aName='%s' where aName='%s' and aWorldNumber = %d", mSERVER_INFO.mDB_1_Table09,
	        tChangeAvatarName, tAvatarName, mSERVER_INFO.mWorldNumber);
	if (!RECONNECT_TO_DATABASE01())
	{
		sprintf(mQUERY, "update %s set aName='%s' where aName='%s'", mSERVER_INFO.mDB_2_Table03, tAvatarName,
		        tChangeAvatarName);
		RECONNECT_TO_DATABASE02();
		iAffected = 0;
		if (mDB02.exec_query(mQUERY, CMySQL::CLEAR) != 0)
	...
	{
	case 0:
		sprintf(mQUERY, "update %s set aName01='%s' where uID='%s' and aWorldNumber = %d", mSERVER_INFO.mDB_1_Table07,
		        tChangeAvatarName, tID, mSERVER_INFO.mWorldNumber);
		break;
	case 1:
		sprintf(mQUERY, "update %s set aName02='%s' where uID='%s' and aWorldNumber = %d", mSERVER_INFO.mDB_1_Table07,
		        tChangeAvatarName, tID, mSERVER_INFO.mWorldNumber);
		break;
	case 2:
		sprintf(mQUERY, "update %s set aName03='%s' where uID='%s' and aWorldNumber = %d", mSERVER_INFO.mDB_1_Table07,
		        tChangeAvatarName, tID, mSERVER_INFO.mWorldNumber);
		break;
	default:
		sprintf(mQUERY, "update %s set aName='%s' where aName='%s' and aWorldNumber = %d", mSERVER_INFO.mDB_1_Table09,
...
void MyDB::DB_PROCESS_12(char* tID, char* tAvatarName, int* tResult)
{
	...
	*tResult = -1;
	...
	sprintf(mQUERY, "select aName01, aName02, aName03 from %s where uID ='%s' AND aWorldNumber = '%d';",
	        mSERVER_INFO.mDB_1_Table07, tID, mSERVER_INFO.mWorldNumber);
	if (!RECONNECT_TO_DATABASE01())
	{
		return;
	}
...
void MyDB::DB_PROCESS_13(char* tID, int* tResult)
{
	...
	*tResult = 0;
	...
	sprintf(mQUERY, "select uBonus100Money from %s where uID ='%s' AND aWorldNumber = '%d';",
	        mSERVER_INFO.mDB_2_Table01, tID, mSERVER_INFO.mWorldNumber);
	if (!RECONNECT_TO_DATABASE02())
	{
		*tResult = 1;
		return;
...
void MyDB::DB_PROCESS_15(char* tID, char* tName, int tTribe, int tKillNum)
{
	...
	sprintf(mQUERY, "select count(*) from %s where aWorldNumber = %d and aName = '%s';", mSERVER_INFO.mDB_1_Table05,
	        mSERVER_INFO.mWorldNumber, tName);
	if (!RECONNECT_TO_DATABASE01())
	{
		return;
	}
	...
	{
		sprintf(mQUERY,
		        "update %s set aKillOtherTribe = aKillOtherTribe + %d where aWorldNumber = %d and aName = '%s';",
