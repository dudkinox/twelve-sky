...
void W_LOGIN_SEND(int tUserIndex)
{
	...
	mDB.DB_PROCESS_02(&mUSER[tUserIndex].mIP[0], &tID[0], &tPassword[0], &mUSER[tUserIndex].uID[0],
	                  ...
	                  &first_login); // &mUSER[tUserIndex].uPremiumServerExpirationDate 추가. // @_Premium_Server_@
	if (tResult != 0)
	{
	...
	// mUSER[tUserIndex].uPremiumServerExpirationDate 추가. // @_Premium_Server_@
...
void W_CHANGE_AVATAR_NAME_SEND(int tUserIndex)
{
	...
	// 서버 통합에 의한 무명 변환의 경우 == 아바타 무명에 특수문자가 들어 가 있는 경우 == tPage, tIndex 가 -1인 경우.
	{
