...
BOOL MyGame::Init(void)
{
	...
	if( !mGMEnableLoginIPList.LoadIPInfo("DATA/GM_ENABLE_LOGIN_IP_INFO.TXT") )
	{
		return FALSE;
	}*/
...
void MyGame::SafeAvatarInfoData(AVATAR_INFO* tAvatarInfo)
{
	...
	{
		tAvatarInfo->aPartyName[index01][(MAX_PARTY_NAME_LENGTH - 1)] = '\0';
	}
}
...
BOOL MyGame::LoadBlockIPInfo(void)
{
	...
	fp = fopen("DATA/BLOCK_IP_INFO.TXT", "r");
	if (fp == nullptr)
	{
		__LogMgr()->Log("![%s] : fopen failed (%s) - BLOCK_IP_INFO.TXT\n",
	...
	for (index01 = 0; index01 < mTotalBlockIPInfo; index01++)
	{
		...
		index02 = 0;
		index03 = 0;
		for (; index02 < static_cast<int>(strlen(tempString01)); index02++)
		{
		...
		if (index02 == static_cast<int>(strlen(tempString01)))
		{
			fclose(fp);
			return FALSE;
		}
...
BOOL MyGame::CompareBlockIPInfo(int tIP1, int tIP2, int tIP3, int tIP4)
{
	...
	for (index01 = 0; index01 < mTotalBlockIPInfo; index01++)
	{
		sprintf(tempString01, "%03d%03d%03d%03d", mBlockIPInfo[index01][0][0], mBlockIPInfo[index01][0][1],
		        mBlockIPInfo[index01][0][2], mBlockIPInfo[index01][0][3]);
		tBlockIP01 = atol(tempString01);
		sprintf(tempString01, "%03d%03d%03d%03d", mBlockIPInfo[index01][1][0], mBlockIPInfo[index01][1][1],
		        mBlockIPInfo[index01][1][2], mBlockIPInfo[index01][1][3]);
...
BOOL CIPInfoList::CompareIPInfo(int tIP1, int tIP2, int tIP3, int tIP4)
{
	...
	for (index01 = 0; index01 < mTotalIPInfoNum; index01++)
	{
		sprintf(tempString01, "%03d%03d%03d%03d", mIPInfoList[index01][0][0], mIPInfoList[index01][0][1],
		        mIPInfoList[index01][0][2], mIPInfoList[index01][0][3]);
		tBlockIP01 = atol(tempString01);
		sprintf(tempString01, "%03d%03d%03d%03d", mIPInfoList[index01][1][0], mIPInfoList[index01][1][1],
		        mIPInfoList[index01][1][2], mIPInfoList[index01][1][3]);
		tBlockIP02 = atol(tempString01);
		if ((tBlockIP01 <= tIP) && (tBlockIP02 >= tIP))
		{
			return TRUE;
