...
{
	initialized = false;
	m_hostname_str_ptr[0] = nullptr;
	m_hostname_str_ptr[1] = nullptr;
	m_dbname_str_ptr = nullptr;
	m_acct_str_ptr = nullptr;
	m_passwd_str_ptr = nullptr;
	m_result_code = 0;
	m_MYSQL = nullptr;
...
int CMySQL::set_db_conn_info(const char* hostname_str_const_ptr, const char* hostname2_str_const_ptr, int port_num,
...
MYSQL* CMySQL::connect(void)
{
	...
	for (int i = 0; i < 2; i++)
	{
		...
		m_MYSQL = mysql_real_connect(&mMYSQL, m_hostname_str_ptr[i], m_acct_str_ptr, m_passwd_str_ptr,
		                             m_dbname_str_ptr, m_port_num, nullptr, 0);
		if (!m_MYSQL)
		{
			if (i == 1)
				return nullptr;
