...
#if 0
void MyTransfer::B_WORLD_INFO2( WORLD_INFO *tWorldInfo, TRIBE_INFO *tTribeInfo )
{
	mOriginal2[0] = P_WORLD_INFO;
	CopyMemory( &mOriginal2[1], tWorldInfo, sizeof( WORLD_INFO ) );
	CopyMemory( &mOriginal2[( 1 + sizeof( WORLD_INFO ) )], tTribeInfo, sizeof( TRIBE_INFO ) );
	mOriginalSize2 = S_WORLD_INFO;
}
#endif
...
//B_TRIBE_CHAT
void MyTransfer::B_TRIBE_CHAT(int tZoneNumber, int tTribe, char tAvatarName01[13], char tAvatarName02[13],
                              char tContent[51])
{
...
//B_SECRET_CHAT
void MyTransfer::B_SECRET_CHAT(int tZoneNumber, int tTribe, char tAvatarName01[13], char tAvatarName02[13],
                               char tContent[51])
{
...
#if 0
	if(!mDB.DB_PROCESS_13(szAvatarName, &rank_info)) {
		LOG_TO_FILE_2("![%s] : Not exist ACCOUNT or GM - aName[%s]\n", __FUNCTION__, aName);
	}
#endif
	mOriginal[0] = P_RANK_INFO_RECV;
	memcpy(&mOriginal[1], &rank_info, sizeof(rank_info_t));
	memcpy(&mOriginal[1 + sizeof(rank_info_t)], aName + MAX_AVATAR_NAME_LENGTH, 4);
	mOriginalSize = S_RANK_INFO_RECV;
}

// # X-Trap # {
void MyTransfer::B_XTRAP_MAP_FILE_CHANGE_FOR_ZONE(void)
{
	mOriginal[0] = P_XTRAP_MAP_FILE_CHANGE_FOR_ZONE;
	mOriginalSize = S_XTRAP_MAP_FILE_CHANGE_FOR_ZONE;
} // # }
// @ Cost Info Download @
