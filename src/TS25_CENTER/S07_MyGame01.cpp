...
BOOL MyGame::Init(void)
{
	...
	//memset(&mWorldInfo, 0, SIZE_OF_WORLD_INFO);
	//memset(&mTribeInfo, 0, SIZE_OF_TRIBE_INFO);
	mTickCount = 0;
	...
	//mWorldInfo.mRainbowIslandCaptureInfo[0][1] = 1;
	//mWorldInfo.mRainbowIslandCaptureInfo[0][2] = 1;
	//mWorldInfo.mRainbowIslandCaptureInfo[1][1] = 1;
	//mWorldInfo.mRainbowIslandCaptureInfo[1][2] = 1;
	//mWorldInfo.mRainbowIslandCaptureInfo[2][1] = 1;
	//mWorldInfo.mRainbowIslandCaptureInfo[2][2] = 1;
#ifdef __GOD__
	...
	//LOG_TO_FILE ("The mGame Init start\n");
	if (!mDB.DB_PROCESS_01(&mWorldInfo))
	{
		//__LogMgr()->Log ("test mGame DB01 Init");
		return FALSE;
	}
	...
	{
		//__LogMgr()->Log ("test mGame DB10 Init");
		return FALSE;
	}

	// Reset Login States
	if (!mDB.DB_PROCESS_15())
	{
		return FALSE;
	}
