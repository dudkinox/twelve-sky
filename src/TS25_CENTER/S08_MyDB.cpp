...
BOOL MyDB::DB_PROCESS_01(WORLD_INFO* tWorldInfo)
{
	...
	sprintf(mQUERY,
	        ...
	        mSERVER_INFO.mDB_1_Table01, g_env.world_num);
...
BOOL MyDB::DB_PROCESS_02(WORLD_INFO* tWorldInfo)
{
	...
	sprintf(mQUERY, "select mTribe1Point,mTribe2Point,mTribe3Point,mTribe4Point from %s where aWorldNumber = %d",
	        mSERVER_INFO.mDB_1_Table01, g_env.world_num);
	if (!RECONNECT_TO_DATABASE01())
	{
		return FALSE;
...
BOOL MyDB::DB_PROCESS_03(int tTribe)
{
	...
	sprintf(mQUERY, "update %s set mZone38WinTribeDate=now(),mZone38WinTribe=%d where aWorldNumber = %d",
	        mSERVER_INFO.mDB_1_Table01, tTribe, g_env.world_num);
	if (!RECONNECT_TO_DATABASE01())
	{
		return FALSE;
	}
...
BOOL MyDB::DB_PROCESS_04(int tSort, int tTribe)
{
	...
	{
	case 1:
		sprintf(mQUERY, "update %s set mTribe1SymbolDate=now(),mTribe1Symbol=%d where aWorldNumber = %d",
		        mSERVER_INFO.mDB_1_Table01, tTribe, g_env.world_num);
		break;
	case 2:
		sprintf(mQUERY, "update %s set mTribe2SymbolDate=now(),mTribe2Symbol=%d where aWorldNumber = %d",
		        mSERVER_INFO.mDB_1_Table01, tTribe, g_env.world_num);
		break;
	case 3:
		sprintf(mQUERY, "update %s set mTribe3SymbolDate=now(),mTribe3Symbol=%d where aWorldNumber = %d",
		        mSERVER_INFO.mDB_1_Table01, tTribe, g_env.world_num);
		break;
	case 4:
		sprintf(mQUERY, "update %s set mTribe4SymbolDate=now(),mTribe4Symbol=%d where aWorldNumber = %d",
		        mSERVER_INFO.mDB_1_Table01, tTribe, g_env.world_num);
		break;
	case 5:
		sprintf(mQUERY, "update %s set mMonsterSymbolDate=now(),mMonsterSymbol=%d where aWorldNumber = %d",
		        mSERVER_INFO.mDB_1_Table01, tTribe, g_env.world_num);
		break;
	default:
		return FALSE;
...
BOOL MyDB::DB_PROCESS_05(int tDate, int tTribe)
{
	...
	sprintf(mQUERY, "update %s set mCloseTime=%d,mCloseTribe=%d where aWorldNumber = %d", mSERVER_INFO.mDB_1_Table01,
	        tDate, tTribe, g_env.world_num);
	if (!RECONNECT_TO_DATABASE01())
	{
		return FALSE;
	}
...
BOOL MyDB::DB_PROCESS_06(int tSort, int tDate, int tValue)
{
	...
	{
	case 1:
		sprintf(mQUERY, "update %s set mPossibleAlliance01Date=%d,mPossibleAlliance01Info=%d where aWorldNumber = %d",
		        mSERVER_INFO.mDB_1_Table01, tDate, tValue, g_env.world_num);
		break;
	case 2:
		sprintf(mQUERY, "update %s set mPossibleAlliance02Date=%d,mPossibleAlliance02Info=%d where aWorldNumber = %d",
		        mSERVER_INFO.mDB_1_Table01, tDate, tValue, g_env.world_num);
		break;
	case 3:
		sprintf(mQUERY, "update %s set mPossibleAlliance03Date=%d,mPossibleAlliance03Info=%d where aWorldNumber = %d",
		        mSERVER_INFO.mDB_1_Table01, tDate, tValue, g_env.world_num);
		break;
	case 4:
		sprintf(mQUERY, "update %s set mPossibleAlliance04Date=%d,mPossibleAlliance04Info=%d where aWorldNumber = %d",
		        mSERVER_INFO.mDB_1_Table01, tDate, tValue, g_env.world_num);
		break;
	default:
		return FALSE;
...
BOOL MyDB::DB_PROCESS_07(int tSort, int tAlliance01, int tAlliance02)
{
	...
	{
	case 1:
		sprintf(mQUERY, "update %s set mAlliance01Date=now(),mAlliance0101=%d,mAlliance0102=%d where aWorldNumber = %d",
		        mSERVER_INFO.mDB_1_Table01, tAlliance01, tAlliance02, g_env.world_num);
		break;
	case 2:
		sprintf(mQUERY, "update %s set mAlliance02Date=now(),mAlliance0201=%d,mAlliance0202=%d where aWorldNumber = %d",
		        mSERVER_INFO.mDB_1_Table01, tAlliance01, tAlliance02, g_env.world_num);
		break;
	default:
		return FALSE;
...
BOOL MyDB::DB_PROCESS_08(int tSort, int tValue)
{
	...
	{
	case 1:
		sprintf(mQUERY, "update %s set mTribe1VoteDate=now(),mTribe1VoteState=%d where aWorldNumber = %d",
		        mSERVER_INFO.mDB_1_Table01, tValue, g_env.world_num);
		break;
	case 2:
		sprintf(mQUERY, "update %s set mTribe2VoteDate=now(),mTribe2VoteState=%d where aWorldNumber = %d",
		        mSERVER_INFO.mDB_1_Table01, tValue, g_env.world_num);
		break;
	case 3:
		sprintf(mQUERY, "update %s set mTribe3VoteDate=now(),mTribe3VoteState=%d where aWorldNumber = %d",
		        mSERVER_INFO.mDB_1_Table01, tValue, g_env.world_num);
		break;
	case 4:
		sprintf(mQUERY, "update %s set mTribe4VoteDate=now(),mTribe4VoteState=%d where aWorldNumber = %d",
		        mSERVER_INFO.mDB_1_Table01, tValue, g_env.world_num);
		break;
	default:
		return FALSE;
...
BOOL MyDB::DB_PROCESS_09(int tSort, int tValue)
{
	...
	{
	case 1:
		sprintf(mQUERY, "update %s set mClose1VoteDate=now(),mClose1VoteState=%d where aWorldNumber = %d",
		        mSERVER_INFO.mDB_1_Table01, tValue, g_env.world_num);
		break;
	case 2:
		sprintf(mQUERY, "update %s set mClose2VoteDate=now(),mClose2VoteState=%d where aWorldNumber = %d",
		        mSERVER_INFO.mDB_1_Table01, tValue, g_env.world_num);
		break;
	case 3:
		sprintf(mQUERY, "update %s set mClose3VoteDate=now(),mClose3VoteState=%d where aWorldNumber = %d",
		        mSERVER_INFO.mDB_1_Table01, tValue, g_env.world_num);
		break;
	case 4:
		sprintf(mQUERY, "update %s set mClose4VoteDate=now(),mClose4VoteState=%d where aWorldNumber = %d",
		        mSERVER_INFO.mDB_1_Table01, tValue, g_env.world_num);
		break;
	default:
		return FALSE;
...
BOOL MyDB::DB_PROCESS_10(TRIBE_INFO* tTribeInfo)
{
	...
	sprintf(mQUERY,
	        ...
	        mSERVER_INFO.mDB_1_Table02, g_env.world_num);
	if (!RECONNECT_TO_DATABASE01())
	{
		return FALSE;
	}
...
BOOL MyDB::DB_PROCESS_11(TRIBE_INFO* tTribeInfo)
{
	...
	for (index01 = 0; index01 < 4; index01++)
	{
		...
		{
			strcpy(tempString01, "@@@@@@@@@@@@");
			CopyMemory(&tempString01[0], &tTribeInfo->mTribeVoteName[index01][index02][0],
			           strlen( tTribeInfo->mTribeVoteName[index01][index02] ));
			tempString01[12] = '\0';
			strcat(mTribeVoteInfo, tempString01);
			sprintf(tempString01, "%02d%06d%06d", (tTribeInfo->mTribeVoteLevel[index01][index02] % 100),
			        (tTribeInfo->mTribeVoteKillOtherTribe[index01][index02] % 1000000),
			        (tTribeInfo->mTribeVotePoint[index01][index02] % 1000000));
			strcat(mTribeVoteInfo, tempString01);
		}
	}
	strcpy(mTribe01SubMasterInfo, "");
	...
	{
		strcpy(tempString01, "@@@@@@@@@@@@");
		CopyMemory(&tempString01[0], &tTribeInfo->mTribeSubMaster[0][index01][0],
		           strlen( tTribeInfo->mTribeSubMaster[0][index01] ));
		tempString01[12] = '\0';
	...
	{
		strcpy(tempString01, "@@@@@@@@@@@@");
		CopyMemory(&tempString01[0], &tTribeInfo->mTribeSubMaster[1][index01][0],
		           strlen( tTribeInfo->mTribeSubMaster[1][index01] ));
		tempString01[12] = '\0';
	...
	{
		strcpy(tempString01, "@@@@@@@@@@@@");
		CopyMemory(&tempString01[0], &tTribeInfo->mTribeSubMaster[2][index01][0],
		           strlen( tTribeInfo->mTribeSubMaster[2][index01] ));
		tempString01[12] = '\0';
	...
	{
		strcpy(tempString01, "@@@@@@@@@@@@");
		CopyMemory(&tempString01[0], &tTribeInfo->mTribeSubMaster[3][index01][0],
		           strlen( tTribeInfo->mTribeSubMaster[3][index01] ));
		tempString01[12] = '\0';
		strcat(mTribe04SubMasterInfo, tempString01);
	}
	sprintf(mQUERY,
	        "update %s set mUpdateTime=now(),mTribeVoteInfo='%s',mTribe01Master='%s',mTribe01SubMasterInfo='%s',mTribe02Master='%s',mTribe02SubMasterInfo='%s',mTribe03Master='%s',mTribe03SubMasterInfo='%s',mTribe04Master='%s',mTribe04SubMasterInfo='%s' where aWorldNumber = %d",
...
BOOL MyDB::DB_PROCESS_12(int up_check)
{
	     ...
	     level_it != (mDB.list_rank_level).end(); ++level_it)
	{
		...
		memcpy(&mGAME.mBuffer[12] + nSize, &*level_it, sizeof(rank_level_t));
		i++;
	}
	     ...
	     kill_it != (mDB.list_rank_kill).end(); ++kill_it)
	{
		...
		memcpy(&mGAME.mBuffer[nSize] + nSize2, &*kill_it, sizeof(rank_kill_t));
		i++;
	}
...
BOOL MyDB::DB_PROCESS_13(const char* aName, rank_info_t* rank_info)
{
	...
	while((row = mysql_fetch_row(res))) {
		_snprintf(rank_info->aName, MAX_AVATAR_NAME_LENGTH, "%s", row[0]);
...
BOOL MyDB::DB_PROCESS_14()
{
	sprintf(mQUERY, "update avatarinfo SET aRankPoints=0, aRankAbility=0 WHERE aWorldNumber=%d ", g_env.world_num);
...
BOOL MyDB::DB_PROCESS_15()
{
	sprintf(mQUERY, "update memberinfo SET uLoginState=0 ");
