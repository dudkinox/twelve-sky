...
void W_GET_ZONE_SERVER_INFO_1(int tUserIndex)
{
	...
	{
		mTRANSFER.B_GET_ZONE_SERVER_INFO_1_RESULT(1, "0.0.0.0", 0);
		mUSER[tUserIndex].Send(TRUE, (char*)&mTRANSFER.mOriginal[0], mTRANSFER.mOriginalSize);
		return;
	}
...
void W_ZONE_FOR_CENTER_SEND(int tUserIndex)
{
	...
	mTRANSFER.B_ZONE_CONNECTION_INFO(tZoneNumber, &mGAME.mZoneConnectionInfo.mIP[(tZoneNumber - 1)][0],
...
void W_DEMAND_BROADCAST_INFO(int tUserIndex)
{
	...
	/** 2009. 11. 02 : 성석전 리뉴얼 (wolf) */
		...
		mDB.DB_PROCESS_04(1, 0);
		mDB.DB_PROCESS_04(2, 1);
		mDB.DB_PROCESS_04(3, 2);
		mDB.DB_PROCESS_04(4, 3);
		mDB.DB_PROCESS_14();
	/** 2009. 11. 02 : 성석전 리뉴얼 (wolf) */
		break;
	case 41: break;
	...
	case 47:
		...
		if (((mGAME.mWorldInfo.mAllianceState[0][0] == tAllianceTribe1) && (mGAME.mWorldInfo.mAllianceState[0][1] ==
			tAllianceTribe2)) || ((mGAME.mWorldInfo.mAllianceState[0][1] == tAllianceTribe1) && (mGAME.mWorldInfo.
			mAllianceState[0][0] == tAllianceTribe2)))
		{
			tAllianceIndex = 0;
		}
		else
	...
	case 55:
		...
		for (index01 = 0; index01 < 4; index01++)
		{
			...
			for (index02 = 0; index02 < MAX_TRIBE_VOTE_AVATAR_NUM; index02++)
			{
				...
				{
					if (mGAME.mTribeInfo.mTribeVotePoint[index01][index02] > mGAME.mTribeInfo.mTribeVotePoint[index01][
						index03])
					{
						index03 = index02;
					}
	...
	case 301:
		...
		{
		case 21:
			mGAME.mWorldInfo.mTribeGeneralExperienceUpRatioInfo[0] = static_cast<float>(tEventValue) * 0.1f;
			break;
		case 22:
			mGAME.mWorldInfo.mTribeGeneralExperienceUpRatioInfo[1] = static_cast<float>(tEventValue) * 0.1f;
			break;
		case 23:
			mGAME.mWorldInfo.mTribeGeneralExperienceUpRatioInfo[2] = static_cast<float>(tEventValue) * 0.1f;
			break;
		case 24:
			mGAME.mWorldInfo.mTribeGeneralExperienceUpRatioInfo[3] = static_cast<float>(tEventValue) * 0.1f;
			break;
		case 31:
			mGAME.mWorldInfo.mTribeItemDropUpRatioInfo[0] = static_cast<float>(tEventValue) * 0.1f;
			break;
		case 32:
			mGAME.mWorldInfo.mTribeItemDropUpRatioInfo[1] = static_cast<float>(tEventValue) * 0.1f;
			break;
		case 33:
			mGAME.mWorldInfo.mTribeItemDropUpRatioInfo[2] = static_cast<float>(tEventValue) * 0.1f;
			break;
		case 34:
			mGAME.mWorldInfo.mTribeItemDropUpRatioInfo[3] = static_cast<float>(tEventValue) * 0.1f;
			break;
		case 41:
			mGAME.mWorldInfo.mTribeItemDropUpRatioForMyoungInfo[0] = static_cast<float>(tEventValue) * 0.1f;
			break;
		case 42:
			mGAME.mWorldInfo.mTribeItemDropUpRatioForMyoungInfo[1] = static_cast<float>(tEventValue) * 0.1f;
			break;
		case 43:
			mGAME.mWorldInfo.mTribeItemDropUpRatioForMyoungInfo[2] = static_cast<float>(tEventValue) * 0.1f;
			break;
		case 44:
			mGAME.mWorldInfo.mTribeItemDropUpRatioForMyoungInfo[3] = static_cast<float>(tEventValue) * 0.1f;
			break;
		case 51:
	...
	case 774:
		CopyMemory(&tTribe, &tInfoData[0], 4);
		CopyMemory(&tZoneNumber, &tInfoData[4], 4);
	...
	//mGAME.mWorldInfo.mProvingGroundsPoint[tTribe] += 1;
	//mDB.DB_PROCESS_14(tTribe + 1);
		break;
	}
...
void W_CHECK_EVENT_SEND(int tUserIndex)
{
	...
	mTRANSFER.B_TOOL_CHECK_EVENT_RESULT(tGmIndex, tEventSort, tResult, mSERVER_INFO.mWorldNumber,
	                                    mUSER[tUserIndex].mZoneServerNumber);
	mUSER[tSendUserIndex].Send(TRUE, (char*)&mTRANSFER.mOriginal[0], mTRANSFER.mOriginalSize);
}
...
void W_GET_RANK_INFO_SEND(int tUserIndex)
{
	...
	if (mUSER[tUserIndex].mCheckServerType != 2
		&& static_cast<int>(tPacket[0]) != P_GET_RANK_INFO_SEND)
	{
...
{
#ifdef __USE_X_TRAP__
    //LOG_TO_FILE_1 ("%s packet recv\n", __FUNCTION__);
    if( mUSER[tUserIndex].mCheckServerType != 3 ) {
        mUSER[tUserIndex].Quit( 5013);
        return;
    }
    mUSER[tUserIndex].mUsedTime = GetTickCount();

    mTRANSFER.B_XTRAP_MAP_FILE_CHANGE_FOR_ZONE();
    for( int index01 = 0 ; index01 < MAX_USER_NUM ; index01++ )
    {
        if( mUSER[index01].mCheckServerType == 2 )
        {
