...
void WORLD_FOR_GXD::Free(void)
{
	...
	if (mQuadtree != nullptr)
	{
		...
		{
			if (mQuadtree[index01].mTrisIndex != nullptr)
			{
				free(mQuadtree[index01].mTrisIndex);
				mQuadtree[index01].mTrisIndex = nullptr;
			}
		}
		free(mQuadtree);
		mQuadtree = nullptr;
...
BOOL WORLD_FOR_GXD::LoadWM(void)
{
	...
	if (!ReadFile(hFile, &tCompress[0], tCompressSize, &nReadByte, nullptr))
	{
	...
	tOriginalIndex += 4;
	mWorldTris = static_cast<WORLDTRIS_FOR_GXD*>(GlobalAlloc(GMEM_FIXED, (mWorldTrisNum * sizeof(WORLDTRIS_FOR_GXD))));
	if (mWorldTris == nullptr)
	{
		GlobalFree(tOriginal);
		CloseHandle(hFile);
	...
	tOriginalIndex += 4;
	mQuadtree = static_cast<QUADTREENODE_FOR_GXD*>(GlobalAlloc(
		GMEM_FIXED, (mTotalQuadtreeNodeNum * sizeof(QUADTREENODE_FOR_GXD))));
	if (mQuadtree == nullptr)
	{
		GlobalFree(tOriginal);
		CloseHandle(hFile);
	...
	for (index01 = 0; index01 < mTotalQuadtreeNodeNum; index01++)
	{
		...
		{
			mQuadtree[index01].mTrisIndex = static_cast<int*>(
				GlobalAlloc(GMEM_FIXED, (mQuadtree[index01].mTrisNum * 4)));
			if (mQuadtree[index01].mTrisIndex == nullptr)
			{
				GlobalFree(tOriginal);
				CloseHandle(hFile);
			...
			CopyMemory(&mQuadtree[index01].mTrisIndex[0], &tOriginal[tOriginalIndex],
			           (mQuadtree[index01].mTrisNum * 4));
...
BOOL WORLD_FOR_GXD::GetYCoordWithTris(int tTrisIndex, float x, float z, float* y)
{
	...
	*y = (mWorldTris[tTrisIndex].mPlaneInfo[3] - mWorldTris[tTrisIndex].mPlaneInfo[0] * x - mWorldTris[tTrisIndex].
		mPlaneInfo[2] * z) / mWorldTris[tTrisIndex].mPlaneInfo[1];
	return TRUE;
}
	...
	while (TRUE)
	{
		if (mQuadtree[tSelectNodeIndex].mChildNodeIndex[0] != -1)
		{
			for (index01 = 0; index01 < 4; index01++)
			{
				...
				if ((tPoint[0] >= mQuadtree[tSelectChildIndex].mBoxMin[0]) && (tPoint[0] <= mQuadtree[tSelectChildIndex]
					.mBoxMax[0]) && (tPoint[2] >= mQuadtree[tSelectChildIndex].mBoxMin[2]) && (tPoint[2] <= mQuadtree[
					tSelectChildIndex].mBoxMax[2]))
				{
...
//GET_Y_COORD
BOOL WORLD_FOR_GXD::GetYCoord(float x, float z, float* y, BOOL tCheckExistPostYCoord, float tPostYCoord,
                              BOOL tCheckTwoSide, BOOL tCheckOnlyOne)
{
	if (!mCheckValidState)
	{
		return FALSE;
	...
	while (TRUE)
	{
		if (mQuadtree[tSelectNodeIndex].mChildNodeIndex[0] != -1)
		{
			...
			{
				tSelectChildIndex = mQuadtree[tSelectNodeIndex].mChildNodeIndex[index01];
				if ((x >= mQuadtree[tSelectChildIndex].mBoxMin[0]) && (x <= mQuadtree[tSelectChildIndex].mBoxMax[0]) &&
					(z >= mQuadtree[tSelectChildIndex].mBoxMin[2]) && (z <= mQuadtree[tSelectChildIndex].mBoxMax[2]))
				{
					tSelectNodeIndex = tSelectChildIndex;
					break;
				}
...
BOOL WORLD_FOR_GXD::PathForMonsterAttack(float tSCoord[3], float tECoord[3], float tSpeed, float tPostTime,
                                         float tLimitCoord[3], float tLimitLength, float tResult[3])
{
	if ((!mCheckValidState) || (tSpeed <= 0.0f) || (tPostTime <= 0.0f))
	{
		CopyMemory(&tResult[0], &tSCoord[0], 12);
	...
	while (TRUE)
	{
		tLength = (tLimitCoord[0] - tResult[0]) * (tLimitCoord[0] - tResult[0]) + (tLimitCoord[2] - tResult[2]) * (
			tLimitCoord[2] - tResult[2]);
