...
BOOL MyGame::Init(void)
{
	...
	{
		__LogMgr()->Log("!INIT ERROR [localtime return NULL]\n");
		return FALSE;
	}
	...
	mPremiumServerExpirationCountDown = -1; // 프리미엄 서버(월드) 입장권 만료 5분 카운트 다운용 변수. // @_Premium_Server_@
...
BOOL MyGame::Decompress(DWORD tCompressSize, BYTE* tCompress, DWORD tOriginalSize, BYTE* tOriginal)
{
	...
	if ((uncompress(tOriginal, &to, tCompress, tCompressSize)) != 0)
	{
		return FALSE;
	}
	...
	if ((tPresentTime->tm_mday >= 11) && (tPresentTime->tm_mday <= 11))
	{
		...
		//--------------//
			...
			for (index01 = 0; index01 < 4; index01++)
			{
				...
				{
					strncpy(mTribeInfo.mTribeMaster[index01], mTribeInfo.mTribeVoteName[index01][index03],
...
int MyGame::ReturnAllianceTribe(int tTribe)
{
	if ((49 == mSERVER_INFO.mServerNumber) || (51 == mSERVER_INFO.mServerNumber) || (53 == mSERVER_INFO.mServerNumber)
		|| (146 == mSERVER_INFO.mServerNumber) || (147 == mSERVER_INFO.mServerNumber) || (148 == mSERVER_INFO.
			mServerNumber)
		|| (149 == mSERVER_INFO.mServerNumber) || (150 == mSERVER_INFO.mServerNumber) || (151 == mSERVER_INFO.
			mServerNumber)
...
void MyGame::Logic(float dTime)
{
		...
		if (mUSER[index01].mCheckValidState)
		{
			if ((tGetTickCount - mUSER[index01].mRegisterTime) >= 180000)
			{
				if (mUSER[index01].mMoveZoneResult == 0)
				{
					...
					mUSER[index01].mAvatarInfo.aLogoutInfo[1] = static_cast<int>(mAVATAR_OBJECT[index01].mDATA.aAction.
						aLocation[0]);
					mUSER[index01].mAvatarInfo.aLogoutInfo[2] = static_cast<int>(mAVATAR_OBJECT[index01].mDATA.aAction.
						aLocation[1]);
					mUSER[index01].mAvatarInfo.aLogoutInfo[3] = static_cast<int>(mAVATAR_OBJECT[index01].mDATA.aAction.
						aLocation[2]);
					...
					mPLAYUSER_COM.U_REGISTER_USER_FOR_ZONE_2_SEND(mUSER[index01].mPlayUserIndex, &mUSER[index01].uID[0],
					                                              &mUSER[index01].mAvatarInfo,
					                                              mUSER[index01].mEffectValue,
	...
	for (index01 = 0; index01 < mSERVER.mMAX_USER_NUM; index01++)
	{
		...
		mTRANSFER.B_AVATAR_ACTION_RECV2(index01, mAVATAR_OBJECT[index01].mUniqueNumber, &mAVATAR_OBJECT[index01].mDATA,
		                                2);
		mUTIL.Broadcast(TRUE, &mAVATAR_OBJECT[index01].mDATA.aAction.aLocation[0], UNIT_SCALE_RADIUS1, index01, true,
		                0);
	}
	...
	for (index01 = 0; index01 < MAX_MONSTER_OBJECT_NUM; index01++)
	{
		...
		mTRANSFER.B_MONSTER_ACTION_RECV(index01, mMONSTER_OBJECT[index01].mUniqueNumber,
		                                &mMONSTER_OBJECT[index01].mDATA, 2);
...
#ifdef __HERO__
	if ((mTickCount % 20) == 0) {
		if (mGAME.mHeroState[0] == 1 || mGAME.mHeroState[1] == 1) {
			mPLAYUSER_COM.U_HERO_DATA_SEND();
		}
		mGAME.HeroUpdate();
	}
#endif
...
void MyGame::ProcessForGuardState(void)
{
	...
	int tBroadcastInfoSort;
	BYTE tBroadcastInfoData[MAX_BROADCAST_INFO_SIZE];
	constexpr int tGuardTribePostNum = 5;
...
void MyGame::Process_Allience_Server(void)
{
	...
	case 0:
		...
		for (index01 = 0; index01 < mSERVER.mMAX_USER_NUM; index01++)
		{
			...
			if (mUTIL.CheckInRange(&mAVATAR_OBJECT[index01].mDATA.aAction.aLocation[0],
			                       &mAllienceBattlePostLocation[0][0], mAllienceBattlePostRadius[0]))
			{
		...
		for (index01 = 0; index01 < mSERVER.mMAX_USER_NUM; index01++)
		{
			...
			if (mUTIL.CheckInRange(&mAVATAR_OBJECT[index01].mDATA.aAction.aLocation[0],
			                       &mAllienceBattlePostLocation[1][0], mAllienceBattlePostRadius[1]))
			{
		...
		if (mAlliencePostAvatarTribe[0] != ReturnAllianceTribe(mAlliencePostAvatarTribe[1]))
		{
			if (
				...
				((mWorldInfo.mPossibleAllianceInfo[mAlliencePostAvatarTribe[0]][1] == 1) && (mUTIL.ReturnNowDate() <=
					mWorldInfo.mPossibleAllianceInfo[mAlliencePostAvatarTribe[0]][0])) ||
				((mWorldInfo.mPossibleAllianceInfo[mAlliencePostAvatarTribe[1]][1] == 1) && (mUTIL.ReturnNowDate() <=
					mWorldInfo.mPossibleAllianceInfo[mAlliencePostAvatarTribe[1]][0]))
			)
			{
	...
	case 2:
		...
		if (
			...
			(!mUTIL.CheckInRange(&mAVATAR_OBJECT[mAlliencePostAvatarIndex[0]].mDATA.aAction.aLocation[0],
			                     &mAllienceBattlePostLocation[0][0], mAllienceBattlePostRadius[0])) ||
			...
			(!mUTIL.CheckInRange(&mAVATAR_OBJECT[mAlliencePostAvatarIndex[1]].mDATA.aAction.aLocation[0],
			                     &mAllienceBattlePostLocation[1][0], mAllienceBattlePostRadius[1])) ||
	...
	case 3:
		...
		if (((mWorldInfo.mAllianceState[0][0] == mAlliencePostAvatarTribe[0]) && (mWorldInfo.mAllianceState[0][1] ==
			mAlliencePostAvatarTribe[1])) || ((mWorldInfo.mAllianceState[0][1] == mAlliencePostAvatarTribe[0]) && (
			mWorldInfo.mAllianceState[0][0] == mAlliencePostAvatarTribe[1])))
		{
			tAllianceIndex = 0;
		}
...
void MyGame::Process_Zone_037(void)
{
	...
	case 0:
		...
		//tuesday, thursday, saturday, 18:00 ~ 21:00
			if (((tPresentTime->tm_wday != 2) && (tPresentTime->tm_wday != 4) && (tPresentTime->tm_wday != 6)) || (
				tPresentTime->tm_hour != 18) || (tPresentTime->tm_min > 10))
			{
				return;
			}
	...
	case 1:
		...
		case 0:
			...
			if (((tPresentTime->tm_wday != 2) && (tPresentTime->tm_wday != 4) && (tPresentTime->tm_wday != 6)) || (
				tPresentTime->tm_hour != 21) || (tPresentTime->tm_min > 10))
			{
				return;
			}
				...
				if (mDuel_124_RemainTime == 1)
				{
					for (index01 = 0; index01 < mSERVER.mMAX_USER_NUM; ++index01)
					{
						...
						mTRANSFER.B_AVATAR_CHANGE_INFO_1(index01, mAVATAR_OBJECT[index01].mUniqueNumber, 7,
						                                 ...
						                                 mAVATAR_OBJECT[index01].mDATA.aDuelState[1],
						                                 mAVATAR_OBJECT[index01].mDATA.aDuelState[2]);
						mUTIL.Broadcast(TRUE, &mAVATAR_OBJECT[index01].mDATA.aAction.aLocation[0], UNIT_SCALE_RADIUS1,
						                false);
					}
					mDuel_124 = FALSE;
					mDuel_124_RemainTime = 0;
...
void MyGame::Process_Zone_049_TYPE(void)
{
	...
	/* Auto Win if no one inside */
		...
		for (index01 = 0; index01 < mSERVER.mMAX_USER_NUM; index01++)
		{
			...
			{
			case 0: tTribeUserNum[0]++;
				break;
			case 1: tTribeUserNum[1]++;
				break;
			case 2: tTribeUserNum[2]++;
				break;
			case 3: tTribeUserNum[3]++;
				break;
			}
		}
		if ((tTribeUserNum[0] > 0 && tTribeUserNum[1] == 0 && tTribeUserNum[2] == 0 && tTribeUserNum[3] == 0) || (
			tTribeUserNum[0] == 0 && tTribeUserNum[1] > 0 && tTribeUserNum[2] == 0 && tTribeUserNum[3] == 0) || (
			tTribeUserNum[0] == 0 && tTribeUserNum[1] == 0 && tTribeUserNum[2] > 0 && tTribeUserNum[3] == 0) || (
			tTribeUserNum[0] == 0 && tTribeUserNum[1] == 0 && tTribeUserNum[2] == 0 && tTribeUserNum[3] > 0))
		{
		...
		if (tWinTribe == -1)
		{
			...
			{
			case 1:
				if ((mZone049TypeBattleResult[0] > mZone049TypeBattleResult[2]) && (mZone049TypeBattleResult[0] >
					mZone049TypeBattleResult[3]))
				{
					tWinTribe = 0;
				}
				break;
			case 2:
				if ((mZone049TypeBattleResult[0] > mZone049TypeBattleResult[1]) && (mZone049TypeBattleResult[0] >
					mZone049TypeBattleResult[3]))
				{
					tWinTribe = 0;
				}
			...
			default:
				if ((mZone049TypeBattleResult[0] > mZone049TypeBattleResult[1]) && (mZone049TypeBattleResult[0] >
					mZone049TypeBattleResult[2]) && (mZone049TypeBattleResult[0] > mZone049TypeBattleResult[3]))
				{
					tWinTribe = 0;
				}
				break;
		...
		if (tWinTribe == -1)
		{
			...
			{
			case 0:
				if ((mZone049TypeBattleResult[1] > mZone049TypeBattleResult[2]) && (mZone049TypeBattleResult[1] >
					mZone049TypeBattleResult[3]))
				{
					tWinTribe = 1;
				}
				break;
			case 2:
				if ((mZone049TypeBattleResult[1] > mZone049TypeBattleResult[0]) && (mZone049TypeBattleResult[1] >
					mZone049TypeBattleResult[3]))
				{
					tWinTribe = 1;
				}
			...
			default:
				if ((mZone049TypeBattleResult[1] > mZone049TypeBattleResult[0]) && (mZone049TypeBattleResult[1] >
					mZone049TypeBattleResult[2]) && (mZone049TypeBattleResult[1] > mZone049TypeBattleResult[3]))
				{
					tWinTribe = 1;
				}
				break;
		...
		if (tWinTribe == -1)
		{
			...
			{
			case 0:
				if ((mZone049TypeBattleResult[2] > mZone049TypeBattleResult[1]) && (mZone049TypeBattleResult[2] >
					mZone049TypeBattleResult[3]))
				{
					tWinTribe = 2;
				}
				break;
			case 1:
				if ((mZone049TypeBattleResult[2] > mZone049TypeBattleResult[0]) && (mZone049TypeBattleResult[2] >
					mZone049TypeBattleResult[3]))
				{
					tWinTribe = 2;
				}
			...
			default:
				if ((mZone049TypeBattleResult[2] > mZone049TypeBattleResult[0]) && (mZone049TypeBattleResult[2] >
					mZone049TypeBattleResult[1]) && (mZone049TypeBattleResult[2] > mZone049TypeBattleResult[3]))
				{
					tWinTribe = 2;
				}
				break;
		...
		if (tWinTribe == -1)
		{
			...
			{
			case 0:
				if ((mZone049TypeBattleResult[3] > mZone049TypeBattleResult[1]) && (mZone049TypeBattleResult[3] >
					mZone049TypeBattleResult[2]))
				{
					tWinTribe = 3;
				}
				break;
			case 1:
				if ((mZone049TypeBattleResult[3] > mZone049TypeBattleResult[0]) && (mZone049TypeBattleResult[3] >
					mZone049TypeBattleResult[2]))
				{
					tWinTribe = 3;
				}
			...
			default:
				if ((mZone049TypeBattleResult[3] > mZone049TypeBattleResult[0]) && (mZone049TypeBattleResult[3] >
					mZone049TypeBattleResult[1]) && (mZone049TypeBattleResult[3] > mZone049TypeBattleResult[2]))
				{
					tWinTribe = 3;
				}
				break;
	...
	//----------------//
		for (index01 = 0; index01 < mSERVER.mMAX_USER_NUM; index01++)
		{
			...
			if ((mAVATAR_OBJECT[index01].mDATA.aTribe != tWinTribe) && (mAVATAR_OBJECT[index01].mDATA.aTribe !=
				ReturnAllianceTribe(tWinTribe)))
			{
				GetLevelBattleRewardInfo(mAVATAR_OBJECT[index01].mDATA.aLevel1, mAVATAR_OBJECT[index01].mDATA.aLevel2,
				                         &tGainExperience, &tGainMoney, FALSE);
				tMedalCnt = 3;
			}
			else
			{
				GetLevelBattleRewardInfo(mAVATAR_OBJECT[index01].mDATA.aLevel1, mAVATAR_OBJECT[index01].mDATA.aLevel2,
				                         &tGainExperience, &tGainMoney, TRUE);
				tMedalCnt = 10;
			}
#if 0
			if (mAVATAR_OBJECT[index01].mDATA.aLifeValue < 1)
			...
			if (mUSER[index01].mAvatarInfo.aDailyWarCount < 10)
			{
				mUSER[index01].mAvatarInfo.aDailyWarCount++;
			}

			if (mSERVER_INFO.mServerNumber >= 120 && mSERVER_INFO.mServerNumber <= 122) // Reward War End
			{
			...
			//QUEST//
			//-----//
			if ((mUSER[index01].mAvatarInfo.aQuestInfo[2] == 8) && (mUSER[index01].mAvatarInfo.aQuestInfo[3] ==
				mSERVER_INFO.mServerNumber) && (mAVATAR_OBJECT[index01].ReturnQuestPresentState() == 2))
			{
	...
	//-----------//
		if ((mZone051TypePostTick % 10) == 0)
		{
			mTRANSFER.B_051_TYPE_BATTLE_INFO(&tExistStone[0],
			                                 tRemainTime - static_cast<int>(mZone051TypePostTick * 0.5));
			mUTIL.Broadcast(FALSE, nullptr, 0, false);
		}
	//-----------//
	//-----------//
...
void MyGame::Process_Zone_053_TYPE(void)
{
	...
	case 3:
		...
		if (!tEnd)
		{
			...
			{
				mTRANSFER.B_053_TYPE_BATTLE_INFO(tRemainTime - static_cast<int>(mZone053TypePostTick * 0.5));
				mUTIL.Broadcast(FALSE, nullptr, 0, false);
			}
			if (mZone053TypePostAvatarIndex == -1)
			{
				for (index01 = 0; index01 < mSERVER.mMAX_USER_NUM; index01++)
				{
					...
					if (!mUTIL.CheckInRange(&mAVATAR_OBJECT[index01].mDATA.aAction.aLocation[0],
					                        &mZone053TypeBattlePostLocation[0], mZone053TypeBattlePostRadius))
					{
						continue;
					}
					break;
				...
				CopyMemory(&tBroadcastInfoData[0], &mZone053TypeZoneIndex, 4);
				tWinTribe = mAVATAR_OBJECT[index01].mDATA.aTribe;
			...
			if ((!mAVATAR_OBJECT[mZone053TypePostAvatarIndex].mCheckValidState) || (mAVATAR_OBJECT[
				mZone053TypePostAvatarIndex].mUniqueNumber != mZone053TypePostAvatarUniqueNumber) || (mAVATAR_OBJECT[
				mZone053TypePostAvatarIndex].mDATA.aLifeValue < 1) || (!mUTIL.CheckInRange(
				&mAVATAR_OBJECT[mZone053TypePostAvatarIndex].mDATA.aAction.aLocation[0],
				&mZone053TypeBattlePostLocation[0],
				mZone053TypeBattlePostRadius)) || (mUSER[mZone053TypePostAvatarIndex].mMoveZoneResult == 1) || (
				mAVATAR_OBJECT[mZone053TypePostAvatarIndex].mDATA.aVisibleState == 0) || (mAVATAR_OBJECT[
				mZone053TypePostAvatarIndex].mDATA.aAction.aSort == 33))
			{
				//--------------//
				//BROADCAST_INFO//
				//--------------//
			...
			tWinTribe = mAVATAR_OBJECT[mZone053TypePostAvatarIndex].mDATA.aTribe;
		}
		else
		{
			tWinTribe = -2;
	...
	//iTribe = static_cast<int> (mAVATAR_OBJECT[mZone053TypePostAvatarIndex].mDATA.aTribe);
		CopyMemory(&tBroadcastInfoData[4], &tWinTribe, 4);
		CopyMemory(&tBroadcastInfoData[8], &mAVATAR_OBJECT[mZone053TypePostAvatarIndex].mDATA.aName[0],
		           MAX_AVATAR_NAME_LENGTH);
		mCENTER_COM.U_DEMAND_BROADCAST_INFO(tBroadcastInfoSort, &tBroadcastInfoData[0]);
	//--------------//
	//--------------//
...
void MyGame::Process_Zone_038(void)
{
	...
	case 3:
		if (mZone038PostAvatarIndex == -1)
		{
			for (index01 = 0; index01 < mSERVER.mMAX_USER_NUM; index01++)
			{
				...
				if ((mAVATAR_OBJECT[index01].mDATA.aTribe == mWorldInfo.mZone038WinTribe) || (mAVATAR_OBJECT[index01].
					mDATA.aTribe == ReturnAllianceTribe(mWorldInfo.mZone038WinTribe)))
				{
					continue;
				}
				if (!mUTIL.CheckInRange(&mAVATAR_OBJECT[index01].mDATA.aAction.aLocation[0],
				                        &mZone038BattlePostLocation[0], mZone038BattlePostRadius))
				{
					continue;
				}
				break;
			...
			iTribe = mAVATAR_OBJECT[index01].mDATA.aTribe;
		...
		if ((!mAVATAR_OBJECT[mZone038PostAvatarIndex].mCheckValidState) || (mAVATAR_OBJECT[mZone038PostAvatarIndex].
			mUniqueNumber != mZone038PostAvatarUniqueNumber) || (mAVATAR_OBJECT[mZone038PostAvatarIndex].mDATA.
			aLifeValue < 1) || (!mUTIL.CheckInRange(&mAVATAR_OBJECT[mZone038PostAvatarIndex].mDATA.aAction.aLocation[0],
			                                        &mZone038BattlePostLocation[0],
			                                        mZone038BattlePostRadius)) || (mUSER[mZone038PostAvatarIndex].
			mMoveZoneResult == 1) || (mAVATAR_OBJECT[mZone038PostAvatarIndex].mDATA.aVisibleState == 0) || (
			mAVATAR_OBJECT[mZone038PostAvatarIndex].mDATA.aAction.aSort == 33))
		{
			//--------------//
			//BROADCAST_INFO//
			//--------------//
	...
	//--------------//
		...
		iTribe = mAVATAR_OBJECT[mZone038PostAvatarIndex].mDATA.aTribe;
		CopyMemory(&tBroadcastInfoData[0], &iTribe, 4);
		CopyMemory(&tBroadcastInfoData[4], &mAVATAR_OBJECT[mZone038PostAvatarIndex].mDATA.aName[0],
		           MAX_AVATAR_NAME_LENGTH);
		mCENTER_COM.U_DEMAND_BROADCAST_INFO(tBroadcastInfoSort, &tBroadcastInfoData[0]);
	//--------------//
	//--------------//
...
void MyGame::Process_Zone_039_TYPE(void)
{
	...
	case 1:
		...
		for (index01 = 0; index01 < mSERVER.mMAX_USER_NUM; index01++)
		{
			...
			if ((mUSER[index01].mAvatarInfo.aTribe != mWorldInfo.mZone038WinTribe) && (mUSER[index01].mAvatarInfo.aTribe
				!= ReturnAllianceTribe(mWorldInfo.mZone038WinTribe)))
			{
				mUSER[index01].Quit(__FILE__, __FUNCTION__, __LINE__);
			}
		}
...
void MyGame::Process_Zone_175_TYPE(void)
{
	...
	case 0:
		...
		//wendensday, friday, sunday 18:00
			if (((tPresentTime->tm_wday != 3) && (tPresentTime->tm_wday != 5) && (tPresentTime->tm_wday != 0)) || (
				tPresentTime->tm_hour != 18) || (tPresentTime->tm_min > 10))
			{
				return;
			}
	...
	case 5: //[1´Ü°è_ÁøÇà]
		...
		for (index01 = 0; index01 < MAX_MONSTER_OBJECT_NUM; index01++)
		{
			...
			if ((mMONSTER_OBJECT[index01].mMONSTER_INFO->mSpecialType == 40) || (mMONSTER_OBJECT[index01].mMONSTER_INFO
				->mSpecialType == 41) || (mMONSTER_OBJECT[index01].mMONSTER_INFO->mSpecialType == 42) || (
				mMONSTER_OBJECT[index01].mMONSTER_INFO->mSpecialType == 43) || (mMONSTER_OBJECT[index01].mMONSTER_INFO->
				mSpecialType == 44))
			{
				break;
			}
	...
	//----------------//
		for (index01 = 0; index01 < mSERVER.mMAX_USER_NUM; index01++)
		{
			...
			if (mAVATAR_OBJECT[index01].mDATA.aAction.aSort == 11)
			{
				...
				mTRANSFER.B_AVATAR_ACTION_RECV(index01, mAVATAR_OBJECT[index01].mUniqueNumber,
				                               &mAVATAR_OBJECT[index01].mDATA, 1);
				mUTIL.Broadcast(TRUE, &mAVATAR_OBJECT[index01].mDATA.aAction.aLocation[0], 1, false, 1);
			}

#ifdef __GOD__
			GetZone175TypeRewardInfo(mAVATAR_OBJECT[index01].mDATA.aLevel1, mAVATAR_OBJECT[index01].mDATA.aLevel2, 1,
			                         &tGainExperience, &tGainMoney);
#else
			...
			mUTIL.ProcessForDropItem(11, 1037, 0, 0, mUTIL.ReturnItemRecognitionNumber(0),
			                         &mAVATAR_OBJECT[index01].mDATA.aAction.aLocation[0],
			                         &mAVATAR_OBJECT[index01].mDATA.aName[0], 0);
			mUTIL.ProcessForDropItem(11, 1022, 0, 0, mUTIL.ReturnItemRecognitionNumber(0),
			                         &mAVATAR_OBJECT[index01].mDATA.aAction.aLocation[0],
			                         &mAVATAR_OBJECT[index01].mDATA.aName[0], 0);
			mUTIL.ProcessForDropItem(11, 1024, 0, 0, mUTIL.ReturnItemRecognitionNumber(0),
			                         &mAVATAR_OBJECT[index01].mDATA.aAction.aLocation[0],
			                         &mAVATAR_OBJECT[index01].mDATA.aName[0], 0);
		}
	//----------------//
	...
	//----------------//
		for (index01 = 0; index01 < mSERVER.mMAX_USER_NUM; index01++)
		{
...
#ifdef __GOD__
			GetZone175TypeRewardInfo(mAVATAR_OBJECT[index01].mDATA.aLevel1, mAVATAR_OBJECT[index01].mDATA.aLevel2, 2,
			                         &tGainExperience, &tGainMoney);
#else
			...
			mUTIL.ProcessForDropItem(11, 1037, 0, 0, mUTIL.ReturnItemRecognitionNumber(0),
			                         &mAVATAR_OBJECT[index01].mDATA.aAction.aLocation[0],
			                         &mAVATAR_OBJECT[index01].mDATA.aName[0], 0);
			mUTIL.ProcessForDropItem(11, 1023, 0, 0, mUTIL.ReturnItemRecognitionNumber(0),
			                         &mAVATAR_OBJECT[index01].mDATA.aAction.aLocation[0],
			                         &mAVATAR_OBJECT[index01].mDATA.aName[0], 0);
			mUTIL.ProcessForDropItem(11, 1025, 0, 0, mUTIL.ReturnItemRecognitionNumber(0),
			                         &mAVATAR_OBJECT[index01].mDATA.aAction.aLocation[0],
			                         &mAVATAR_OBJECT[index01].mDATA.aName[0], 0);
		}
	//----------------//
...
#ifdef __GOD__
			GetZone175TypeRewardInfo(mAVATAR_OBJECT[index01].mDATA.aLevel1, mAVATAR_OBJECT[index01].mDATA.aLevel2, 3,
			                         &tGainExperience, &tGainMoney);
#else
	...
	//----------------//
		for (index01 = 0; index01 < mSERVER.mMAX_USER_NUM; index01++)
		{
...
#ifdef __GOD__
			GetZone175TypeRewardInfo(mAVATAR_OBJECT[index01].mDATA.aLevel1, mAVATAR_OBJECT[index01].mDATA.aLevel2, 4,
			                         &tGainExperience, &tGainMoney);
#else
			...
			mUTIL.ProcessForDropItem(11, 1025, 0, 0, mUTIL.ReturnItemRecognitionNumber(0),
			                         &mAVATAR_OBJECT[index01].mDATA.aAction.aLocation[0],
			                         &mAVATAR_OBJECT[index01].mDATA.aName[0], 0);
			mUTIL.ProcessForDropItem(11, 1041, 0, 0, mUTIL.ReturnItemRecognitionNumber(0),
			                         &mAVATAR_OBJECT[index01].mDATA.aAction.aLocation[0],
			                         &mAVATAR_OBJECT[index01].mDATA.aName[0], 0);
		}
	//----------------//
	...
	//----------------//
		for (index01 = 0; index01 < mSERVER.mMAX_USER_NUM; index01++)
		{
...
#ifdef __GOD__
			GetZone175TypeRewardInfo(mAVATAR_OBJECT[index01].mDATA.aLevel1, mAVATAR_OBJECT[index01].mDATA.aLevel2, 5,
			                         &tGainExperience, &tGainMoney);
#else
			...
			mUTIL.ProcessForDropItem(11, 1041, 0, 0, mUTIL.ReturnItemRecognitionNumber(0),
			                         &mAVATAR_OBJECT[index01].mDATA.aAction.aLocation[0],
			                         &mAVATAR_OBJECT[index01].mDATA.aName[0], 0);
			mUTIL.ProcessForDropItem(11, 1049, 0, 0, mUTIL.ReturnItemRecognitionNumber(0),
			                         &mAVATAR_OBJECT[index01].mDATA.aAction.aLocation[0],
			                         &mAVATAR_OBJECT[index01].mDATA.aName[0], 0);
			mUTIL.ProcessForDropItem(11, 1048, 0, 0, mUTIL.ReturnItemRecognitionNumber(0),
			                         &mAVATAR_OBJECT[index01].mDATA.aAction.aLocation[0],
			                         &mAVATAR_OBJECT[index01].mDATA.aName[0], 0);
		}
	//----------------//
...
void MyGame::Process_Zone_194(void)
{
	...
	case 0:
		...
		case 0:
			...
			if ((tPresentTime->tm_hour != 21) || (tPresentTime->tm_min > 10))
			{
				return;
			}
			mZone194PostTick = 0;
	...
	case 2:
		if (mZone194PostTick < (120 * 1))
		{
			if ((mZone194PostTick % 2) == 1)
			{
				...
				mUTIL.Broadcast(FALSE, nullptr, 1);
			}
			return;
		}
	//--------------//
	...
	case 3:
		...
		if (mZone194RemainTime2 == 0)
		{
			if (tWinTribe == -1)
			{
				if ((mZone194BattleResult[0] > 0) && (mZone194BattleResult[0] > mZone194BattleResult[1]) && (
					mZone194BattleResult[0] > mZone194BattleResult[2]) && (mZone194BattleResult[0] >
					mZone194BattleResult[3]))
				{
					tWinTribe = 0;
				}
			}
			if (tWinTribe == -1)
			{
				if ((mZone194BattleResult[1] > 0) && (mZone194BattleResult[1] > mZone194BattleResult[0]) && (
					mZone194BattleResult[1] > mZone194BattleResult[2]) && (mZone194BattleResult[1] >
					mZone194BattleResult[3]))
				{
					tWinTribe = 1;
				}
			...
			if (tWinTribe == -1)
			{
				if ((mZone194BattleResult[2] > 0) && (mZone194BattleResult[2] > mZone194BattleResult[0]) && (
					mZone194BattleResult[2] > mZone194BattleResult[1]) && (mZone194BattleResult[2] >
					mZone194BattleResult[3]))
				{
					tWinTribe = 2;
				}
			...
			if (tWinTribe == -1)
			{
				if ((mZone194BattleResult[3] > 0) && (mZone194BattleResult[3] > mZone194BattleResult[0]) && (
					mZone194BattleResult[3] > mZone194BattleResult[1]) && (mZone194BattleResult[3] >
					mZone194BattleResult[2]))
				{
					tWinTribe = 3;
				}
			}
		...
		else
		{
			if (tWinTribe == -1)
			{
				if ((tTribeUserNum[0] > 0) && (tTribeUserNum[1] == 0) && (tTribeUserNum[2] == 0) && (tTribeUserNum[3] ==
					0))
				{
					tWinTribe = 0;
				}
			...
			if (tWinTribe == -1)
			{
				if ((tTribeUserNum[1] > 0) && (tTribeUserNum[0] == 0) && (tTribeUserNum[2] == 0) && (tTribeUserNum[3] ==
					0))
				{
					tWinTribe = 1;
				}
			...
			if (tWinTribe == -1)
			{
				if ((tTribeUserNum[2] > 0) && (tTribeUserNum[0] == 0) && (tTribeUserNum[1] == 0) && (tTribeUserNum[3] ==
					0))
				{
					tWinTribe = 2;
				}
			...
			if (tWinTribe == -1)
			{
				if ((tTribeUserNum[3] > 0) && (tTribeUserNum[0] == 0) && (tTribeUserNum[1] == 0) && (tTribeUserNum[2] ==
					0))
				{
					tWinTribe = 3;
				}
	...
	//----------------//
		for (index01 = 0; index01 < mSERVER.mMAX_USER_NUM; index01++)
		{
			...
			if (mAVATAR_OBJECT[index01].mDATA.aTribe == tWinTribe)
			{
				mUTIL.ProcessForDropItem(8, 694, 0, 0, mUTIL.ReturnItemRecognitionNumber(0),
				                         &mAVATAR_OBJECT[index01].mDATA.aAction.aLocation[0],
				                         &mUSER[index01].mAvatarInfo.aName[0], 0);
			}
		}
	//----------------//
	//----------------//
...
void MyGame::Process_Zone_195(void)
{
	...
	case 0:
		if (mZone195PostAvatarIndex == -1)
		{
			for (index01 = 0; index01 < mSERVER.mMAX_USER_NUM; index01++)
			{
				...
				if (((mAVATAR_OBJECT[index01].mDATA.aTribe + 1) == mWorldInfo.mProvingGroundsState[mZone195LevelState])
					|| ((mAVATAR_OBJECT[index01].mDATA.aTribe + 1) == ReturnAllianceTribe(
						mWorldInfo.mProvingGroundsState[mZone195LevelState])))
				{
					continue;
				}
				if (!mUTIL.CheckInRange(&mAVATAR_OBJECT[index01].mDATA.aAction.aLocation[0],
				                        &mZone195BattlePostLocation[0], mZone195BattlePostRadius))
				{
					continue;
				}
				break;
		...
		if ((!mAVATAR_OBJECT[mZone195PostAvatarIndex].mCheckValidState) || (mAVATAR_OBJECT[mZone195PostAvatarIndex].
			mUniqueNumber != mZone195PostAvatarUniqueNumber) || (mAVATAR_OBJECT[mZone195PostAvatarIndex].mDATA.
			aLifeValue < 1) || (!mUTIL.CheckInRange(&mAVATAR_OBJECT[mZone195PostAvatarIndex].mDATA.aAction.aLocation[0],
			                                        &mZone195BattlePostLocation[0],
			                                        mZone195BattlePostRadius)) || (mUSER[mZone195PostAvatarIndex].
			mMoveZoneResult == 1) || (mAVATAR_OBJECT[mZone195PostAvatarIndex].mDATA.aVisibleState == 0) || (
			mAVATAR_OBJECT[mZone195PostAvatarIndex].mDATA.aAction.aSort == 33))
		{
			//--------------//
			//BROADCAST_INFO//
			//--------------//
	...
	//--------------//
		...
		iTribe = mAVATAR_OBJECT[mZone195PostAvatarIndex].mDATA.aTribe;
		tZone = mSERVER_INFO.mServerNumber;
		CopyMemory(&tBroadcastInfoData[0], &iTribe, 4);
		CopyMemory(&tBroadcastInfoData[4], &tZone, 4);
		CopyMemory(&tBroadcastInfoData[8], &mAVATAR_OBJECT[mZone195PostAvatarIndex].mDATA.aName[0],
		           MAX_AVATAR_NAME_LENGTH);
	...
	//--------------//
	//--------------//
		mWorldInfo.mProvingGroundsState[mZone195LevelState] = (mAVATAR_OBJECT[mZone195PostAvatarIndex].mDATA.aTribe +
			1);
		mZone195PostTick = 0;
		mZone195PostAvatarIndex = -1;
		mZone195PostAvatarUniqueNumber = 0;
...
void MyGame::Process_Zone_241(void)
{
	...
	mZone241PostTick++;
	switch (mZone241BattleState[mZone241Type])
	{
	case 0:
		...
		if ((!mAVATAR_OBJECT[mZone241PostAvatarIndex].mCheckValidState) || (mAVATAR_OBJECT[mZone241PostAvatarIndex].
			mUniqueNumber != mZone241PostAvatarUniqueNumber) || (mAVATAR_OBJECT[mZone241PostAvatarIndex].mDATA.
			aVisibleState == 0) || (mUSER[mZone241PostAvatarIndex].mMoveZoneResult))
		{
			//--------------//
			//BROADCAST_INFO//
			//--------------//
			...
			CopyMemory(&tBroadcastInfoData[0], &mZone241Type, 4);
			CopyMemory(&tBroadcastInfoData[4], &mAVATAR_OBJECT[mZone241PostAvatarIndex].mDATA.aName[0],
			           MAX_AVATAR_NAME_LENGTH);
			mCENTER_COM.U_DEMAND_BROADCAST_INFO(tBroadcastInfoSort, &tBroadcastInfoData[0]);
			//--------------//
			//--------------//
	...
	case 3:
		...
		if (mZone267TypeRemainTime2 == 0)
		{
			if (tWinTribe == -1)
			{
				...
				{
				case 1:
					if ((mZone267TypeBattleResult[0] > mZone267TypeBattleResult[2]) && (mZone267TypeBattleResult[0] >
						mZone267TypeBattleResult[3]))
					{
						tWinTribe = 0;
					}
					break;
				case 2:
					if ((mZone267TypeBattleResult[0] > mZone267TypeBattleResult[1]) && (mZone267TypeBattleResult[0] >
						mZone267TypeBattleResult[3]))
					{
						tWinTribe = 0;
					}
				...
				default:
					if ((mZone267TypeBattleResult[0] > mZone267TypeBattleResult[1]) && (mZone267TypeBattleResult[0] >
						mZone267TypeBattleResult[2]) && (mZone267TypeBattleResult[0] > mZone267TypeBattleResult[3]))
					{
						tWinTribe = 0;
					}
					break;
			...
			if (tWinTribe == -1)
			{
				...
				{
				case 0:
					if ((mZone267TypeBattleResult[1] > mZone267TypeBattleResult[2]) && (mZone267TypeBattleResult[1] >
						mZone267TypeBattleResult[3]))
					{
						tWinTribe = 1;
					}
					break;
				case 2:
					if ((mZone267TypeBattleResult[1] > mZone267TypeBattleResult[0]) && (mZone267TypeBattleResult[1] >
						mZone267TypeBattleResult[3]))
					{
						tWinTribe = 1;
					}
				...
				default:
					if ((mZone267TypeBattleResult[1] > mZone267TypeBattleResult[0]) && (mZone267TypeBattleResult[1] >
						mZone267TypeBattleResult[2]) && (mZone267TypeBattleResult[1] > mZone267TypeBattleResult[3]))
					{
						tWinTribe = 1;
					}
					break;
			...
			if (tWinTribe == -1)
			{
				...
				{
				case 0:
					if ((mZone267TypeBattleResult[2] > mZone267TypeBattleResult[1]) && (mZone267TypeBattleResult[2] >
						mZone267TypeBattleResult[3]))
					{
						tWinTribe = 2;
					}
					break;
				case 1:
					if ((mZone267TypeBattleResult[2] > mZone267TypeBattleResult[0]) && (mZone267TypeBattleResult[2] >
						mZone267TypeBattleResult[3]))
					{
						tWinTribe = 2;
					}
				...
				default:
					if ((mZone267TypeBattleResult[2] > mZone267TypeBattleResult[0]) && (mZone267TypeBattleResult[2] >
						mZone267TypeBattleResult[1]) && (mZone267TypeBattleResult[2] > mZone267TypeBattleResult[3]))
					{
						tWinTribe = 2;
					}
					break;
			...
			if (tWinTribe == -1)
			{
				...
				{
				case 0:
					if ((mZone267TypeBattleResult[3] > mZone267TypeBattleResult[1]) && (mZone267TypeBattleResult[3] >
						mZone267TypeBattleResult[2]))
					{
						tWinTribe = 3;
					}
					break;
				case 1:
					if ((mZone267TypeBattleResult[3] > mZone267TypeBattleResult[0]) && (mZone267TypeBattleResult[3] >
						mZone267TypeBattleResult[2]))
					{
						tWinTribe = 3;
					}
				...
				default:
					if ((mZone267TypeBattleResult[3] > mZone267TypeBattleResult[0]) && (mZone267TypeBattleResult[3] >
						mZone267TypeBattleResult[1]) && (mZone267TypeBattleResult[3] > mZone267TypeBattleResult[2]))
					{
						tWinTribe = 3;
					}
					break;
		...
		else
		{
			if (tWinTribe == -1)
			{
				...
				case 3:
					...
					break;
				default:
					if ((tTribeUserNum[0] > 0) && (tTribeUserNum[1] == 0) && (tTribeUserNum[2] == 0) && (tTribeUserNum[
						3] == 0))
					{
						tWinTribe = 0;
					}
			...
			if (tWinTribe == -1)
			{
				...
				case 3:
					...
					break;
				default:
					if ((tTribeUserNum[1] > 0) && (tTribeUserNum[0] == 0) && (tTribeUserNum[2] == 0) && (tTribeUserNum[
						3] == 0))
					{
						tWinTribe = 1;
					}
			...
			if (tWinTribe == -1)
			{
				...
				case 3:
					...
					break;
				default:
					if ((tTribeUserNum[2] > 0) && (tTribeUserNum[0] == 0) && (tTribeUserNum[1] == 0) && (tTribeUserNum[
						3] == 0))
					{
						tWinTribe = 2;
					}
			...
			if (tWinTribe == -1)
			{
				...
				case 2:
					...
					break;
				default:
					if ((tTribeUserNum[3] > 0) && (tTribeUserNum[0] == 0) && (tTribeUserNum[1] == 0) && (tTribeUserNum[
						2] == 0))
					{
						tWinTribe = 3;
					}
	...
	//----------------//
		for (index01 = 0; index01 < mSERVER.mMAX_USER_NUM; index01++)
		{
			...
			if ((mAVATAR_OBJECT[index01].mDATA.aTribe != tWinTribe) && (mAVATAR_OBJECT[index01].mDATA.aTribe !=
				ReturnAllianceTribe(tWinTribe)))
			{
				ReturnLevelWarReward3(mAVATAR_OBJECT[index01].mDATA.aLevel1, mAVATAR_OBJECT[index01].mDATA.aLevel2,
				                      &tGainExperience, &tGainMoney);
				tGainExperience /= 2;
				tGainMoney /= 2;
				if (tGainExperience < 1)
