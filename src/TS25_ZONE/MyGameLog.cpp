...
void R_ProLog::GameLog(int tSort, char* tLogData, ...)
{
	...
	iPos += sprintf(tempString02 + iPos, "[%d:%d:%d] ", tPresentTime->tm_hour, tPresentTime->tm_min,
	                tPresentTime->tm_sec);
	...
	hFile = CreateFile(tempString01, GENERIC_WRITE, FILE_SHARE_WRITE, nullptr, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL,
	                   nullptr);
	if (hFile == INVALID_HANDLE_VALUE)
	{
		return;
	}
	if (SetFilePointer(hFile, 0, nullptr, FILE_END) == 0xFFFFFFFF)
	{
		CloseHandle(hFile);
		return;
	}
	if (!WriteFile(hFile, tempString02, strlen(tempString02), &nWriteByte, nullptr))
	{
		CloseHandle(hFile);
		return;
	}
