...
unsigned int MyGame::getCriticalAttackDefValue(AVATAR_OBJECT* _attacker, AVATAR_OBJECT* _defenser)
{
	...
	{
		t_cmp_critical = static_cast<unsigned int>(t_critical - t_critical_defense);
	}
...
void MyGame::ProcessAttack01(int tUserIndex, ATTACK_FOR_PROTOCOL* tAttackInfo)
{
	...
	if ((tAVATAR_OBJECT1->mDATA.aDuelState[0] != 1) || (tAVATAR_OBJECT2->mDATA.aDuelState[0] != 1) || (tAVATAR_OBJECT1->
		mDATA.aDuelState[1] != tAVATAR_OBJECT2->mDATA.aDuelState[1]) || (tAVATAR_OBJECT1->mDATA.aDuelState[2] ==
		tAVATAR_OBJECT2->mDATA.aDuelState[2]))
	{
		return;
	}
	...
	case 2:
		...
		{
			if ((tAVATAR_OBJECT1->mDATA.aAction.aSkillNumber != tAttackInfo->mAttackActionValue2) || ((tAVATAR_OBJECT1->
					mDATA.aAction.aSkillGradeNum1 + tAVATAR_OBJECT1->mDATA.aAction.aSkillGradeNum2) != tAttackInfo->
				mAttackActionValue3))
			{
				return;
			}
	...
	if (tAttackBlockValue > 0)
	{
		...
		{
			tAttackDetermineValue = static_cast<int>(70.0f + (((float)tAttackSuccessValue / (float)tAttackBlockValue) -
				1.0f) * 25.0f);
		...
		{
			tAttackDetermineValue = static_cast<int>(70.0f - (((float)tAttackBlockValue / (float)tAttackSuccessValue) -
				1.0f) * 25.0f);
		...
		tRandom = rand();
		if ((tRandom % 100) >= static_cast<unsigned int>(tAttackDetermineValue))
		{
	...
	case 2:
		tAttackPowerValue = tAVATAR_OBJECT1->GetAttackPower();
		tAttackPowerUpRatio = mSKILL.ReturnSkillValue(tAttackInfo->mAttackActionValue2,
		                                              tAttackInfo->mAttackActionValue3, 7);
		if (tAttackPowerUpRatio > 0.0f)
		{
			tAttackPowerValue = static_cast<int>((float)tAttackPowerValue * (tAttackPowerUpRatio + 100.0f) * 0.01f);
		}
		break;
	}
	...
	if (tChargingValue > 0)
	{
		tDamageValue = static_cast<int>((float)(tDamageValue * (tChargingValue + 100)) * 0.01f);
	}
	tRandom = rand();
	if ((tRandom % 2) == 0)
	{
		tRandom = rand();
		tDamageValue += static_cast<int>((float)(tDamageValue * (tRandom % 11)) * 0.01f);
	}
	else
	{
		tRandom = rand();
	...
	{
		tRandom = rand();
		if ((tRandom % 1000) < static_cast<unsigned int>(mUSER[tAttackInfo->mServerIndex1].mEffectValue[14][0]))
		{
	...
	if (mUSER[tAttackInfo->mServerIndex2].mEffectValue[12][0] > 0)
	{
		...
		{
			tRandom = rand();
			tReturnDamageValue = static_cast<int>((float)(tDamageValue * (100 + tRandom % 51)) * 0.01f);
			mTRANSFER.B_AVATAR_CHANGE_INFO_1(tAttackInfo->mServerIndex1, tAttackInfo->mUniqueNumber1, 4,
			                                 tReturnDamageValue, 0, 0);
			mUTIL.Broadcast(TRUE, &tAVATAR_OBJECT1->mDATA.aAction.aLocation[0], UNIT_SCALE_RADIUS1, false);
			...
			if (tAVATAR_OBJECT1->mDATA.aLifeValue < 1)
			{
				...
				//------------------------//
				tDeathDirection[0] = tAVATAR_OBJECT1->mDATA.aAction.aLocation[0] - tAVATAR_OBJECT2->mDATA.aAction.
					aLocation[0];
				tDeathDirection[2] = tAVATAR_OBJECT1->mDATA.aAction.aLocation[2] - tAVATAR_OBJECT2->mDATA.aAction.
					aLocation[2];
				...
				tAVATAR_OBJECT1->mDATA.aAction.aFront = mUTIL.GetYAngle(tAVATAR_OBJECT1->mDATA.aAction.aLocation[0],
				                                                        tAVATAR_OBJECT1->mDATA.aAction.aLocation[2],
				                                                        tAVATAR_OBJECT2->mDATA.aAction.aLocation[0],
				                                                        tAVATAR_OBJECT2->mDATA.aAction.aLocation[2]);
				...
				mTRANSFER.B_AVATAR_ACTION_RECV(tAttackInfo->mServerIndex1, tAVATAR_OBJECT1->mUniqueNumber,
				                               &tAVATAR_OBJECT1->mDATA, 1);
				mUSER[tAttackInfo->mServerIndex1].Send(TRUE, (char*)&mTRANSFER.mOriginal[0], mTRANSFER.mOriginalSize);
				mUTIL.Broadcast(TRUE, &tAVATAR_OBJECT1->mDATA.aAction.aLocation[0], UNIT_SCALE_RADIUS1,
				                tAttackInfo->mServerIndex1, false);
	...
	if (mUSER[tAttackInfo->mServerIndex2].mEffectValue[9][0] > 0)
	{
		...
		mTRANSFER.B_AVATAR_CHANGE_INFO_1(tAttackInfo->mServerIndex2, tAttackInfo->mUniqueNumber2, 5, tShieldDamageValue,
		                                 0, 0);
		mUTIL.Broadcast(TRUE, &tAVATAR_OBJECT2->mDATA.aAction.aLocation[0], UNIT_SCALE_RADIUS1, false);
	...
	{
	case 3:
		tElementAttackPowerUpRatio = mSKILL.ReturnSkillValue(tAttackInfo->mAttackActionValue2,
		                                                     tAttackInfo->mAttackActionValue3, 8);
		if (tElementAttackPowerUpRatio > 0.0f)
		{
			tElementAttackPower = static_cast<int>((float)tElementAttackPower * (tElementAttackPowerUpRatio + 100.0f) *
				0.01f);
		}
		break;
	}
	...
	tAVATAR_OBJECT2->mDATA.aAction.aFront = mUTIL.GetYAngle(tAVATAR_OBJECT2->mDATA.aAction.aLocation[0],
	                                                        tAVATAR_OBJECT2->mDATA.aAction.aLocation[2],
	                                                        tAVATAR_OBJECT1->mDATA.aAction.aLocation[0],
	                                                        tAVATAR_OBJECT1->mDATA.aAction.aLocation[2]);
	...
	mTRANSFER.B_AVATAR_ACTION_RECV(tAttackInfo->mServerIndex2, tAVATAR_OBJECT2->mUniqueNumber, &tAVATAR_OBJECT2->mDATA,
	                               1);
	mUSER[tAttackInfo->mServerIndex2].Send(TRUE, (char*)&mTRANSFER.mOriginal[0], mTRANSFER.mOriginalSize);
	mUTIL.Broadcast(TRUE, &tAVATAR_OBJECT2->mDATA.aAction.aLocation[0], UNIT_SCALE_RADIUS1, tAttackInfo->mServerIndex2,
	                false);
...
void MyGame::ProcessAttack02(int tUserIndex, ATTACK_FOR_PROTOCOL* tAttackInfo)
{
	...
	if ((mZONEMAININFO.ReturnZoneTribeInfo2(mSERVER_INFO.mServerNumber) == 0) || (tAVATAR_OBJECT1->mDATA.aTribe ==
		tAVATAR_OBJECT2->mDATA.aTribe) || (tAVATAR_OBJECT2->mDATA.aTribe == ReturnAllianceTribe(
		tAVATAR_OBJECT1->mDATA.aTribe)))
	{
		return;
	}
	...
	if (mUSER[tAttackInfo->mServerIndex1].mEffectValue[15][1] > 0 && mAVATAR_OBJECT[tAttackInfo->mServerIndex2].mDATA.
		bStateDarkAttack == false)
	{
		tRandom = rand() % 100;
		...
		if (tRandom < mUSER[tAttackInfo->mServerIndex1].mEffectValue[15][0])
		{
			...
			mUSER[tAttackInfo->mServerIndex2].mTickCountForDarkAttack = GetTickCount64();
	...
	{
		tAttackPowerValue = static_cast<int>((float)tAttackPowerValue * 1.1f);
	}
	//--------//
	//--------//
	//--------//
	...
	{
		tDefensePowerValue = static_cast<int>((float)tDefensePowerValue * 1.1f);
	}
	//--------//
	//--------//
	//--------//
	...
	case 1:
		...
		//일격진법_강벽진법//
		//-----------------//
		if ((mWorldInfo.mTribeMasterCallAbility[tAVATAR_OBJECT1->mDATA.aTribe] != 3) && (mWorldInfo.
			mTribeMasterCallAbility[tAVATAR_OBJECT2->mDATA.aTribe] != 4))
		{
			tRandom = rand();
		...
		else if ((mWorldInfo.mTribeMasterCallAbility[tAVATAR_OBJECT1->mDATA.aTribe] == 3) && (mWorldInfo.
			mTribeMasterCallAbility[tAVATAR_OBJECT2->mDATA.aTribe] != 4))
		{
			tRandom = rand();
	...
	if (mUSER[tAttackInfo->mServerIndex2].mEffectValue[12][0] > 0)
	{
		...
		if ((tRandom % 1000) < tReturnDamageProbability)
		{
			...
			if (tAVATAR_OBJECT1->mDATA.aLifeValue < 1)
			{
				...
				//------------------------//
				//------------------------//
				mUTIL.ProcessForKillOtherTribe(tAttackInfo->mServerIndex2, tAttackInfo->mServerIndex1,
...
void MyGame::ProcessAttack03(int tUserIndex, ATTACK_FOR_PROTOCOL* tAttackInfo)
{
	...
	if ((strlen(tMONSTER_OBJECT->mAvatarName) > 0) && (strncmp(tMONSTER_OBJECT->mAvatarName,
	                                                           mUSER[tUserIndex].mAvatarInfo.aName,
	                                                           MAX_AVATAR_NAME_LENGTH) != 0))
	{
		return;
	}

	float fHeight = abs(
		static_cast<int>(tAVATAR_OBJECT1->mDATA.aAction.aLocation[1] - tMONSTER_OBJECT->mDATA.mAction.aLocation[1]));
	if (fHeight > 40)
	{
		return;
	}
	/* Vacuum Protection */
	if (!mUTIL.CheckRAttackRange(tUserIndex, tAttackInfo->mAttackActionValue2,
	                             &tAVATAR_OBJECT1->mDATA.aAction.aLocation[0]
	                             , &tMONSTER_OBJECT->mDATA.mAction.aLocation[0], tMONSTER_OBJECT->mRadiusForSize))
	{
		return;
	}
	tChargingValue = mUSER[tAttackInfo->mServerIndex1].mEffectValue[8][0];
	...
	case 2:
		...
		{
		case 11:
			if ((mWorldInfo.mTribeSymbolBattle == 0) || (mWorldInfo.mTribe1Symbol == tAVATAR_OBJECT1->mDATA.aTribe) || (
				mWorldInfo.mTribe1Symbol == tAllianceTribe))
			{
				return;
			}
			if (!tMONSTER_OBJECT->mCheckFirstAttackForTribeSymbol)
			{
				...
				tMONSTER_OBJECT->mFirstAttackTimeForTribeSymbol = GetTickCount64();
				//--------------//
				//BROADCAST_INFO//
				//--------------//
				tTribeSymbolIndex = 0;
				...
				iTribe = tAVATAR_OBJECT1->mDATA.aTribe;
			...
			break;
		case 12:
			if ((mWorldInfo.mTribeSymbolBattle == 0) || (mWorldInfo.mTribe2Symbol == tAVATAR_OBJECT1->mDATA.aTribe) || (
				mWorldInfo.mTribe2Symbol == tAllianceTribe))
			{
				return;
			}
			if (!tMONSTER_OBJECT->mCheckFirstAttackForTribeSymbol)
			...
			break;
		case 13:
			if ((mWorldInfo.mTribeSymbolBattle == 0) || (mWorldInfo.mTribe3Symbol == tAVATAR_OBJECT1->mDATA.aTribe) || (
				mWorldInfo.mTribe3Symbol == tAllianceTribe))
			{
				return;
			}
			if (!tMONSTER_OBJECT->mCheckFirstAttackForTribeSymbol)
			...
			break;
		case 28:
			if ((mWorldInfo.mTribeSymbolBattle == 0) || (mWorldInfo.mTribe4Symbol == tAVATAR_OBJECT1->mDATA.aTribe) || (
				mWorldInfo.mTribe4Symbol == tAllianceTribe))
			{
				return;
			}
			if (!tMONSTER_OBJECT->mCheckFirstAttackForTribeSymbol)
	...
	case 4:
		...
		case 31:
			...
			if (!tMONSTER_OBJECT->mCheckFirstAttackForAllianceStone)
			{
				...
				tMONSTER_OBJECT->mFirstAttackTimeForAllianceStone = GetTickCount64();
				//--------------//
				//BROADCAST_INFO//
				//--------------//
	...
	case 5:
	case 6:
		tMONSTER_OBJECT->SetAttackInfoWithAvatar(tAttackInfo->mServerIndex1, tAttackInfo->mUniqueNumber1,
		                                         &tWhoAttackMonster);
		break;
	...
	if (ReturnSymbolDamageUp(tAVATAR_OBJECT1->mDATA.aTribe) > 0.0f)
	{
		...
		tDamageValue = static_cast<int>((float)tDamageValue + ReturnSymbolDamageUp(tAVATAR_OBJECT1->mDATA.aTribe));
	}
	if (tAVATAR_OBJECT1->mDATA.aLevel1 > 112)
	{
		if (ReturnSymbolDamageDown(tAVATAR_OBJECT1->mDATA.aTribe) > 0.0f)
		{
			tDamageValue -= static_cast<int>((float)tDamageValue *
				ReturnSymbolDamageDown(tAVATAR_OBJECT1->mDATA.aTribe));
		}
	}
	//-------------//
	//-------------//
	...
	{
		tRandom = rand();
		if ((tMONSTER_OBJECT->mMONSTER_INFO->mDamageType != 1) && ((tRandom % 2) == 0) && (tDamageValue > static_cast<
			int>((float)tMONSTER_OBJECT->mMONSTER_INFO->mLife * 0.10f)) && (tMONSTER_OBJECT->mDATA.mAction.aSort != 8))
		{
			tMONSTER_OBJECT->mDATA.mAction.aSort = 8;
			tMONSTER_OBJECT->mDATA.mAction.aFrame = 0.0f;
			tMONSTER_OBJECT->mDATA.mAction.aFront = mUTIL.GetYAngle(tMONSTER_OBJECT->mDATA.mAction.aLocation[0],
			                                                        tMONSTER_OBJECT->mDATA.mAction.aLocation[2],
			                                                        tAVATAR_OBJECT1->mDATA.aAction.aLocation[0],
			                                                        tAVATAR_OBJECT1->mDATA.aAction.aLocation[2]);
			//------------------------//
			//BROADCAST_MONSTER_ACTION//
			//------------------------//
			mTRANSFER.B_MONSTER_ACTION_RECV(tAttackInfo->mServerIndex2, tMONSTER_OBJECT->mUniqueNumber,
			                                &tMONSTER_OBJECT->mDATA, 1);
			//-----------------------------------------------------------------//
			//[01].일반,[02].성석,[03].수호비,[04].동맹석,[05].수호석,[06].석차//
			//-----------------------------------------------------------------//
			switch (tMONSTER_OBJECT->mSpecialSortNumber)
	...
	if (mUSER[tUserIndex].mAvatarInfo.aDailyPVMKillCount < 1000) { mUSER[tUserIndex].mAvatarInfo.aDailyPVMKillCount++; }
	...
	//몬스터사냥_기여도_처리//
	//----------------------//
	if ((mUTIL.ReturnFixedLevel((mUSER[tUserIndex].mAvatarInfo.aLevel1 + mUSER[tUserIndex].mAvatarInfo.aLevel2)) -
		tMONSTER_OBJECT->mMONSTER_INFO->mRealLevel) < 10)
	{
	...
	//BROADCAST_MONSTER_ACTION//
	//------------------------//
	mTRANSFER.B_MONSTER_ACTION_RECV(tAttackInfo->mServerIndex2, tMONSTER_OBJECT->mUniqueNumber, &tMONSTER_OBJECT->mDATA,
	                                1);
	//-----------------------------------------------------------------//
	//[01].일반,[02].성석,[03].수호비,[04].동맹석,[05].수호석,[06].석차//
	//-----------------------------------------------------------------//
	...
	//------------------------//
	//------------------------//
	if ((tSelectAvatarIndex == -1) || (tMONSTER_OBJECT->mMONSTER_INFO->mGeneralExperience < 1) || ((mUTIL.
			ReturnFixedLevel(mUSER[tSelectAvatarIndex].mAvatarInfo.aLevel1) - tMONSTER_OBJECT->mMONSTER_INFO->
			mRealLevel) >
		9))
	{
		return;
	...
	//몬스터//
	//------//
	if (tMONSTER_OBJECT->mMONSTER_INFO->mRealLevel > mUTIL.ReturnFixedLevel(
		mUSER[tSelectAvatarIndex].mAvatarInfo.aLevel1))
	{
		if ((tMONSTER_OBJECT->mMONSTER_INFO->mRealLevel - mUTIL.ReturnFixedLevel(
			mUSER[tSelectAvatarIndex].mAvatarInfo.aLevel1)) > 20)
		{
			tGainValue = static_cast<int>((float)tMONSTER_OBJECT->mMONSTER_INFO->mGeneralExperience * (3.0f + mUSER[
					tSelectAvatarIndex].
				mGeneralExperienceUpRatio));
		}
		else
		{
			tGainValue = static_cast<int>((float)tMONSTER_OBJECT->mMONSTER_INFO->mGeneralExperience * ((1.0f + mUSER[
					tSelectAvatarIndex].
				mGeneralExperienceUpRatio) + (float)(tMONSTER_OBJECT->mMONSTER_INFO->mRealLevel - mUTIL.
				ReturnFixedLevel(
					mUSER[tSelectAvatarIndex].mAvatarInfo.aLevel1)) * 0.1f));
		}
	}
	else
	{
		tGainValue = static_cast<int>((float)tMONSTER_OBJECT->mMONSTER_INFO->mGeneralExperience * ((1.0f + mUSER[
				tSelectAvatarIndex].
			mGeneralExperienceUpRatio) - (float)(mUTIL.ReturnFixedLevel(mUSER[tSelectAvatarIndex].mAvatarInfo.aLevel1) -
			tMONSTER_OBJECT->mMONSTER_INFO->mRealLevel) * 0.1f));
	}
	//------//
	//------//
	//------//
	...
	//막타정보_반영//
	//-------------//
	if (strcmp(mUSER[tSelectAvatarIndex].mAvatarInfo.aPartyName[0], "") == 0)
	{
		tRandom = rand();
		if ((tRandom % 100) < mAVATAR_OBJECT[tSelectAvatarIndex].GetLastAttackBonusProbability())
		{
			tGainValue += static_cast<int>((float)tMONSTER_OBJECT->mMONSTER_INFO->mGeneralExperience * mAVATAR_OBJECT[
					tSelectAvatarIndex].
				GetLastAttackBonusValue());
		}
	}
	//--------------------//
	...
	//용폭진//
	//------//
	if ((mUSER[tSelectAvatarIndex].mAvatarInfo.aTribe == mWorldInfo.mZone038WinTribe) || (mUSER[tSelectAvatarIndex].
		mAvatarInfo.aTribe == ReturnAllianceTribe(mGAME.mWorldInfo.mZone038WinTribe)))
	{
		tGainValue += static_cast<int>((float)tMONSTER_OBJECT->mMONSTER_INFO->mGeneralExperience * 0.1f);
	}
	//------------//
	// Time-Effect//
	//------------//
	if ((mUSER[tSelectAvatarIndex].mAvatarInfo.aTimeEffect == 360) && (mUSER[tSelectAvatarIndex].mAvatarInfo.
		aTimeEffect2 == 2))
	{
		tGainValue += static_cast<int>((float)tMONSTER_OBJECT->mMONSTER_INFO->mGeneralExperience * 0.2f);
	}
	//------//
	//------//
	//------//
	...
	//사제_반영//
	//---------//
	if ((strcmp(mUSER[tSelectAvatarIndex].mAvatarInfo.aTeacher, "") != 0) && (mUSER[tSelectAvatarIndex].mAvatarInfo.
		aLevel1 < 113) && (strcmp(mUSER[tSelectAvatarIndex].mAvatarInfo.aPartyName[0], "") == 0))
	{
		for (index01 = 0; index01 < mSERVER.mMAX_USER_NUM; index01++)
		{
			if (!mAVATAR_OBJECT[index01].mCheckValidState)
			...
			if ((strcmp(mUSER[index01].mAvatarInfo.aName, mUSER[tSelectAvatarIndex].mAvatarInfo.aTeacher) == 0) && (
				strcmp(mUSER[index01].mAvatarInfo.aStudent, mUSER[tSelectAvatarIndex].mAvatarInfo.aName) == 0))
			{
				break;
			}
		}
		...
		{
			tGainValue += static_cast<int>((float)tMONSTER_OBJECT->mMONSTER_INFO->mGeneralExperience * 0.5f) * mGAME.
				mTeacherPointUpRatio;
			if (mUSER[index01].mAvatarInfo.aTeacherPoint < MAX_NUMBER_SIZE)
			{
				mUSER[index01].mAvatarInfo.aTeacherPoint += tMONSTER_OBJECT->mMONSTER_INFO->mItemLevel * mGAME.
					mTeacherPointUpRatio;
	...
	{
		tGainValue = static_cast<int>((float)tGainValue * mGeneralExperienceUpRatio);
	}

	if (mPatExperienceUpRatio > 1.0f)
	{
		tPatExperience = static_cast<int>((float)tMONSTER_OBJECT->mMONSTER_INFO->mPatExperience *
	...
	// Time Effect 3
	if ((mUSER[tSelectAvatarIndex].mAvatarInfo.aTimeEffect == 480) && (mUSER[tSelectAvatarIndex].mAvatarInfo.
		aTimeEffect2 == 3))
	{
	...
	mUTIL.ProcessForExperience(tSelectAvatarIndex, tGainValue, tPatExperience,
	                           (tMONSTER_OBJECT->mMONSTER_INFO->mLife / 100));
	if (strcmp(mUSER[tSelectAvatarIndex].mAvatarInfo.aPartyName[0], "") == 0)
	{
		return;
	}
	...
	for (index01 = 0; index01 < mSERVER.mMAX_USER_NUM; index01++)
	{
		...
		if (strcmp(mAVATAR_OBJECT[index01].mDATA.aPartyName, mUSER[tSelectAvatarIndex].mAvatarInfo.aPartyName[0]) != 0)
		{
			continue;
		}
	...
	{
	case 2: tBonusGainGeneralExperience = static_cast<int>((float)tMONSTER_OBJECT->mMONSTER_INFO->mGeneralExperience *
			0.1f);
		break;
	case 3: tBonusGainGeneralExperience = static_cast<int>((float)tMONSTER_OBJECT->mMONSTER_INFO->mGeneralExperience *
			0.2f);
		break;
	case 4: tBonusGainGeneralExperience = static_cast<int>((float)tMONSTER_OBJECT->mMONSTER_INFO->mGeneralExperience *
			0.3f);
		break;
	case 5: tBonusGainGeneralExperience = static_cast<int>((float)tMONSTER_OBJECT->mMONSTER_INFO->mGeneralExperience *
			0.5f);
		break;
	}
	if (mBonusGainGeneralExperienceUpRatio > 1.0f)
	{
		tBonusGainGeneralExperience = static_cast<int>((float)tBonusGainGeneralExperience *
			mBonusGainGeneralExperienceUpRatio);
	}
...
void MyGame::ProcessAttack04(int tUserIndex, ATTACK_FOR_PROTOCOL* tAttackInfo)
{
	...
	if ((tAttackInfo->mServerIndex2 != tUserIndex) || (tAttackInfo->mUniqueNumber2 != mAVATAR_OBJECT[tUserIndex].
		mUniqueNumber))
	{
		mUSER[tUserIndex].Quit(__FILE__, __FUNCTION__, __LINE__);
		return;
	...
	if (mUTIL.GetDoubleX_Z(&tMONSTER_OBJECT->mDATA.mAction.aLocation[0], &tAVATAR_OBJECT1->mDATA.aAction.aLocation[0]) >
		static_cast<float>(tMONSTER_OBJECT->mMONSTER_INFO->mRadiusInfo[1] * tMONSTER_OBJECT->mMONSTER_INFO->mRadiusInfo[
			1]))
	{
		return;
	}
	...
	if (tAttackInfo->mAttackActionValue4 == 0)
	{
		...
		case 40:
			...
			if ((tRandom % 100) < 10)
			{
				...
				mTRANSFER.B_AVATAR_ACTION_RECV(tAttackInfo->mServerIndex2, tAVATAR_OBJECT1->mUniqueNumber,
				                               &tAVATAR_OBJECT1->mDATA, 1);
				mUTIL.Broadcast(TRUE, &tAVATAR_OBJECT1->mDATA.aAction.aLocation[0], UNIT_SCALE_RADIUS1, false);
			}
			break;
	...
	if (mUSER[tAttackInfo->mServerIndex2].mEffectValue[9][0] > 0)
	{
		...
		mTRANSFER.B_AVATAR_CHANGE_INFO_1(tAttackInfo->mServerIndex2, tAttackInfo->mUniqueNumber2, 6, tShieldDamageValue,
		                                 0, 0);
		mUTIL.Broadcast(TRUE, &tAVATAR_OBJECT1->mDATA.aAction.aLocation[0], UNIT_SCALE_RADIUS1, false);
		tDamageValue -= tShieldDamageValue;
	}
	...
	tAVATAR_OBJECT1->mDATA.aAction.aFront = mUTIL.GetYAngle(tAVATAR_OBJECT1->mDATA.aAction.aLocation[0],
	                                                        tAVATAR_OBJECT1->mDATA.aAction.aLocation[2],
	                                                        tMONSTER_OBJECT->mDATA.mAction.aLocation[0],
	                                                        tMONSTER_OBJECT->mDATA.mAction.aLocation[2]);
	tAVATAR_OBJECT1->mDATA.aAction.aTargetFront = tAVATAR_OBJECT1->mDATA.aAction.aFront;
	tAVATAR_OBJECT1->mDATA.aAction.aSkillNumber = 5;
	mTRANSFER.B_AVATAR_ACTION_RECV(tAttackInfo->mServerIndex2, tAVATAR_OBJECT1->mUniqueNumber, &tAVATAR_OBJECT1->mDATA,
	                               1);
	mUSER[tAttackInfo->mServerIndex2].Send(TRUE, (char*)&mTRANSFER.mOriginal[0], mTRANSFER.mOriginalSize);
	mUTIL.Broadcast(TRUE, &tAVATAR_OBJECT1->mDATA.aAction.aLocation[0], UNIT_SCALE_RADIUS1, tAttackInfo->mServerIndex1,
	                false);
	tAVATAR_OBJECT1->ProcessForDeath();
	...
	{
	case 1:
		if ((mUSER[tAttackInfo->mServerIndex2].mAvatarInfo.aLevel1 < 10) || (mUSER[tAttackInfo->mServerIndex2].
		                                                                     mAvatarInfo.aLevel1 >= 145))
		{
			...
			if (mUSER[tAttackInfo->mServerIndex2].mAvatarInfo.aLevel1 >= 145)
			{
				if (mUSER[tAttackInfo->mServerIndex2].mAvatarInfo.aProtectForDeath > 0)
				{
					...
					mUSER[tAttackInfo->mServerIndex2].Send(TRUE, (char*)&mTRANSFER.mOriginal[0],
					                                       mTRANSFER.mOriginalSize);
				}
				else
				{
		...
		tMinusGeneralExperience = static_cast<int>((float)(mUSER[tAttackInfo->mServerIndex2].mAvatarInfo.
			aGeneralExperience1 - mLEVEL.
			ReturnLevelFactor1(mUSER[tAttackInfo->mServerIndex2].mAvatarInfo.aLevel1)) * (0.05f * mUSER[tAttackInfo->
			mServerIndex2].mGeneralExperienceDownRatio));
#if 0
...
void MyGame::ProcessAttack05(int tUserIndex, ATTACK_FOR_PROTOCOL* tAttackInfo)
{
	...
	if ((mZONEMAININFO.ReturnZoneTribeInfo2(mSERVER_INFO.mServerNumber) == 0) || (tAVATAR_OBJECT1->mDATA.aTribe ==
		...
		tAVATAR_OBJECT1->mDATA.aTribe)))
	{
		if ((tAVATAR_OBJECT1->mDATA.aDuelState[0] != 1) || (tAVATAR_OBJECT2->mDATA.aDuelState[0] != 1) || (
			tAVATAR_OBJECT1->mDATA.aDuelState[1] != tAVATAR_OBJECT2->mDATA.aDuelState[1]) || (tAVATAR_OBJECT1->mDATA.
			aDuelState[2] == tAVATAR_OBJECT2->mDATA.aDuelState[2]))
		{
			return;
		}
	}
	...
	tAttackSuccessValue = static_cast<int>(mSKILL.ReturnSkillValue(tAttackInfo->mAttackActionValue2,
	                                                               tAttackInfo->mAttackActionValue3, 4));
	...
	if (tAttackInfo->mAttackActionValue2 != 80)
	{
		...
		{
			if ((mUSER[tAttackInfo->mServerIndex2].mAvatarInfo.aSkill[index01][0] == 5) || (mUSER[tAttackInfo->
				mServerIndex2].mAvatarInfo.aSkill[index01][0] == 24) || (mUSER[tAttackInfo->mServerIndex2].mAvatarInfo.
				aSkill[index01][0] == 43))
			{
				break;
			}
		...
		{
			tAttackBlockValue = static_cast<int>(mSKILL.ReturnSkillValue(
				mUSER[tAttackInfo->mServerIndex2].mAvatarInfo.aSkill[index01][0],
				(mUSER[tAttackInfo->mServerIndex2].mAvatarInfo.aSkill[index01][1] +
					tAVATAR_OBJECT2->GetBonusSkillValue(
						mUSER[tAttackInfo->mServerIndex2].mAvatarInfo.aSkill[index01][0])), 5));
		...
		{
			tAttackBlockValue = static_cast<int>((float)(tAttackBlockValue * (tAVATAR_OBJECT2->mDATA.aEffectValueForView
				[13] + 100)) * 0.01f);
			//LOG_TO_FILE_1("tAttackBlockValue : %d\n", tAttackBlockValue);
		}
	}
	...
	if (static_cast<int>(tRandom) > tAttackSuccessValue - tAttackBlockValue - 1)
	{
		return;
	}
	tAVATAR_OBJECT2->mDATA.aAction.aType = 0;
	...
	tAVATAR_OBJECT2->mDATA.aAction.aFrame = 0.0f;
	tAVATAR_OBJECT2->mDATA.aAction.aSkillValue = static_cast<int>(mSKILL.ReturnSkillValue(
		tAttackInfo->mAttackActionValue2, tAttackInfo->mAttackActionValue3, 10));
	mTRANSFER.B_AVATAR_ACTION_RECV(tAttackInfo->mServerIndex2, tAVATAR_OBJECT2->mUniqueNumber, &tAVATAR_OBJECT2->mDATA,
	                               1);
	mUTIL.Broadcast(TRUE, &tAVATAR_OBJECT2->mDATA.aAction.aLocation[0], UNIT_SCALE_RADIUS1, false);
}
...
void MyGame::ProcessAttack06(int tUserIndex, ATTACK_FOR_PROTOCOL* tAttackInfo)
{
	...
	if ((tAVATAR_OBJECT1->mDATA.aTribe != tAVATAR_OBJECT2->mDATA.aTribe) && (tAVATAR_OBJECT1->mDATA.aTribe !=
		ReturnAllianceTribe(tAVATAR_OBJECT2->mDATA.aTribe)))
	{
		return;
	}
	if (mUSER[tUserIndex].mCheckMaxAttackPacketNum == 1)
	...
	tAttackSuccessValue = static_cast<int>(mSKILL.ReturnSkillValue(tAttackInfo->mAttackActionValue2,
	                                                               tAttackInfo->mAttackActionValue3, 5));
	if (tAttackSuccessValue < 1)
	{
		return;
	}
