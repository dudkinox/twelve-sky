...
public:
	...
	int GetAbsorbBonusStats(int tUserIndex);
	...
	BOOL ProcessForDropItem(int tDropSort, int tItemIndex, int tItemQuantity, int tItemValue,
	...
	void Broadcast(BOOL tCheckInRange, float tPostLocation[3], int iScale, int tUserIndexForNoSend,
	               bool bUDPSend = true, int type = 1);
	...
	int GetEmptyHotKeyPage(int tUserIndex);
	int GetEmptyHotKeyIndex(int tUserIndex);
	...
	bool CheckRAttackRange(int tUserIndex, int tSkillNumber, float tLocation1[3], float tLocation2[3],
	                       float tObjectSize);
	float ReturnLengthXYZ(float tLocation01[3], float tLocation02[3]);
	int CheckPVNumber(int tTribe);
	int GetRandomGachaItem();
	void WarlordChanceDrop(int tUserIndex);
	// # }
