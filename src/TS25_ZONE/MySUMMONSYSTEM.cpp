...
MySummonSystem::MySummonSystem(void)
{
	...
	mMONSTER_SUMMON = nullptr;
	mTotalBossMonsterSummonInfoNum = 0;
	mBOSS_MONSTER_SUMMON = nullptr;
}
...
void MySummonSystem::LoadRegionInfo_1(char* tFileName)
{
	...
	if (!ReadFile(hFile, &tTotalRegionInfoNum, 4, &nReadByte, nullptr))
	{
		CloseHandle(hFile);
		return;
	}
	...
	{
		tRegionInfo = static_cast<WORLD_REGION_INFO*>(GlobalAlloc(
			GMEM_FIXED, (tTotalRegionInfoNum * sizeof(WORLD_REGION_INFO))));
		if (tRegionInfo == nullptr)
		{
			CloseHandle(hFile);
			return;
		}
		if (!ReadFile(hFile, &tRegionInfo[0], (tTotalRegionInfoNum * sizeof(WORLD_REGION_INFO)), &nReadByte, nullptr))
		{
	...
	for (index01 = 0; index01 < mTotalMonsterSummonInfoNum; index01++)
	{
		for (index02 = 0; index02 < mMONSTER_SUMMON[index01].mVALUE04; index02++)
		{
			...
			if (tMONSTER_INFO == nullptr)
			{
				index03++;
				continue;
			}
			tRandom = rand();
			mMonsterSummonPostTime[index03] = tMONSTER_INFO->mSummonTime[0] + static_cast<int>(((float)(tMONSTER_INFO->
					mSummonTime[1] - tMONSTER_INFO->mSummonTime[0]) * (float)(tRandom % 101)) /
				100.0f);
			index03++;
		}
	}
...
void MySummonSystem::SummonMonster(void)
{
	...
	for (index01 = 0; index01 < mTotalMonsterSummonInfoNum; index01++)
	{
		for (index02 = 0; index02 < mMONSTER_SUMMON[index01].mVALUE04; index02++)
		{
			...
			if ((static_cast<float>(GetTickCount64() - mMONSTER_OBJECT[index03].mInvalidTimeForSummon) / 1000.0f) <
				static_cast<float>(mMonsterSummonPostTime[index03]))
			{
				index03++;
				continue;
			}
			...
			{
				tRandom = rand();
				tRadius = static_cast<float>(tRandom % mMONSTER_SUMMON[index01].mRADIUS);
			}
			else
			{
				tRadius = 0.0f;
			}
			tLocation[0] = static_cast<float>(mMONSTER_SUMMON[index01].mCENTER[0]) + tDirection[0] * tRadius;
			tLocation[2] = static_cast<float>(mMONSTER_SUMMON[index01].mCENTER[2]) + tDirection[2] * tRadius;
			...
			tRandom = rand();
			mMONSTER_OBJECT[index03].mDATA.mAction.aFront = static_cast<float>(tRandom % 360);
			...
			tRandom = rand();
			mMONSTER_OBJECT[index03].mSameTargetPostNum = tMONSTER_INFO->mFollowInfo[0] + (tRandom % (tMONSTER_INFO->
				mFollowInfo[1] - tMONSTER_INFO->mFollowInfo[0] + 1));
			...
			mMONSTER_OBJECT[index03].mRadiusForSize = sqrtf(
				static_cast<float>(tMONSTER_INFO->mSize[0] * tMONSTER_INFO->mSize[0] + tMONSTER_INFO->mSize[2] *
					tMONSTER_INFO->mSize[
						2])) * 0.5f;
			//------------------------//
			//BROADCAST_MONSTER_ACTION//
			//------------------------//
			mTRANSFER.B_MONSTER_ACTION_RECV(index03, mMONSTER_OBJECT[index03].mUniqueNumber,
			                                &mMONSTER_OBJECT[index03].mDATA, 1);
			//-----------------------------------------------------------------//
			//[01].일반,[02].성석,[03].수호비,[04].동맹석,[05].수호석,[06].석차//
			//-----------------------------------------------------------------//
			...
			{
			case 1:
				mUTIL.Broadcast(TRUE, &mMONSTER_OBJECT[index03].mDATA.mAction.aLocation[0], UNIT_SCALE_RADIUS1, false,
				                0);
				break;
			case 2:
				mUTIL.Broadcast(TRUE, &mMONSTER_OBJECT[index03].mDATA.mAction.aLocation[0], UNIT_SCALE_RADIUS3, false,
				                0);
				break;
			case 3:
				mUTIL.Broadcast(TRUE, &mMONSTER_OBJECT[index03].mDATA.mAction.aLocation[0], UNIT_SCALE_RADIUS3, false,
			...
			case 4:
				...
				break;
			case 5:
				mUTIL.Broadcast(TRUE, &mMONSTER_OBJECT[index03].mDATA.mAction.aLocation[0], UNIT_SCALE_RADIUS2, false,
				                0);
				break;
			case 6:
				mUTIL.Broadcast(TRUE, &mMONSTER_OBJECT[index03].mDATA.mAction.aLocation[0], UNIT_SCALE_RADIUS3, false,
...
void MySummonSystem::SummonBossMonster(void)
{
	...
	case 1:
		...
		for (index01 = MAX_MONSTER_OBJECT_BASE_5_NUM; index01 < MAX_MONSTER_OBJECT_NUM; index01++)
		{
			...
			mMONSTER_OBJECT[index01].mDATA.mAction.aLocation[0] = static_cast<float>(mMONSTER_SUMMON[
				tSelectRegionInfoIndex].mCENTER[0]);
			mMONSTER_OBJECT[index01].mDATA.mAction.aLocation[1] = static_cast<float>(mMONSTER_SUMMON[
				tSelectRegionInfoIndex].mCENTER[1]);
			mMONSTER_OBJECT[index01].mDATA.mAction.aLocation[2] = static_cast<float>(mMONSTER_SUMMON[
				tSelectRegionInfoIndex].mCENTER[2]);
			...
			tRandom = rand();
			mMONSTER_OBJECT[index01].mDATA.mAction.aFront = static_cast<float>(tRandom % 360);
			...
			tRandom = rand();
			mMONSTER_OBJECT[index01].mSameTargetPostNum = tMONSTER_INFO->mFollowInfo[0] + (tRandom % (tMONSTER_INFO->
				mFollowInfo[1] - tMONSTER_INFO->mFollowInfo[0] + 1));
			mMONSTER_OBJECT[index01].mInvalidTimeForSummon = GetTickCount64();
			mMONSTER_OBJECT[index01].mCheckFirstLocationTime = GetTickCount64();
			mMONSTER_OBJECT[index01].mFirstLocation[0] = static_cast<float>(mMONSTER_SUMMON[tSelectRegionInfoIndex].
				mCENTER[0]);
			mMONSTER_OBJECT[index01].mFirstLocation[1] = static_cast<float>(mMONSTER_SUMMON[tSelectRegionInfoIndex].
				mCENTER[1]);
			mMONSTER_OBJECT[index01].mFirstLocation[2] = static_cast<float>(mMONSTER_SUMMON[tSelectRegionInfoIndex].
				mCENTER[2]);
			...
			mMONSTER_OBJECT[index01].mRadiusForSize = sqrtf(
				static_cast<float>(tMONSTER_INFO->mSize[0] * tMONSTER_INFO->mSize[0] + tMONSTER_INFO->mSize[2] *
					tMONSTER_INFO->mSize[
						2])) * 0.5f;
			//------------------------//
			...
			mTRANSFER.B_MONSTER_ACTION_RECV(index01, mMONSTER_OBJECT[index01].mUniqueNumber,
			                                &mMONSTER_OBJECT[index01].mDATA, 1);
			//-----------------------------------------------------------------//
			//[01].일반,[02].성석,[03].수호비,[04].동맹석,[05].수호석,[06].석차//
			//-----------------------------------------------------------------//
			...
			{
			case 1:
				mUTIL.Broadcast(TRUE, &mMONSTER_OBJECT[index01].mDATA.mAction.aLocation[0], UNIT_SCALE_RADIUS1, false,
				                0);
				break;
			case 2:
				mUTIL.Broadcast(TRUE, &mMONSTER_OBJECT[index01].mDATA.mAction.aLocation[0], UNIT_SCALE_RADIUS3, false,
				                0);
				break;
			case 3:
				mUTIL.Broadcast(TRUE, &mMONSTER_OBJECT[index01].mDATA.mAction.aLocation[0], UNIT_SCALE_RADIUS3, false,
			...
			case 4:
				...
				break;
			case 5:
				mUTIL.Broadcast(TRUE, &mMONSTER_OBJECT[index01].mDATA.mAction.aLocation[0], UNIT_SCALE_RADIUS2, false,
				                0);
				break;
			case 6:
				mUTIL.Broadcast(TRUE, &mMONSTER_OBJECT[index01].mDATA.mAction.aLocation[0], UNIT_SCALE_RADIUS3, false,
...
void MySummonSystem::SummonMonsterForSpecial(int tMonsterNumber, float tMonsterLocation[3], BOOL tCheckExistMonster,
                                             int tUserIndex /*= -1*/)
{
	int index01;
	unsigned int tRandom = 0;
	...
	mMONSTER_OBJECT[index01].mMONSTER_INFO = tMONSTER_INFO;
	tRandom = rand();
	mMONSTER_OBJECT[index01].mSameTargetPostNum = tMONSTER_INFO->mFollowInfo[0] + (tRandom % (tMONSTER_INFO->mFollowInfo
		[1] - tMONSTER_INFO->mFollowInfo[0] + 1));
	mMONSTER_OBJECT[index01].mInvalidTimeForSummon = GetTickCount64();
	mMONSTER_OBJECT[index01].mCheckFirstLocationTime = GetTickCount64();
	...
	mMONSTER_OBJECT[index01].mCheckAttackPacketRecv = FALSE;
	mMONSTER_OBJECT[index01].mRadiusForSize = sqrtf(
		static_cast<float>(tMONSTER_INFO->mSize[0] * tMONSTER_INFO->mSize[0] + tMONSTER_INFO->mSize[2] * tMONSTER_INFO->
			mSize[
				2])) * 0.5f;
	memset(mMONSTER_OBJECT[index01].mAvatarName, 0, MAX_AVATAR_NAME_LENGTH);
	if (tUserIndex != -1)
	{
		// 몬스터 소유자 정보 지정 - 지정된 소유자 외에는 타격 불가.
		memcpy(mMONSTER_OBJECT[index01].mAvatarName, mUSER[tUserIndex].mAvatarInfo.aName, MAX_AVATAR_NAME_LENGTH);
	}
	//------------------------//
	//BROADCAST_MONSTER_ACTION//
	//------------------------//
	mTRANSFER.B_MONSTER_ACTION_RECV(index01, mMONSTER_OBJECT[index01].mUniqueNumber, &mMONSTER_OBJECT[index01].mDATA,
	                                1);
	//-----------------------------------------------------------------//
	//[01].일반,[02].성석,[03].수호비,[04].동맹석,[05].수호석,[06].석차//
	//-----------------------------------------------------------------//
...
void MySummonSystem::SummonGuard(BOOL tCheckZone038, BOOL tCheckFirst)
{
	...
	for (index01 = 0; index01 < 5; index01++)
	{
		...
		for (index02 = 0; index02 < tGuardTribePostNum; index02++)
		{
			...
			{
				if ((static_cast<float>(GetTickCount64() - mMONSTER_OBJECT[index03].mInvalidTimeForSummon) / 1000.0f) <
					static_cast<float>(tMONSTER_INFO->mSummonTime[0]))
				{
					index03++;
					continue;
				}
...
void MySummonSystem::SummonMonsterWithoutTimeLimitForZone175Type(int tStep)
{
	...
	for (index01 = 0; index01 < mTotalMonsterSummonInfoNum; index01++)
	{
		for (index02 = 0; index02 < mMONSTER_SUMMON[index01].mVALUE04; index02++)
		{
			...
			mMONSTER_OBJECT[index03].mDATA.mAction.aSort = 0;
			mMONSTER_OBJECT[index03].mDATA.mAction.aFrame = 0.0f;
			mMONSTER_OBJECT[index03].mDATA.mAction.aLocation[0] = static_cast<float>(mMONSTER_SUMMON[index01].mCENTER[
				0]);
			mMONSTER_OBJECT[index03].mDATA.mAction.aLocation[1] = static_cast<float>(mMONSTER_SUMMON[index01].mCENTER[
				1]);
			mMONSTER_OBJECT[index03].mDATA.mAction.aLocation[2] = static_cast<float>(mMONSTER_SUMMON[index01].mCENTER[
				2]);
			mMONSTER_OBJECT[index03].mDATA.mAction.aTargetLocation[0] = 0.0f;
			mMONSTER_OBJECT[index03].mDATA.mAction.aTargetLocation[1] = 0.0f;
			mMONSTER_OBJECT[index03].mDATA.mAction.aTargetLocation[2] = 0.0f;
			...
			mMONSTER_OBJECT[index03].mInvalidTimeForSummon = GetTickCount64();
			mMONSTER_OBJECT[index03].mCheckFirstLocationTime = GetTickCount64();
			mMONSTER_OBJECT[index03].mFirstLocation[0] = static_cast<float>(mMONSTER_SUMMON[index01].mCENTER[0]);
			mMONSTER_OBJECT[index03].mFirstLocation[1] = static_cast<float>(mMONSTER_SUMMON[index01].mCENTER[1]);
			mMONSTER_OBJECT[index03].mFirstLocation[2] = static_cast<float>(mMONSTER_SUMMON[index01].mCENTER[2]);
			mMONSTER_OBJECT[index03].mCheckDetectEnemyTime = GetTickCount64();
			mMONSTER_OBJECT[index03].mCheckAttackState = FALSE;
			mMONSTER_OBJECT[index03].mTotalAttackNum = 0;
			mMONSTER_OBJECT[index03].mUpdateTimeForBroadcast = GetTickCount64();
...
void MySummonSystem::SummonMonsterForZone175Type(void)
{
	...
	for (index01 = 0; index01 < mTotalMonsterSummonInfoNum; index01++)
	{
		for (index02 = 0; index02 < mMONSTER_SUMMON[index01].mVALUE04; index02++)
		{
			...
			if ((tMONSTER_INFO->mSpecialType == 40) || (tMONSTER_INFO->mSpecialType == 41) || (tMONSTER_INFO->
				mSpecialType == 42) || (tMONSTER_INFO->mSpecialType == 43) || (tMONSTER_INFO->mSpecialType == 44))
			{
				index03++;
				continue;
			}
